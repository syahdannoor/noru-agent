...
2. **Isolated NORU_HOME.** NORU_HOME points to a per-test tempdir so
   code reading ``~/.noru/*`` via ``get_noru_home()`` can't see the
   real one. (We do NOT also redirect HOME — that broke subprocesses in
   CI. Code using ``Path.home() / ".hermes"`` instead of the canonical
   ``get_noru_home()`` is a bug to fix at the callsite.)
...
_HERMES_BEHAVIORAL_VARS = frozenset({
    ...
    # the real ~/.noru/kanban.db instead of the per-test NORU_HOME.
    "HERMES_KANBAN_DB",
    "HERMES_KANBAN_BOARD",
    "HERMES_KANBAN_HOME",
...
def _hermetic_environment(tmp_path, monkeypatch):
    ...
    Also redirects HOME and NORU_HOME to per-test tempdirs so code that
    reads ``~/.noru/*`` can't touch the real one, and pins TZ/LANG so
    ...
    # 3. Redirect NORU_HOME to a per-test tempdir. Code that reads
    #    ``~/.noru/*`` via ``get_noru_home()`` now gets the tempdir.
    #
    #    NOTE: We do NOT also redirect HOME. Doing so broke CI because
    #    some tests (and their transitive deps) spawn subprocesses that
    #    inherit HOME and expect it to be stable. If a test genuinely
    #    needs HOME isolated, it should set it explicitly in its own
    #    fixture. Any code in the codebase reading ``~/.noru/*`` via
    #    ``Path.home() / ".hermes"`` instead of ``get_noru_home()``
    ...
    monkeypatch.setenv("NORU_HOME", str(fake_hermes_home))
    ...
    #    ~/.noru/plugins/ (which, per step 3, is now empty — but the
    #    singleton might still be cached from a previous test).
    try:
        import noru_cli.plugins as _plugins_mod
    ...
    def _is_process_killer(cmd) -> bool:
        ...
        for tok in tokens:
            ...
            if head in _PROCESS_KILLERS:
                ...
                # whose cmdline contains "python -m noru_cli.main".
                if (
                    "noru" in low
                    or "gateway" in low
