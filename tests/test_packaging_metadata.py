...
def test_every_on_disk_subpackage_is_covered_by_packages_find():
    ...
    itself and ``noru_cli.*`` for its subpackages — EXCEPT when someone
    forgets the wildcard. v0.15.x listed ``hermes_cli`` without ``noru_cli.*``,
    ...
    ``ModuleNotFoundError: No module named 'noru_cli.dashboard_auth'``.
