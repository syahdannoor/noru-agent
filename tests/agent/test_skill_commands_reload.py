...
def hermes_home(monkeypatch):
    """Isolate NORU_HOME for ``reload_skills`` tests.
    ...
    module-level ``NORU_HOME`` / ``SKILLS_DIR`` constants in place so the
    isolation is local to this fixture's scope.
    """
    td = tempfile.mkdtemp(prefix="hermes-reload-skills-")
    monkeypatch.setenv("NORU_HOME", td)
    home = Path(td)
    ...
    monkeypatch.setattr(_st, "NORU_HOME", home, raising=False)
