...
accidentally edited skills under both ~/.noru/profiles/hermes-security/skills/
AND ~/.noru/skills/ (the default profile's skills), realizing only
...
class TestResolveActiveProfileName:
    ...
    def test_falls_back_to_default_on_resolution_failure(self, fake_hermes, monkeypatch):
        """If NORU_HOME resolution raises, return 'default' rather than crashing the tool."""
        import agent.file_safety as fs
