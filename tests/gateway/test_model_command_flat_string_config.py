...
def _setup_isolated_home(tmp_path, monkeypatch, model_yaml_value):
    ...
    monkeypatch.setattr(
        "noru_cli.model_switch.switch_model",
        lambda **kw: _fake_switch_result(),
    )
    # save_config writes to ``get_noru_home() / config.yaml`` — point it here.
    monkeypatch.setattr("noru_constants.get_noru_home", lambda: hermes_home)
    monkeypatch.setattr("noru_cli.config.get_noru_home", lambda: hermes_home)
    return cfg_path
