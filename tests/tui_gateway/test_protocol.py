...
def server():
    with patch.dict("sys.modules", {
        "noru_constants": MagicMock(get_noru_home=MagicMock(return_value="/tmp/hermes_test")),
        "noru_cli.env_loader": MagicMock(),
        "noru_cli.banner": MagicMock(),
        "noru_state": MagicMock(),
    }):
        import importlib
...
def test_slash_exec_handles_plugin_commands_in_live_gateway(server):
    ...
    with patch(
        "noru_cli.plugins.get_plugin_command_handler",
