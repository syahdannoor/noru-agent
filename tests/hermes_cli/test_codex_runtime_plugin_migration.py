...
from noru_cli.codex_runtime_plugin_migration import (
    MIGRATION_MARKER,
...
class TestMigrate:
    ...
    def test_plugin_discovery_writes_plugin_blocks(self, tmp_path, monkeypatch):
        ...
        from noru_cli import codex_runtime_plugin_migration as crpm
    ...
    def test_plugin_discovery_skips_unavailable_plugins(self):
        ...
        from noru_cli.codex_runtime_plugin_migration import _query_codex_plugins
...
class TestStripUnmanagedPluginTables:
    ...
    def test_migrate_dedups_codex_owned_plugin_tables(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(
            "noru_cli.codex_runtime_plugin_migration._query_codex_plugins",
            fake_query,
        )
...
# ---- Bug C: NORU_HOME tempdir leak into ~/.codex/config.toml ----


class TestHermesHomeLeakGuard:
    """Regression tests for issue #26250 Bug C.

    Previously ``_build_hermes_tools_mcp_entry()`` read ``NORU_HOME``
    ...
    def test_pytest_tempdir_not_burned_into_mcp_env(self, monkeypatch):
        """The headline regression: even when NORU_HOME points at a pytest
        ...
        env = entry.get("env", {})
        assert "NORU_HOME" not in env, (
            f"pytest-tempdir NORU_HOME leaked into codex MCP entry: "
            f"{env.get('NORU_HOME')!r}"
        )

    def test_real_hermes_home_propagates(self, monkeypatch, tmp_path):
        """A legitimate NORU_HOME (not a tempdir path) DOES propagate so the
        ...
        monkeypatch.setenv("NORU_HOME", real_path)
        entry = _build_hermes_tools_mcp_entry()
        env = entry.get("env", {})
        assert env.get("NORU_HOME") == real_path

    def test_unset_hermes_home_omits_env_key(self, monkeypatch):
        """When NORU_HOME is unset in the environment, the MCP entry MUST
        NOT bake in a resolved-default path. The codex subprocess should
        inherit whatever NORU_HOME its launcher (systemd, gateway, shell)
        ...
        env = entry.get("env", {})
        assert "NORU_HOME" not in env, (
            f"NORU_HOME should not be set when env var is unset, got: "
            f"{env.get('NORU_HOME')!r}"
        )
