...
def _run_user_provider_override_case(
    *,
    ...
    with patch("noru_cli.model_switch.resolve_alias", return_value=None), \
         patch("noru_cli.model_switch.list_provider_models", return_value=[]), \
         patch("noru_cli.model_switch.normalize_model_for_provider", side_effect=lambda model, provider: model), \
         patch("noru_cli.models.validate_requested_model", return_value=_REJECTED_VALIDATION), \
         ...
         patch("noru_cli.runtime_provider.resolve_runtime_provider", return_value={"api_key": "***", "base_url": base_url, "api_mode": "anthropic_messages"}):
        return switch_model(
            raw_input=raw_input,
