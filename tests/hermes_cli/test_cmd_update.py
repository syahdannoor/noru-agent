...
from noru_cli.main import cmd_update, PROJECT_ROOT
...
class TestCmdUpdatePip:
    ...
    def test_update_pip_exports_virtualenv_from_sys_prefix(
        ...
        from noru_cli import main as hm
        ...
        assert mock_run.call_args.args[0] == ["/usr/bin/uv", "pip", "install", "--upgrade", "noru-agent"]
...
class TestCmdUpdateBranchFallback:
    ...
    def test_update_on_fork_checks_upstream_when_origin_up_to_date(
        ...
        origin but behind syahdannoor/noru-agent silently misses updates.
        """
        from noru_cli import main as hm
    ...
    def test_update_non_interactive_runs_safe_config_migrations(self, mock_args, capsys):
        ...
        with patch("shutil.which", return_value=None), patch(
            ...
            "noru_cli.config.get_missing_env_vars", return_value=["MISSING_KEY"]
        ), patch(
            "noru_cli.config.get_missing_config_fields",
            return_value=[{"key": "new.option", "default": True}],
        ), patch("noru_cli.config.check_config_version", return_value=(1, 2)), patch(
            "noru_cli.config.migrate_config",
            return_value={"env_added": [], "config_added": ["new.option"]},
        ), patch("noru_cli.main.sys") as mock_sys:
            ...
            from noru_cli.config import migrate_config
...
class TestCmdUpdateMigrationPrompt:
    ...
    def test_version_bump_only_applies_silently_without_prompt(
        ...
        with patch("shutil.which", return_value=None), patch(
            ...
            "noru_cli.config.get_missing_env_vars", return_value=[]
        ), patch(
            "noru_cli.config.get_missing_config_fields", return_value=[]
        ), patch(
            "noru_cli.config.check_config_version", return_value=(5, 24)
        ), patch(
            "noru_cli.config.migrate_config",
    ...
    def test_new_options_are_listed_by_name_before_prompt(
        ...
        with patch("shutil.which", return_value=None), patch(
            ...
            "noru_cli.config.get_missing_env_vars", return_value=env_items
        ), patch(
            "noru_cli.config.get_missing_config_fields", return_value=cfg_items
        ), patch(
            "noru_cli.config.check_config_version", return_value=(1, 24)
        ), patch(
            "noru_cli.config.migrate_config",
            return_value={"env_added": [], "config_added": [], "warnings": []},
        ), patch("noru_cli.main.sys") as mock_sys:
...
class TestCmdUpdateProfileSkillSync:
    ...
    def test_active_profile_included_in_skill_sync(
        ...
        with (
            patch("noru_cli.profiles.list_profiles", return_value=all_profiles),
            patch("noru_cli.profiles.seed_profile_skills", side_effect=fake_seed),
    ...
    def test_single_profile_default_is_synced(
        ...
        with (
            patch("noru_cli.profiles.list_profiles", return_value=[default_p]),
            patch("noru_cli.profiles.seed_profile_skills", side_effect=fake_seed),
            patch("tools.skills_sync.sync_skills", return_value=empty_sync),
        ):
            cmd_update(mock_args)
...
class TestCmdUpdateCheckBranchFlag:
    ...
    @patch("noru_cli.config.detect_install_method", return_value="git")
    @patch("subprocess.run")
    ...
    @patch("noru_cli.config.detect_install_method", return_value="pip")
    @patch("noru_cli.banner.check_via_pypi", return_value=0)
    @patch("subprocess.run")
...
class TestCmdUpdateZipBranchRefusal:
    ...
    def test_zip_fallback_refuses_non_main_branch(self, capsys):
        from noru_cli.main import _update_via_zip
