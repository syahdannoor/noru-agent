...
When ``NORU_HOME`` points to a named profile, ``read_credential_pool()``
...
def profile_env(tmp_path, monkeypatch):
    ...
    * Profile     -> tmp_path/.hermes/profiles/coder   (active, NORU_HOME points here)
...
def test_load_provider_state_falls_back_to_global(profile_env):
    ...
    from noru_cli.auth import _load_auth_store, _load_provider_state
...
def test_classic_mode_does_not_double_read_same_file(tmp_path, monkeypatch):
    """In classic mode (NORU_HOME == global root), no fallback path runs.
    ...
    # to the NORU_HOME we set (tmp_path/classic), so the guard passes.
    fake_home = tmp_path / "home"
    fake_home.mkdir()
    monkeypatch.setattr(Path, "home", lambda: fake_home)
    hermes_home = tmp_path / "classic"
    ...
    from noru_cli.auth import read_credential_pool, _global_auth_file_path

    # Classic mode: NORU_HOME is set to a custom path that is NOT under
    # ~/.noru/profiles/ — get_default_hermes_root() returns NORU_HOME
...
def test_write_credential_pool_targets_profile_not_global(profile_env):
    from noru_cli.auth import read_credential_pool, write_credential_pool
