...
from noru_cli.main, these tests mock the delegation point and verify
that the setup wizard correctly syncs config from disk after the call.
"""

from __future__ import annotations

from noru_cli.config import load_config, save_config, save_env_value
from noru_cli.nous_subscription import NousFeatureState, NousSubscriptionFeatures
from noru_cli.setup import _print_setup_summary, setup_model_provider
...
def test_setup_copilot_acp_skips_same_provider_pool_step(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("noru_cli.setup.prompt_yes_no", fake_prompt_yes_no)
    monkeypatch.setattr("noru_cli.setup.prompt", lambda *args, **kwargs: "")
    monkeypatch.setattr("noru_cli.auth.get_active_provider", lambda: None)
