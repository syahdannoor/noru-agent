...
def _make_hermes_tree(root: Path) -> None:
    ...
    (root / "noru_state.db").write_bytes(b"fake-state")
    ...
    (root / "noru-agent").mkdir(exist_ok=True)
    (root / "noru-agent" / "run_agent.py").write_text("# big file\n")
    (root / "noru-agent" / ".git").mkdir()
    (root / "noru-agent" / ".git" / "HEAD").write_text("ref: refs/heads/main\n")
...
class TestShouldExclude:
    def test_excludes_noru_agent(self):
        from noru_cli.backup import _should_exclude
...
class TestBackup:
    ...
    def test_excludes_noru_agent(self, tmp_path, monkeypatch):
        ...
        with zipfile.ZipFile(out_zip, "r") as zf:
            names = zf.namelist()
            agent_files = [n for n in names if "noru-agent" in n]
    ...
    def test_skips_symlinked_files(self, tmp_path, monkeypatch):
        """Backup must not dereference symlinks and leak files outside NORU_HOME."""
        hermes_home = tmp_path / ".hermes"
        hermes_home.mkdir()
        _make_hermes_tree(hermes_home)
...
class TestValidateBackupZip:
    ...
    def test_state_db_passes(self, tmp_path):
        ...
        from noru_cli.backup import _validate_backup_zip
    ...
    def test_old_wrong_db_name_fails(self, tmp_path):
        """A zip with only noru_state.db (old wrong name) is rejected."""
        from noru_cli.backup import _validate_backup_zip
        zip_path = tmp_path / "old.zip"
        self._make_zip(zip_path, ["noru_state.db", "memory_store.db"])
        with zipfile.ZipFile(zip_path, "r") as zf:
            ok, reason = _validate_backup_zip(zf)
        assert not ok
...
class TestRoundTrip:
    def test_backup_then_import(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("NORU_HOME", str(src_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path / "source")

        # Backup
        out_zip = tmp_path / "roundtrip.zip"
        from noru_cli.backup import run_backup, run_import
        ...
        monkeypatch.setenv("NORU_HOME", str(dst_home))
        ...
        assert not (dst_home / "noru-agent").exists()
...
class TestFormatSize:
    def test_bytes(self):
        from noru_cli.backup import _format_size
...
class TestValidation:
    ...
    def test_detect_prefix_hermes(self):
        ...
        import io
        from noru_cli.backup import _detect_prefix
...
class TestBackupEdgeCases:
    def test_nonexistent_hermes_home(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("NORU_HOME", str(fake_home))
...
class TestSafeCopyDb:
    def test_copies_valid_database(self, tmp_path):
        from noru_cli.backup import _safe_copy_db
...
class TestQuickSnapshot:
    ...
    def hermes_home(self, tmp_path):
        """Create a fake NORU_HOME with critical state files."""
    ...
    def test_list_snapshots(self, hermes_home):
        from noru_cli.backup import create_quick_snapshot, list_quick_snapshots
    ...
    def test_restore_config(self, hermes_home):
        from noru_cli.backup import create_quick_snapshot, restore_quick_snapshot
        snap_id = create_quick_snapshot(hermes_home=hermes_home)
    ...
    def test_restore_nonexistent(self, hermes_home):
        from noru_cli.backup import restore_quick_snapshot
        assert restore_quick_snapshot("nonexistent", hermes_home=hermes_home) is False

    def test_auto_prune(self, hermes_home):
        from noru_cli.backup import create_quick_snapshot, list_quick_snapshots, _QUICK_DEFAULT_KEEP
    ...
    def test_manual_prune(self, hermes_home):
        from noru_cli.backup import create_quick_snapshot, prune_quick_snapshots, list_quick_snapshots
        for i in range(10):
            create_quick_snapshot(label=f"s{i}", hermes_home=hermes_home)
...
class TestPreUpdateBackup:
    ...
    def test_skips_symlinked_files(self, hermes_home, tmp_path):
        """Pre-update backups must not dereference symlinks outside NORU_HOME."""
        from noru_cli.backup import create_pre_update_backup
...
class TestRunPreUpdateBackup:
    ...
    def hermes_home(self, tmp_path, monkeypatch):
        ...
        root.mkdir()
        _make_hermes_tree(root)
        # Point NORU_HOME at the temp dir so config + backup paths resolve here
        monkeypatch.setenv("NORU_HOME", str(root))
        # Make Path.home() point at tmp_path for anything that uses it
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        # Bust caches for noru_cli.config + noru_constants so they pick up NORU_HOME
        for mod in list(__import__("sys").modules.keys()):
            if mod.startswith("noru_cli.config") or mod == "noru_constants":
    ...
    def test_backup_flag_creates_backup(self, hermes_home, capsys):
        ...
        from noru_cli.main import _run_pre_update_backup
    ...
    def test_config_enabled_creates_backup(self, hermes_home, capsys):
        ...
        for mod in list(_sys.modules.keys()):
            if mod.startswith("noru_cli.config"):
...
class TestPreMigrationBackup:
    ...
    ``hermes claw migrate`` runs before mutating ~/.noru/."""
    ...
    def test_creates_backup_under_backups_dir(self, hermes_home):
        from noru_cli.backup import create_pre_migration_backup
    ...
    def test_restorable_with_hermes_import(self, hermes_home, tmp_path):
        ...
        from noru_cli.backup import create_pre_migration_backup, _validate_backup_zip
        out = create_pre_migration_backup(hermes_home=hermes_home)
        assert out is not None
        with zipfile.ZipFile(out) as zf:
    ...
    def test_does_not_touch_pre_update_backups(self, hermes_home):
        ...
        from noru_cli.backup import create_pre_update_backup, create_pre_migration_backup
