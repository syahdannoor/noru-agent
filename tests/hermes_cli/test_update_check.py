"""Tests for the update check mechanism in noru_cli.banner."""
...
def test_check_for_updates_uses_cache(tmp_path, monkeypatch):
    ...
    with patch("noru_cli.banner.subprocess.run") as mock_run:
...
def test_check_for_updates_invalidates_on_version_change(tmp_path, monkeypatch):
    ...
    """
    import noru_cli.banner as banner
    ...
    with patch("noru_cli.banner.subprocess.run") as mock_run, \
         patch("noru_cli.banner.check_via_pypi", return_value=0) as mock_pypi:
...
def test_check_for_updates_expired_cache(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("NORU_HOME", str(tmp_path))
    with patch("noru_cli.banner.subprocess.run", return_value=mock_result) as mock_run:
        result = check_for_updates()
...
def test_check_for_updates_no_git_dir(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("NORU_HOME", str(tmp_path))
    with patch("noru_cli.banner.subprocess.run") as mock_run:
        with patch("noru_cli.banner.check_via_pypi", return_value=0):
            result = banner.check_for_updates()
    assert result == 0
    mock_run.assert_not_called()
...
def test_check_for_updates_fallback_to_project_root(tmp_path, monkeypatch):
    """Dev install: falls back to Path(__file__).parent.parent when NORU_HOME has no git repo."""
    import noru_cli.banner as banner
    ...
    # Point NORU_HOME at a temp dir with no hermes-agent/.git
    monkeypatch.setenv("NORU_HOME", str(tmp_path))
    with patch("noru_cli.banner.subprocess.run") as mock_run:
...
def test_invalidate_update_cache_clears_all_profiles(tmp_path):
    ...
    from noru_cli.main import _invalidate_update_cache
    ...
    with patch.object(Path, "home", return_value=tmp_path), \
         patch.dict(os.environ, {"NORU_HOME": str(default_home)}):
