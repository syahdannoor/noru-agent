...
NORU_HOME scoping, and tick() serialization for profile jobs.
"""
...
class TestRunJobProfileContext:
    ...
    def _install_agent_stubs(monkeypatch, observed: dict):
        ...
        class FakeAgent:
            def __init__(self, **kwargs):
                ...
                observed["env_home_during_init"] = os.environ.get("NORU_HOME")
                ...
                observed["hermes_home_during_init"] = str(get_noru_home())
                observed["scheduler_home_during_init"] = str(sched._get_noru_home())
                observed["skip_context_files"] = kwargs.get("skip_context_files")

            def run_conversation(self, *_a, **_kw):
                from noru_constants import get_noru_home

                observed["env_home_during_run"] = os.environ.get("NORU_HOME")
                ...
                observed["hermes_home_during_run"] = str(get_noru_home())
                observed["scheduler_home_during_run"] = str(sched._get_noru_home())
        ...
        from noru_cli import runtime_provider as runtime_provider
    ...
    def test_run_job_sets_and_restores_profile_home(
        ...
        assert os.environ["NORU_HOME"] == str(root)
        assert sched._get_noru_home() == root
    ...
    def test_no_agent_profile_uses_profile_scripts_dir_and_restores_env(
        ...
        (scripts_dir / "print_home.py").write_text(
            "import os\nprint(os.environ.get('NORU_HOME', ''))\n",
            encoding="utf-8",
        )
        monkeypatch.setattr(sched, "_hermes_home", None)
