...
def _restore_tool_modules():
    original_hermes_home = os.environ.get("NORU_HOME")
    original_modules = {
        name: module
    ...
    try:
        ...
        if original_hermes_home is None:
            ...
            os.environ["NORU_HOME"] = original_hermes_home
...
def _install_modal_test_modules(
    ...
    *,
    ...
    os.environ["NORU_HOME"] = str(hermes_home)
    sys.modules["noru_cli.config"] = types.SimpleNamespace(
        get_noru_home=lambda: hermes_home,
    )
    ...
    async def _lookup_aio(_name: str, create_if_missing: bool = False):
        return types.SimpleNamespace(name="noru-agent", create_if_missing=create_if_missing)
