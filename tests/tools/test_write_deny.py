...
class TestWriteDenyExactPaths:
    ...
    def test_hermes_env(self):
        # ``.env`` under the active NORU_HOME (profile-aware, not just
        # ``~/.hermes``) must be write-denied. The hermetic test conftest
        # points NORU_HOME at a tempdir — resolve via get_noru_home()
        # to match the denylist.
        from noru_constants import get_noru_home
        path = str(get_noru_home() / ".env")
        assert _is_write_denied(path) is True
    ...
    def test_hermes_root_env_when_running_under_profile(self, tmp_path, monkeypatch):
        ...
        ``~/.noru/.env`` (whose credentials are inherited by every profile)
        ...
        # Sanity check: NORU_HOME does point to the profile dir, not the root.
        from noru_constants import get_noru_home, get_default_hermes_root
        assert get_noru_home() == profile_home
