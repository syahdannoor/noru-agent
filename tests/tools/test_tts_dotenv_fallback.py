...
TTS provider tools must resolve API keys from ``~/.noru/.env`` (via
``noru_cli.config.get_env_value``) and not only from ``os.environ`` —
...
class TestDotenvFallbackPerProvider:
    """For each affected provider, when only ``~/.noru/.env`` carries the
    ...
    ``noru_cli.config.load_env`` integration. Before the fix, ``os.getenv``
...
class TestRegressionGuard:
    ...
    ``noru_cli.config.load_env`` to simulate ``~/.noru/.env`` carrying the
    ...
    def test_import_after_config_env_patch_uses_restored_dotenv_loader(self, tmp_path, monkeypatch):
        """Importing TTS while noru_cli.config.get_env_value is patched must
    ...
    def test_minimax_missing_when_only_in_dotenv_before_fix(self, tmp_path, monkeypatch):
        ...
        # Simulate ~/.noru/.env carrying the key (load_env returns the dict
        ...
        with patch(
            ...
            # os.environ is empty.
            from noru_cli.config import get_env_value as live_get
    ...
    def test_check_tts_requirements_sees_dotenv_minimax(self, monkeypatch):
        ...
        in ``~/.noru/.env``, even though the dispatcher would later succeed.
        """
        from tools import tts_tool
