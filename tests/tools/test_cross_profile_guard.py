...
def fake_hermes(tmp_path, monkeypatch):
    """Build a two-profile Hermes layout and point NORU_HOME at
    ...
    monkeypatch.setenv("NORU_HOME", str(sec_home))

    import noru_constants
    monkeypatch.setattr(noru_constants, "get_default_hermes_root", lambda: root)
...
class TestSkillManageCrossProfileErrorUX:
    ...
    def test_error_names_other_profile_when_skill_lives_there(
        ...
        # Re-import the module so SKILLS_DIR picks up NORU_HOME (set in
...
class TestSystemPromptActiveProfile:
    def test_default_profile_line_in_prompt(self, tmp_path, monkeypatch):
        ...
        about ~/.noru/profiles/<name>/."""
        # Don't set NORU_HOME — falls back to default.
        import agent.file_safety as fs
