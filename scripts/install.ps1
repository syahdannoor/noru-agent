# ============================================================================
# noru Agent Installer for Windows
# ============================================================================
# Installation script for Windows (PowerShell).
# Uses uv for fast Python provisioning and package management.
#
# Usage:
#   iex (irm https://raw.githubusercontent.com/syahdannoor/noru-agent/main/scripts/install.ps1)
#
...
param(
    ...
    [string]$Commit = "",
    [string]$Tag = "",
    [string]$noruHome = $(if ($env:NORU_HOME) { $env:NORU_HOME } else { "$env:LOCALAPPDATA\noru" }),
    [string]$InstallDir = $(if ($env:NORU_HOME) { "$env:NORU_HOME\noru-agent" } else { "$env:LOCALAPPDATA\noru\noru-agent" }),
    ...
    # builds apps/desktop into a launchable noru.exe.
    #
    # Why opt-in:
    #   * noru-Setup.exe (the signed Tauri bootstrap installer) passes
    ...
    #     from inside an already-launched noru.exe; if THAT recursively
    #     built apps/desktop it would try to overwrite the live noru.exe
    ...
    #     `noru desktop` already builds on demand.
...
$RepoUrlSsh = "git@github.com:syahdannoor/noru-agent.git"
$RepoUrlHttps = "https://github.com/syahdannoor/noru-agent.git"
$PythonVersion = "3.11"
$NodeVersion = "22"
...
function Write-Banner {
    ...
    Write-Host "|             * noru Agent Installer                    |" -ForegroundColor Magenta
    Write-Host "+---------------------------------------------------------+" -ForegroundColor Magenta
...
function Write-BrowserEnv {
    ...
    if (-not (Test-Path $noruHome)) {
        New-Item -ItemType Directory -Force -Path $noruHome | Out-Null
    }
    $envFile = Join-Path $noruHome ".env"
...
function Install-AgentBrowser {
    ...
    $prefixDir = Join-Path $noruHome "node"
...
function Install-Uv {
    # noru owns its own uv at $noruHome\bin\uv.exe.  Always install there —
    # no PATH probing, no conda guards, no multi-location resolution chains.
    # The runtime update path (noru_cli/managed_uv.py) looks in the same
    # place, so install.ps1 and `noru update` stay in sync.
    $managedUv = Join-Path $noruHome "bin\uv.exe"
    ...
    Write-Info "Installing managed uv into $noruHome\bin ..."
    New-Item -ItemType Directory -Path (Join-Path $noruHome "bin") -Force | Out-Null

    # UV_INSTALL_DIR tells the astral installer to place the binary
    # directly into $noruHome\bin instead of ~/.local/bin.
    $prevEAP = $ErrorActionPreference
    try {
        $ErrorActionPreference = "Continue"
        $env:UV_INSTALL_DIR = Join-Path $noruHome "bin"
...
function Resolve-UvCmd {
    ...
    # sibling process and NORU_HOME wasn't propagated).
...
function Install-Git {
    ...
    which noru uses to run shell commands.
    ...
    package manager):
      ...
      2. Download **PortableGit** from the official git-for-windows GitHub
         ...
         ``%LOCALAPPDATA%\noru\git`` -- never touches system Git, never
    ...
    **Why PortableGit, not MinGit:**  MinGit is the minimal-automation
    ...
    noru needs ``bash.exe`` to run shell commands.  PortableGit is the
    ...
    blocked).  Owning the noru copy of Git ourselves is predictable and
    ...
    ``noru_GIT_BASH_PATH`` (User scope) so noru can find it in a fresh
    ...
    # Download PortableGit into $noruHome\git.  Always works as long as
    # we can reach github.com -- no admin, no winget, no reliance on the
    # user's possibly-broken system Git install.
    Write-Info "Git not found -- downloading PortableGit to $noruHome\git\ ..."
    ...
    try {
        ...
        if ($arch -eq "32-bit-mingit") {
            Write-Warn "32-bit Windows detected -- PortableGit is 64-bit only.  Installing MinGit 32-bit as a last resort; bash-dependent noru features (terminal tool, agent-browser) will not work on this machine."
        ...
        Write-Info "then re-run this installer.  noru needs Git Bash on Windows to run"
...
function Set-GitBashEnvVar {
    ...
    ``noru_GIT_BASH_PATH`` (User env scope) so noru can find it even before
    ...
    #
    # Layouts:
    #   PortableGit (our default): $noruHome\git\bin\bash.exe
    #   MinGit (32-bit fallback):  $noruHome\git\usr\bin\bash.exe
    $candidates += "$noruHome\git\bin\bash.exe"       # PortableGit layout (primary)
    $candidates += "$noruHome\git\usr\bin\bash.exe"   # MinGit / PortableGit usr\bin fallback
    ...
    foreach ($candidate in $candidates) {
        if ($candidate -and (Test-Path $candidate)) {
            [Environment]::SetEnvironmentVariable("noru_GIT_BASH_PATH", $candidate, "User")
            $env:noru_GIT_BASH_PATH = $candidate
            Write-Info "Set noru_GIT_BASH_PATH=$candidate"
            return
        }
    }

    Write-Warn "Could not locate bash.exe -- noru may not find Git Bash."
    Write-Info "If needed, set noru_GIT_BASH_PATH manually to your bash.exe path."
}
...
function Test-Node {
    ...
    $managedNode = "$noruHome\node\node.exe"
    if (Test-Path $managedNode) {
        $version = & $managedNode --version
        $env:Path = "$noruHome\node;$env:Path"
        Write-Success "Node.js $version found (noru-managed)"
        $script:HasNode = $true
        return $true
    }
    ...
    # The portable zip path drops node.exe + npm into $noruHome\node\
    ...
    Write-Info "Downloading portable Node.js $NodeVersion to $noruHome\node\ ..."
    ...
    try {
        ...
        if ($zipName) {
            ...
            if ($extractedDir) {
                if (Test-Path "$noruHome\node") { Remove-Item -Recurse -Force "$noruHome\node" }
                Move-Item $extractedDir.FullName "$noruHome\node"
                ...
                $version = & "$noruHome\node\node.exe" --version
                Write-Success "Node.js $version installed to $noruHome\node\ (portable, user-scoped)"
                $script:HasNode = $true
...
function Install-SystemPackages {
    ...
    if ($hasWinget) {
        ...
        foreach ($pkg in $wingetPkgs) {
            $log = "$env:TEMP\noru-winget-$($pkg -replace '[^A-Za-z0-9]','_')-$(Get-Random).log"
            $pkgLogs[$pkg] = $log
    ...
    if (Test-Path $InstallDir) {
        ...
        if ($repoValid) {
            ...
            try {
                ...
                Write-Info "terminals, running noru processes) and try again."
                throw
            }
        }
    }
    ...
    if (-not $didUpdate) {
        ...
        if (-not $cloneSuccess) {
            ...
            try {
                ...
                if ($Commit) {
                    $zipUrl = "https://github.com/syahdannoor/noru-agent/archive/$Commit.zip"
                    $zipLabel = $Commit
                } elseif ($Tag) {
                    $zipUrl = "https://github.com/syahdannoor/noru-agent/archive/refs/tags/$Tag.zip"
                    $zipLabel = $Tag
                } else {
                    $zipUrl = "https://github.com/syahdannoor/noru-agent/archive/refs/heads/$Branch.zip"
                    $zipLabel = $Branch
                }
                $zipPath = "$env:TEMP\noru-agent-$zipLabel.zip"
...
function Install-Dependencies {
    ...
    if (Test-Path "uv.lock") {
        ...
        # empty and producing the broken state where `noru.exe` exists
...
"@ 2>$null
    ...
    if (-not $installed) {
        throw "Failed to install noru-agent package even with no extras. Inspect the uv pip install output above."
    }
    ...
    # isn't set, leaving venv\ empty and noru.exe broken with
    ...
    # here, not 30 seconds later when the user runs `noru`.
    if (-not $NoVenv) {
    ...
    # users hit and lazy-import errors from `noru dashboard` are confusing.
    ...
    if (Test-Path $pythonExe) {
        ...
        $ErrorActionPreference = $prevEAP
        if (-not $webOk) {
            Write-Warn "fastapi/uvicorn not importable -- `noru dashboard` will not work."
            Write-Info "Attempting targeted install of [web] extra as last resort..."
            & $UvCmd pip install -e ".[web]"
            if ($LASTEXITCODE -eq 0) {
                Write-Success "[web] extra installed; `noru dashboard` should now work."
            } else {
...
function Set-PathVariable {
    Write-Info "Setting up noru command..."
    
    if ($NoVenv) {
        $noruBin = "$InstallDir"
    } else {
        $noruBin = "$InstallDir\venv\Scripts"
    }
    
    # Add the venv Scripts dir to user PATH so noru is globally available
    # On Windows, the noru.exe in venv\Scripts\ has the venv Python baked in
    $currentPath = [Environment]::GetEnvironmentVariable("Path", "User")
    
    if ($currentPath -notlike "*$noruBin*") {
        [Environment]::SetEnvironmentVariable(
            "Path",
            "$noruBin;$currentPath",
            "User"
        )
        Write-Success "Added to user PATH: $noruBin"
    } else {
        Write-Info "PATH already configured"
    }
    
    # Set NORU_HOME so the Python code finds config/data in the right place.
    # Only needed on Windows where we install to %LOCALAPPDATA%\noru instead
    # of the Unix default ~/.noru
    $currentnoruHome = [Environment]::GetEnvironmentVariable("NORU_HOME", "User")
    if (-not $currentnoruHome -or $currentnoruHome -ne $noruHome) {
        [Environment]::SetEnvironmentVariable("NORU_HOME", $noruHome, "User")
        Write-Success "Set NORU_HOME=$noruHome"
    }
    $env:NORU_HOME = $noruHome
    
    # Update current session
    $env:Path = "$noruBin;$env:Path"
    
    Write-Success "noru command ready"
}

function Write-BootstrapMarker {
    # Writes $InstallDir\.noru-bootstrap-complete which tells the noru
    ...
    $markerPath = Join-Path $InstallDir ".noru-bootstrap-complete"
    $marker = [ordered]@{
        schemaVersion = 1
...
function Copy-ConfigTemplates {
    ...
    # Create ~/.noru directory structure
    New-Item -ItemType Directory -Force -Path "$noruHome\cron" | Out-Null
    New-Item -ItemType Directory -Force -Path "$noruHome\sessions" | Out-Null
    New-Item -ItemType Directory -Force -Path "$noruHome\logs" | Out-Null
    New-Item -ItemType Directory -Force -Path "$noruHome\pairing" | Out-Null
    New-Item -ItemType Directory -Force -Path "$noruHome\hooks" | Out-Null
    New-Item -ItemType Directory -Force -Path "$noruHome\image_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$noruHome\audio_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$noruHome\memories" | Out-Null
    New-Item -ItemType Directory -Force -Path "$noruHome\skills" | Out-Null

    
    # Create .env
    $envPath = "$noruHome\.env"
    if (-not (Test-Path $envPath)) {
        $examplePath = "$InstallDir\.env.example"
        if (Test-Path $examplePath) {
            Copy-Item $examplePath $envPath
            Write-Success "Created ~/.noru/.env from template"
        } else {
            New-Item -ItemType File -Force -Path $envPath | Out-Null
            Write-Success "Created ~/.noru/.env"
        }
    } else {
        Write-Info "~/.noru/.env already exists, keeping it"
    }
    
    # Create config.yaml
    $configPath = "$noruHome\config.yaml"
    if (-not (Test-Path $configPath)) {
        $examplePath = "$InstallDir\cli-config.yaml.example"
        if (Test-Path $examplePath) {
            Copy-Item $examplePath $configPath
            Write-Success "Created ~/.noru/config.yaml from template"
        }
    } else {
        Write-Info "~/.noru/config.yaml already exists, keeping it"
    }
    ...
    # (the default PS5 behaviour), and noru's prompt-injection scanner
...
"@
        ...
        Write-Success "Created ~/.noru/SOUL.md (edit to customize personality)"
    }
    
    Write-Success "Configuration directory ready: ~/.noru/"
    
    # Seed bundled skills into ~/.noru/skills/ (manifest-based, one-time per skill)
    Write-Info "Syncing bundled skills to ~/.noru/skills/ ..."
    $pythonExe = "$InstallDir\venv\Scripts\python.exe"
    if (Test-Path $pythonExe) {
        try {
            & $pythonExe "$InstallDir\tools\skills_sync.py" 2>$null
            Write-Success "Skills synced to ~/.noru/skills/"
        } catch {
            # Fallback: simple directory copy
            $bundledSkills = "$InstallDir\skills"
            $userSkills = "$noruHome\skills"
            if ((Test-Path $bundledSkills) -and -not (Get-ChildItem $userSkills -Exclude '.bundled_manifest' -ErrorAction SilentlyContinue)) {
                Copy-Item -Path "$bundledSkills\*" -Destination $userSkills -Recurse -Force -ErrorAction SilentlyContinue
                Write-Success "Skills copied to ~/.noru/skills/"
            }
        }
    }
}
...
function Install-NodeDeps {
    if (-not $HasNode) {
        # Cross-process driver mode (noru-Setup.exe runs each -Stage NAME
    ...
    if (-not $npmCmd) {
        ...
        Write-Info "Open a new PowerShell window and re-run 'noru setup tools' later."
        return
    }
    $npmExe = $npmCmd.Source
    if ($npmExe -like "*.ps1") {
    ...
    if (Test-Path "$InstallDir\package.json") {
        ...
        $browserLog = "$env:TEMP\noru-npm-browser-$(Get-Random).log"
        ...
        if ($browserNpmOk) {
            ...
            if (-not $npxExe) {
                ...
                $pwLog = "$env:TEMP\noru-playwright-install-$(Get-Random).log"
                Push-Location $InstallDir
    ...
    if (Test-Path "$tuiDir\package.json") {
        ...
        $tuiLog = "$env:TEMP\noru-npm-tui-$(Get-Random).log"
        [void](_Run-NpmInstall "TUI" $tuiDir $tuiLog $npmExe)
    }
}

function Install-Desktop {
    # Build apps/desktop into a launchable noru.exe. Only called from
    ...
    #
    # The Tauri bootstrap installer's launch_noru_desktop command
    # resolves apps/desktop/release/win-unpacked/noru.exe directly,
    ...
    # The --dir mode produces an unpacked noru.exe in
    ...
    # are NOT trying to work around). The noru icon + product name are
    # stamped onto noru.exe by our own rcedit step (Set-DesktopExeIdentity)
    ...
    $buildLog = "$env:TEMP\noru-desktop-build-$(Get-Random).log"
    ...
    $exeCandidates = @(
        "$desktopDir\release\win-unpacked\noru.exe",
        "$desktopDir\release\win-arm64-unpacked\noru.exe"
    )
    $found = $false
    $desktopExe = $null
    ...
    if (-not $found) {
        throw "Desktop build completed but no noru.exe was found under $desktopDir\release\*-unpacked\"
    }

    # 3b. The noru icon + identity are stamped onto noru.exe by the
    ...
    #    noru.exe. We deliberately do NOT point them at `noru desktop`: that
...
function New-DesktopShortcuts {
    ...
    try {
        ...
        $targets = @(
            (Join-Path ([Environment]::GetFolderPath('Programs')) 'noru.lnk'),
            (Join-Path ([Environment]::GetFolderPath('Desktop')) 'noru.lnk')
        )
        ...
        # with the noru icon, but without this the shortcut can keep drawing
...
function Install-PlatformSdks {
    ...
    # ~/.noru/.env are importable.  Two problems this solves:
    #
...
function Invoke-SetupWizard {
    ...
    if ($NonInteractive) {
        ...
        Write-Info "Skipping setup wizard (non-interactive). Configure via the GUI or 'noru setup'."
        return
    }
    ...
    # Run noru setup using the venv Python directly (no activation needed)
    if (-not $NoVenv) {
        & ".\venv\Scripts\python.exe" -m noru_cli.main setup
    } else {
        python -m noru_cli.main setup
    }
...
function Start-GatewayIfConfigured {
    ...
    $noruCmd = "$InstallDir\venv\Scripts\noru.exe"
    if (-not (Test-Path $noruCmd)) {
        $noruCmd = "noru"
    }

    # If WhatsApp is enabled but not yet paired, run foreground for QR scan
    $whatsappEnabled = $content | Where-Object { $_ -match "^WHATSAPP_ENABLED=true" }
    $whatsappSession = "$noruHome\whatsapp\session\creds.json"
    if ($whatsappEnabled -and -not (Test-Path $whatsappSession)) {
        Write-Host ""
        Write-Info "WhatsApp is enabled but not yet paired."
        Write-Info "Running 'noru whatsapp' to pair via QR code..."
        Write-Host ""
    ...
    if ($NonInteractive) {
        ...
        Write-Info "Start the gateway later with: noru gateway"
        return
    }
    ...
    if ($response -eq "" -or $response -match "^[Yy]") {
        ...
        try {
            $logFile = "$noruHome\logs\gateway.log"
            Start-Process -FilePath $noruCmd -ArgumentList "gateway" `
                -RedirectStandardOutput $logFile `
                -RedirectStandardError "$noruHome\logs\gateway-error.log" `
                -WindowStyle Hidden
            Write-Success "Gateway started! Your bot is now online."
            Write-Info "Logs: $logFile"
            Write-Info "To stop: close the gateway process from Task Manager"
        } catch {
            Write-Warn "Failed to start gateway. Run manually: noru gateway"
        }
    } else {
        Write-Info "Skipped. Start the gateway later with: noru gateway"
    }
}
...
function Write-Completion {
    ...
    Write-Host "$noruHome\config.yaml"
    Write-Host "   API Keys:  " -NoNewline -ForegroundColor Yellow
    Write-Host "$noruHome\.env"
    Write-Host "   Data:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$noruHome\cron\, sessions\, logs\"
    ...
    Write-Host ""
    Write-Host "   noru              " -NoNewline -ForegroundColor Green
    Write-Host "Start chatting"
    Write-Host "   noru setup        " -NoNewline -ForegroundColor Green
    Write-Host "Configure API keys & settings"
    Write-Host "   noru config       " -NoNewline -ForegroundColor Green
    Write-Host "View/edit configuration"
    Write-Host "   noru config edit  " -NoNewline -ForegroundColor Green
    Write-Host "Open config in editor"
    Write-Host "   noru gateway      " -NoNewline -ForegroundColor Green
    Write-Host "Start messaging gateway (Telegram, Discord, etc.)"
    Write-Host "   noru update       " -NoNewline -ForegroundColor Green
...
$InstallStages = @(
    ...
    @{ Name = "repository";       Title = "Cloning noru repository";            Category = "install";      NeedsUserInput = $false; Worker = "Stage-Repository" }
...
if ($IncludeDesktop) {
    ...
    # (noru-Setup.exe), never via the irm|iex CLI one-liner.
    $InstallStages += @{ Name = "desktop"; Title = "Building desktop app"; Category = "install"; NeedsUserInput = $false; Worker = "Stage-Desktop" }
}
$InstallStages += @(
    @{ Name = "path";             Title = "Adding noru to PATH";                Category = "finalize";     NeedsUserInput = $false; Worker = "Stage-Path" }
...
try {
    ...
    Write-Host "  Invoke-WebRequest -Uri 'https://raw.githubusercontent.com/syahdannoor/noru-agent/main/scripts/install.ps1' -OutFile install.ps1" -ForegroundColor Yellow
