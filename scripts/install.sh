...
# Configuration
REPO_URL_SSH="git@github.com:syahdannoor/noru-agent.git"
REPO_URL_HTTPS="https://github.com/syahdannoor/noru-agent.git"
NORU_HOME="${NORU_HOME:-$HOME/.noru}"
# INSTALL_DIR is resolved AFTER arg parsing and OS detection so we can pick an
# FHS-style layout for root installs.  Track whether the user gave us an
# explicit directory — if so we never override it.
if [ -n "${NORU_INSTALL_DIR:-}" ]; then
    INSTALL_DIR="$NORU_INSTALL_DIR"
...
#   data still at /root/.hermes (NORU_HOME).  Matches Claude Code / Codex CLI
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            ...
            echo "                   write \$NORU_HOME/.no-bundled-skills so future"
            echo "                   'noru update' runs never inject bundled skills either"
            ...
            echo "  --noru-home PATH  Data directory (default: ~/.hermes, or \$NORU_HOME)"
            ...
            echo "  Existing installs at \$NORU_HOME/hermes-agent are preserved in-place."
            echo "  --ensure DEPS  Install only specified deps (comma-separated)"
            echo "                   Supported: node, browser, ripgrep, ffmpeg"
            echo "                   Does NOT clone repo or create venv"
            echo "  --postinstall  Run post-install setup only (for pip users)"
            echo "                   Installs optional deps + runs noru setup"
            echo "                   Does NOT clone repo or create venv"
            exit 0
            ;;
        *)
...
# intentional, but they leave the checkout dirty — which forces `noru update`
...
emit_manifest() {
    ...
    printf '%s' '{"protocol_version":1,"stages":[{"name":"prerequisites","title":"System prerequisites","category":"runtime","needs_user_input":false},{"name":"repository","title":"Download Noru Agent","category":"runtime","needs_user_input":false},{"name":"venv","title":"Create Python virtual environment","category":"runtime","needs_user_input":false},{"name":"python-deps","title":"Install Python dependencies","category":"runtime","needs_user_input":false},{"name":"node-deps","title":"Install browser-tool dependencies","category":"runtime","needs_user_input":false},{"name":"path","title":"Install noru command","category":"runtime","needs_user_input":false},{"name":"config","title":"Prepare config and skills","category":"configuration","needs_user_input":false},{"name":"setup","title":"Configure API keys and settings","category":"configuration","needs_user_input":true},{"name":"gateway","title":"Configure gateway service","category":"configuration","needs_user_input":true},'"$desktop_stage"'{"name":"complete","title":"Finish install","category":"runtime","needs_user_input":false}]}'
    printf '\n'
}
...
#
# Defaults:
#   - Non-root, any OS:       INSTALL_DIR = $NORU_HOME/hermes-agent
#                             command link in $HOME/.local/bin
#   - Termux (any uid):       INSTALL_DIR = $NORU_HOME/hermes-agent
...
#                              $NORU_HOME/hermes-agent — then preserve it)
#
# Always no-op when the user set --dir or $NORU_INSTALL_DIR.
resolve_install_layout() {
    ...
    # Termux: package manager manages /data/data/..., keep code in NORU_HOME.
    if is_termux; then
        INSTALL_DIR="$NORU_HOME/hermes-agent"
        return 0
    fi
    ...
    if [ "$OS" = "linux" ] && [ "$(id -u)" -eq 0 ]; then
        if [ -d "$NORU_HOME/hermes-agent/.git" ]; then
            INSTALL_DIR="$NORU_HOME/hermes-agent"
        ...
        log_info "  Data:    $NORU_HOME (unchanged)"
...
detect_os() {
    case "$(uname -s)" in
        ...
        CYGWIN*|MINGW*|MSYS*)
            ...
            log_info "  iex (irm https://raw.githubusercontent.com/syahdannoor/noru-agent/main/scripts/install.ps1)"
            exit 1
            ;;
        *)
            OS="unknown"
...
install_uv() {
    ...
    # Hermes owns its own uv at $NORU_HOME/bin/uv.  Always install there —
    # no PATH probing, no conda guards, no multi-location resolution chains.
    # The runtime update path (hermes_cli/managed_uv.py) looks in the same
    # place, so install.sh and `noru update` stay in sync.
    local _managed_uv="$NORU_HOME/bin/uv"
    ...
    log_info "Installing managed uv into $NORU_HOME/bin ..."
    mkdir -p "$NORU_HOME/bin"
    ...
    # directly into $NORU_HOME/bin instead of ~/.local/bin.
    if UV_UNMANAGED_INSTALL="$NORU_HOME/bin" sh "$_uv_installer" >>"$_uv_install_log" 2>&1; then
        rm -f "$_uv_installer"
        if [ -x "$_managed_uv" ]; then
            UV_CMD="$_managed_uv"
        else
...
check_node() {
    ...
    if [ -x "$NORU_HOME/node/bin/node" ]; then
        export PATH="$NORU_HOME/node/bin:$PATH"
        local found_ver=$("$NORU_HOME/node/bin/node" --version)
        log_success "Node.js $found_ver found (Noru-managed)"
        HAS_NODE=true
        return 0
    fi
...
install_node() {
    ...
    log_info "Extracting to ~/.noru/node/..."
    ...
    # Place into ~/.noru/node/ and symlink binaries to ~/.local/bin/
    rm -rf "$NORU_HOME/node"
    mkdir -p "$NORU_HOME"
    mv "$extracted_dir" "$NORU_HOME/node"
    rm -rf "$tmp_dir"

    mkdir -p "$HOME/.local/bin"
    ln -sf "$NORU_HOME/node/bin/node" "$HOME/.local/bin/node"
    ln -sf "$NORU_HOME/node/bin/npm"  "$HOME/.local/bin/npm"
    ln -sf "$NORU_HOME/node/bin/npx"  "$HOME/.local/bin/npx"

    export PATH="$NORU_HOME/node/bin:$PATH"

    local installed_ver
    installed_ver=$("$NORU_HOME/node/bin/node" --version 2>/dev/null)
    log_success "Node.js $installed_ver installed to ~/.noru/node/"
    HAS_NODE=true
}
...
check_network_prerequisites() {
    ...
    if [ "$DISTRO" = "termux" ]; then
        ...
        log_warn "Network checks failed. Noru install may complete, but web search and dependency downloads can fail."
    ...
    if [ -n "$pkg_install" ]; then
        ...
        # sudo needs password — ask once for everything
            ...
            if [ "$IS_INTERACTIVE" = true ]; then
                ...
                log_info "Noru Agent itself does not require or retain root access."
...
copy_config_templates() {
    ...
    mkdir -p "$NORU_HOME"/{cron,sessions,logs,pairing,hooks,image_cache,audio_cache,memories,skills}

    # Create .env at ~/.noru/.env (top level, easy to find)
    if [ ! -f "$NORU_HOME/.env" ]; then
        if [ -f "$INSTALL_DIR/.env.example" ]; then
            cp "$INSTALL_DIR/.env.example" "$NORU_HOME/.env"
            log_success "Created ~/.noru/.env from template"
        else
            touch "$NORU_HOME/.env"
            log_success "Created ~/.noru/.env"
        fi
    else
        log_info "~/.noru/.env already exists, keeping it"
    fi
    ...
    chmod 600 "$NORU_HOME/.env"
    configure_browser_env_from_system_browser

    # Create config.yaml at ~/.noru/config.yaml (top level, easy to find)
    if [ ! -f "$NORU_HOME/config.yaml" ]; then
        if [ -f "$INSTALL_DIR/cli-config.yaml.example" ]; then
            cp "$INSTALL_DIR/cli-config.yaml.example" "$NORU_HOME/config.yaml"
            log_success "Created ~/.noru/config.yaml from template"
        fi
    else
        log_info "~/.noru/config.yaml already exists, keeping it"
    fi
...
-->
SOUL_EOF
        log_success "Created ~/.noru/SOUL.md (edit to customize personality)"
    fi

    log_success "Configuration directory ready: ~/.noru/"
    ...
    if [ "$NO_SKILLS" = true ]; then
        ...
        # skills_sync.py and `noru update` both honor this marker, so the
        ...
        printf '%s\n' \
            ...
            "Delete this file to re-enable sync on the next 'noru update'." \
            > "$NORU_HOME/.no-bundled-skills" 2>/dev/null || true
        log_info "Skipping bundled skills (--no-skills). Wrote $NORU_HOME/.no-bundled-skills"
        log_info "  Future 'noru update' runs will not inject bundled skills. Delete the marker to opt back in."
    else
        log_info "Syncing bundled skills to ~/.noru/skills/ ..."
        if "$INSTALL_DIR/venv/bin/python" "$INSTALL_DIR/tools/skills_sync.py" 2>/dev/null; then
            log_success "Skills synced to ~/.noru/skills/"
        else
            # Fallback: simple directory copy if Python sync fails
            if [ -d "$INSTALL_DIR/skills" ] && [ ! "$(ls -A "$NORU_HOME/skills/" 2>/dev/null | grep -v '.bundled_manifest')" ]; then
                cp -r "$INSTALL_DIR/skills/"* "$NORU_HOME/skills/" 2>/dev/null || true
                log_success "Skills copied to ~/.noru/skills/"
            fi
        fi
    fi
}
...
install_node_deps() {
    ...
    # Keep the checkout clean so `noru update` doesn't autostash every run.
...
run_setup_wizard() {
    ...
    if ! (: </dev/tty) 2>/dev/null; then
        log_info "Setup wizard skipped (no terminal available). Run 'noru setup' after install."
        return 0
    fi
    ...
    if [ "$USE_VENV" = true ]; then
        "$INSTALL_DIR/venv/bin/python" -m noru_cli.main setup < /dev/tty
    else
        python -m noru_cli.main setup < /dev/tty
    fi
}

maybe_start_gateway() {
    # Check if any messaging platform tokens were configured
    ENV_FILE="$NORU_HOME/.env"
    if [ ! -f "$ENV_FILE" ]; then
        return 0
    fi
    ...
    WHATSAPP_SESSION="$NORU_HOME/whatsapp/session/creds.json"
    ...
    if [ "$should_install_gateway" = true ]; then
        ...
        if [ "$DISTRO" != "termux" ] && command -v systemctl &> /dev/null; then
            ...
            nohup $HERMES_CMD gateway > "$NORU_HOME/logs/gateway.log" 2>&1 &
            GATEWAY_PID=$!
            log_success "Gateway started (PID $GATEWAY_PID). Logs: ~/.noru/logs/gateway.log"
...
print_success() {
    ...
    echo ""
    echo -e "   ${YELLOW}Config:${NC}    $NORU_HOME/config.yaml"
    echo -e "   ${YELLOW}API Keys:${NC}  $NORU_HOME/.env"
    echo -e "   ${YELLOW}Data:${NC}      $NORU_HOME/cron/, sessions/, logs/"
    ...
    echo -e "   ${GREEN}noru setup${NC}        Configure API keys & settings"
    ...
    echo -e "   ${GREEN}noru update${NC}       Update to latest version"
    echo ""

    echo -e "${CYAN}─────────────────────────────────────────────────────────${NC}"
    echo ""
    if [ "$DISTRO" = "termux" ]; then
        echo -e "${YELLOW}⚡ 'noru' was linked into $(get_command_link_display_dir), which is already on PATH in Termux.${NC}"
        echo ""
    elif [ "$ROOT_FHS_LAYOUT" = true ]; then
        echo -e "${YELLOW}⚡ 'noru' was linked into /usr/local/bin and is ready to use — no shell reload needed.${NC}"
        echo ""
    else
        echo -e "${YELLOW}⚡ Reload your shell to use 'noru' command:${NC}"
        echo ""
        LOGIN_SHELL="$(basename "${SHELL:-/bin/bash}")"
...
ensure_browser() {
    if ! command -v node >/dev/null 2>&1; then
        local node_bin="$NORU_HOME/node/bin/node"
        if [ -x "$node_bin" ]; then
            export PATH="$NORU_HOME/node/bin:$PATH"
        else
    ...
    local npm_bin
    npm_bin="$(command -v npm 2>/dev/null || echo "$NORU_HOME/node/bin/npm")"
    if [ ! -x "$npm_bin" ]; then
    ...
    local log_file
    log_file="$(mktemp)"
    if ! "$npm_bin" install -g --prefix "$NORU_HOME/node" --silent --ignore-scripts \
    ...
    local ab_bin="$NORU_HOME/node/bin/agent-browser"
    if [ -x "$ab_bin" ]; then
...
postinstall_mode() {
    ...
    if [ -n "$HERMES_CMD" ]; then
        log_info "Running noru setup..."
        "$HERMES_CMD" setup
    else
        log_warn "hermes command not found on PATH"
        log_info "Try: python -m noru_cli.main setup"
    fi
}
...
install_desktop() {
    ...
    # Try the Noru-managed Node first (check_node adds $NORU_HOME/node/bin
    ...
    # checkout stays clean for the next `noru update`.
    restore_dirty_lockfiles "$INSTALL_DIR"
}
...
run_stage_body() {
    ...
    case "$stage" in
        ...
        desktop)
            ...
            resolve_install_layout
            require_install_dir
            # Each stage runs in its own process, so the Noru-managed Node
            # provisioned during prerequisites/node-deps (at $NORU_HOME/node/bin)
        ...
        complete)
            ...
            resolve_install_layout
            print_success
            echo "git" > "$NORU_HOME/.install_method"
            ;;
        *)
