...
#   * Proper venv activation (probes .venv, venv, then ~/.noru/...)
#
# Usage:
...
# ── Activate venv ───────────────────────────────────────────────────────────
VENV=""
for candidate in "$REPO_ROOT/.venv" "$REPO_ROOT/venv" "$HOME/.noru/hermes-agent/venv"; do
...
if [ -f "$HOME/.noru/pytest_live_guard.py" ]; then
  EXTRA_PYTHONPATH="$HOME/.noru"
