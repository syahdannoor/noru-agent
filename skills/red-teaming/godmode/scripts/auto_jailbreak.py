...
Usage in execute_code:
    exec(open(os.path.expanduser(
        os.path.join(os.environ.get("NORU_HOME", os.path.expanduser("~/.hermes")), "skills/red-teaming/godmode/scripts/auto_jailbreak.py")
    )).read())
...
try:
    ...
    _SKILL_DIR = Path(os.getenv("NORU_HOME", Path.home() / ".hermes")) / "skills" / "red-teaming" / "godmode"
...
NORU_HOME = Path(os.getenv("NORU_HOME", Path.home() / ".hermes"))
CONFIG_PATH = NORU_HOME / "config.yaml"
PREFILL_PATH = NORU_HOME / "prefill.json"
...
def _detect_model_family(model: str) -> str:
    ...
    if "grok" in model_lower or "x-ai" in model_lower:
        return "grok"
    if "noru" in model_lower or "nous" in model_lower:
        return "noru"
...
def _write_prefill(prefill_messages: list):
    """Write prefill messages to ~/.noru/prefill.json."""
