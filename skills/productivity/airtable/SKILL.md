...
4. Store the token in `~/.noru/.env` (or via `hermes setup`):
   ```
...
- **`AIRTABLE_API_KEY` flows from `~/.noru/.env` into the subprocess automatically** when this skill is loaded — no need to re-export it before each `curl` call.
