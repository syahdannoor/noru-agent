...
if command -v gh &>/dev/null && gh auth status &>/dev/null; then
  ...
  if [ -z "$GITHUB_TOKEN" ]; then
    if [ -f ~/.noru/.env ] && grep -q "^GITHUB_TOKEN=" ~/.noru/.env; then
      GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" ~/.noru/.env | head -1 | cut -d= -f2 | tr -d '\n\r')
...
```bash
source "${NORU_HOME:-$HOME/.noru}/skills/github/github-auth/scripts/gh-env.sh"
