...
```bash
mkdir -p ~/.noru/cache/darwinian-evolver && cd ~/.noru/cache/darwinian-evolver
...
```bash
cd ~/.noru/cache/darwinian-evolver/darwinian_evolver \
...
Open `~/.noru/cache/darwinian-evolver/darwinian_evolver/darwinian_evolver/lineage_visualizer.html`
...
SKILL_DIR=~/.noru/skills/research/darwinian-evolver
DE_DIR=~/.noru/cache/darwinian-evolver/darwinian_evolver
...
7. **AGPL.** Never `from darwinian_evolver import ...` inside Hermes core.
   Custom driver scripts under `~/.noru/skills/...` are user-side and fine.
