...
- save provider credentials into `~/.noru/.env`
...
```bash
SCRIPT="$(find ~/.noru/skills -path '*/telephony/scripts/telephony.py' -print -quit)"
```
...
3. Buy it and save it into `~/.noru/.env` + state:
```bash
python3 "$SCRIPT" twilio-buy "+17025551234" --save-env
```
...
3. persist that number to `~/.noru/.env`
