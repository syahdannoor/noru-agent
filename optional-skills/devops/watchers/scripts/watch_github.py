...
Usage (via cron with --no-agent):
    ...
    hermes cron create hermes-issues \\
      --schedule "*/5 * * * *" --no-agent \\
      --script "$NORU_HOME/skills/devops/watchers/scripts/watch_github.py" \\
      --script-args "--name hermes-issues --repo syahdannoor/noru-agent --scope issues"

Set GITHUB_TOKEN (or GH_TOKEN) in ~/.noru/.env to avoid the 60 req/hr
