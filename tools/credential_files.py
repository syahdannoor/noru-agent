...
def register_credential_file(
    ...
    *relative_path* is relative to ``NORU_HOME`` (e.g. ``google_token.json``).
    ...
    The resolved host path must remain inside NORU_HOME so that a malicious
    skill cannot declare ``required_credential_files: ['../../.ssh/id_rsa']``
    and exfiltrate sensitive host files into a container sandbox.
    """
    hermes_home = _resolve_hermes_home()

    # Reject absolute paths — they bypass the NORU_HOME sandbox entirely.
    if os.path.isabs(relative_path):
        logger.warning(
            "credential_files: rejected absolute path %r (must be relative to NORU_HOME)",
            relative_path,
        )
        return False
    ...
    # that traversal like ``../. ssh/id_rsa`` cannot escape NORU_HOME.
...
# Each tuple is (new_subpath, old_name) matching noru_constants.get_hermes_dir().
