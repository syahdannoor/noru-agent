...
def _fire_approval_hook(hook_name: str, **kwargs) -> None:
    ...
    """
    try:
        from noru_cli.plugins import invoke_hook
    except Exception:
...
# via shell expansions like $HOME or $NORU_HOME.
...
# ~/.noru/config.yaml IS the security policy: approvals.mode, yolo, and the
...
# theater. Mirrors _HERMES_ENV_PATH; matches the NORU_HOME override form as
# well as ~/.noru/.
_HERMES_CONFIG_PATH = (
    r'(?:~\/\.hermes/|'
    r'(?:\$home|\$\{home\})/\.hermes/|'
...
DANGEROUS_PATTERNS = [
    ...
    # In-place edit of a Hermes-managed security file (~/.noru/config.yaml or
