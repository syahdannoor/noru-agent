...
def has_xai_credentials() -> bool:
    ...
    2. ``~/.noru/auth.json`` has a non-empty ``providers.xai-oauth.tokens.access_token``
    ...
    try:
        ...
        auth_path = get_noru_home() / "auth.json"
...
def get_env_value(name: str, default=None):
    """Read ``name`` from ``~/.noru/.env`` first, then ``os.environ``.

    Wraps :func:`noru_cli.config.get_env_value` so tests can patch
    ``tools.xai_http.get_env_value`` to inject dotenv-only secrets into the
    xAI credential resolver.
    """
    try:
        from noru_cli.config import get_env_value as _hermes_get_env_value
...
def resolve_xai_http_credentials(*, force_refresh: bool = False) -> Dict[str, str]:
    ...
    to ``XAI_API_KEY`` resolved via ``noru_cli.config.get_env_value`` so keys
    stored in ``~/.noru/.env`` (the standard Hermes location) are honored —
