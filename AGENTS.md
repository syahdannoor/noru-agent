# Noru Agent - Development Guide

Instructions for AI coding assistants and developers working on the noru-agent codebase.
...
`$HOME/.noru/noru-agent/venv` (for worktrees that share a venv with the
main checkout).
...
├── hermes_constants.py   # get_noru_home(), display_noru_home() — profile-aware paths
...
**User config:** `~/.noru/config.yaml` (settings), `~/.noru/.env` (API keys only).
**Logs:** `~/.noru/logs/` — `agent.log` (INFO+), `errors.log` (WARNING+),
`gateway.log` when running the gateway. Profile-aware via `get_noru_home()`.
...
- Skill slash commands: `agent/skill_commands.py` scans `~/.noru/skills/`, injects as **user message** (not system prompt) to preserve prompt caching
...
route instead: create `~/.noru/plugins/<name>/plugin.yaml` and
`~/.noru/plugins/<name>/__init__.py`, then register tools with
...
**Path references in tool schemas**: If the schema description mentions file paths (e.g. default output directories), use `display_noru_home()` to make them profile-aware. The schema is generated at import time, which is after `_apply_profile_override()` sets `NORU_HOME`.

**State files**: If a tool stores persistent state (caches, logs, checkpoints), use `get_noru_home()` for the base directory — never `Path.home() / ".hermes"`. This ensures each profile gets its own state.
...
~/.noru/skins/*.yaml       # User-installed custom skins (drop-in)
```
...
Users create `~/.noru/skins/<name>.yaml`:
...
`~/.noru/plugins/` and pip-installed entry points.

### General plugins (`hermes_cli/plugins.py` + `plugins/<name>/`)

`PluginManager` discovers plugins from `~/.noru/plugins/`, `./.hermes/plugins/`,
...
into `~/.noru/plugins/` (or via pip entry points) — they implement
...
2. User: `$NORU_HOME/plugins/model-providers/<name>/`
...
4. **`author` credits the human contributor first.** For external
   ...
   first; "Noru Agent" is the secondary collaborator. If the
   contributor's commit shows "Noru Agent" as author (because they
...
lives in the `noru-agent-dev` skill at
...
go to `~/.noru/skills/.archive/` and are restorable.
...
- **Telemetry:** `tools/skill_usage.py` owns the sidecar
  `~/.noru/skills/.usage.json` — per-skill `use_count`, `view_count`,
...
- File lock at `~/.noru/cron/.tick.lock` prevents duplicate ticks
  across processes.
...
`NORU_HOME` directory (config, API keys, memory, sessions, skills, gateway, etc.).

The core mechanism: `_apply_profile_override()` in `hermes_cli/main.py` sets
`NORU_HOME` before any module imports. All `get_noru_home()` references
automatically scope to the active profile.

### Rules for profile-safe code

1. **Use `get_noru_home()` for all NORU_HOME paths.** Import from `hermes_constants`.
   NEVER hardcode `~/.hermes` or `Path.home() / ".hermes"` in code that reads/writes state.
   ```python
   # GOOD
   from hermes_constants import get_noru_home
   config_path = get_noru_home() / "config.yaml"

   # BAD — breaks profiles
   config_path = Path.home() / ".hermes" / "config.yaml"
   ```

2. **Use `display_noru_home()` for user-facing messages.** Import from `hermes_constants`.
   This returns `~/.hermes` for default or `~/.noru/profiles/<name>` for profiles.
   ```python
   # GOOD
   from hermes_constants import display_noru_home
   print(f"Config saved to {display_noru_home()}/config.yaml")

   # BAD — shows wrong path for profiles
   print("Config saved to ~/.noru/config.yaml")
   ```

3. **Module-level constants are fine** — they cache `get_noru_home()` at import time,
   which is AFTER `_apply_profile_override()` sets the env var. Just use `get_noru_home()`,
   not `Path.home() / ".hermes"`.

4. **Tests that mock `Path.home()` must also set `NORU_HOME`** — since code now uses
   `get_noru_home()` (reads env var), not `Path.home() / ".hermes"`:
   ```python
   with patch.object(Path, "home", return_value=tmp_path), \
        patch.dict(os.environ, {"NORU_HOME": str(tmp_path / ".hermes")}):
       ...
   ```
...
6. **Profile operations are HOME-anchored, not NORU_HOME-anchored** — `_get_profiles_root()`
   returns `Path.home() / ".hermes" / "profiles"`, NOT `get_noru_home() / "profiles"`.
...
Use `get_noru_home()` from `hermes_constants` for code paths. Use `display_noru_home()`
for user-facing print/log messages. Hardcoding `~/.hermes` breaks profiles — each profile
has its own `NORU_HOME` directory. This was the source of 5 bugs fixed in PR #3575.
...
with actual imports (not mocks) against a temp `NORU_HOME`.

### Tests must not write to `~/.noru/`
The `_isolate_hermes_home` autouse fixture in `tests/conftest.py` redirects `NORU_HOME` to a temp dir. Never hardcode `~/.noru/` paths in tests.
...
| HOME / `~/.noru/` | Your real config+auth.json | Temp dir per test |
