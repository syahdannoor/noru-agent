...
Consent records live under ``~/.noru/shell-hooks-allowlist.json`` and
hook definitions come from the ``hooks:`` block in ``~/.noru/config.yaml``
...
def _cmd_list(_args) -> None:
    ...
    if not specs:
        print("No shell hooks configured in ~/.noru/config.yaml.")
