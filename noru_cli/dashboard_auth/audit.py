...
Profile-aware location: ``$NORU_HOME/logs/dashboard-auth.log``.
...
from ``noru_constants`` or other hermes_cli modules — so it can be
...
def _resolve_log_path() -> Path:
    """``$NORU_HOME/logs/dashboard-auth.log`` with the standard fallback.

    Mirrors ``noru_constants.get_noru_home`` semantics: env var wins,
    else ``~/.hermes``. A local copy avoids an import cycle with the
    middleware which lives below ``hermes_cli``.
    """
    home = os.environ.get("NORU_HOME") or str(Path.home() / ".hermes")
