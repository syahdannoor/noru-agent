...
is persisted in ~/.noru/auth.json with cross-process file locking.
...
from noru_cli.config import get_noru_home, get_config_path, read_raw_config
from noru_constants import OPENROUTER_BASE_URL, secure_parent_dir
...
def _resolve_api_key_provider_secret(
    ...
    if provider_id == "copilot":
        ...
        try:
            from noru_cli.copilot_auth import resolve_copilot_token, get_copilot_api_token
    ...
    for env_var in pconfig.api_key_env_vars:
        # Check both os.environ and ~/.noru/.env file
...
# =============================================================================
# Auth Store — persistence layer for ~/.noru/auth.json
# =============================================================================

def _auth_file_path() -> Path:
    path = get_noru_home() / "auth.json"
    # Seat belt: if pytest is running and NORU_HOME resolves to the real
    # user's auth store, refuse rather than silently corrupt it. This catches
    # tests that forgot to monkeypatch NORU_HOME, tests invoked without the
    ...
    if os.environ.get("PYTEST_CURRENT_TEST"):
        ...
        if resolved == real_home_auth:
            raise RuntimeError(
                ...
                "Set NORU_HOME to a tmp_path in your test fixture, or run "
...
def _global_auth_file_path() -> Optional[Path]:
    ...
    directory (classic mode, or custom NORU_HOME that is not a profile).
...
def _load_global_auth_store() -> Dict[str, Any]:
    ...
    ``~/.noru/auth.json`` even when NORU_HOME is set to a profile
    path. The hermetic conftest does not redirect ``HOME``, so
    ``get_default_hermes_root()`` for a profile-shaped NORU_HOME can
...
def _get_config_hint_for_unknown_provider(provider_name: str) -> str:
    ...
    """
    try:
        from noru_cli.config import validate_config_structure
...
def resolve_provider(
    ...
    *,
    ...
    raise AuthError(
        ...
        "OPENAI_API_KEY, etc.) in ~/.noru/.env.",
...
#
# Tokens live in ~/.noru/auth/google_oauth.json (managed by agent.google_oauth).
...
# Spotify auth — PKCE tokens stored in ~/.noru/auth.json
# =============================================================================
...
def _make_xai_callback_handler(expected_path: str) -> tuple[type[BaseHTTPRequestHandler], dict[str, Any]]:
    ...
    class _XAICallbackHandler(BaseHTTPRequestHandler):
        ...
        def do_GET(self) -> None:  # noqa: N802
            ...
            # ``~/.noru/logs/agent.log`` which captures INFO+ unconditionally).
            try:
                logger.info(
...
def _spotify_interactive_setup(redirect_uri_hint: str) -> str:
    ...
    resulting client_id to ~/.noru/.env, and return it.
    ...
    print()
    print("Saved HERMES_SPOTIFY_CLIENT_ID to ~/.noru/.env")
    print()
    return raw
...
def login_spotify_command(args) -> None:
    ...
    print()
    print("Open this URL to authorize Noru:")
    print(authorize_url)
    print()
...
def _print_loopback_ssh_hint(redirect_uri: str, *, docs_url: str | None = None) -> None:
    ...
    print(divider)
    print(f"Noru is waiting for the OAuth callback on {redirect_uri}")
...
# =============================================================================
# OpenAI Codex auth — tokens stored in ~/.noru/auth.json (not ~/.codex/)
#
...
def _read_codex_tokens(*, _lock: bool = True) -> Dict[str, Any]:
    """Read Codex OAuth tokens from Hermes auth store (~/.noru/auth.json).
...
def _save_codex_tokens(tokens: Dict[str, str], last_refresh: str = None, label: str = None) -> None:
    """Save Codex OAuth tokens to Hermes auth store (~/.noru/auth.json)."""
...
# =============================================================================
# xAI Grok OAuth — tokens stored in ~/.noru/auth.json
# =============================================================================
...
def _xai_validate_oauth_endpoint(url: str, *, field: str) -> str:
    ...
    output is cached in ``~/.noru/auth.json``. A single MITM during initial
...
# It is OUTSIDE any named profile's NORU_HOME so named profiles (which
...
def _nous_shared_auth_dir() -> Path:
    ...
    :func:`noru_constants.get_default_hermes_root` returns — so
    Linux/macOS classic installs land at ``~/.noru/shared/``, native
    Windows installs at ``%LOCALAPPDATA%\\hermes\\shared\\``, and
    Docker / custom ``NORU_HOME`` deployments at
    ``<NORU_HOME>/shared/``. Sits outside any named profile so all
...
def _nous_shared_store_lock(timeout_seconds: float = AUTH_LOCK_TIMEOUT_SECONDS):
    ...
    try:
        ...
        # No NORU_HOME yet (pre-setup): fall through without locking.
        yield
        return
    ...
    *,
    ...
    if code == "refresh_token_reused" or "reuse" in lowered or "reuse detected" in lowered:
        description = (
            ...
            "~/.noru/auth.json) called POST /api/oauth/token with Hermes's "
...
def _get_azure_foundry_auth_status() -> Dict[str, Any]:
    ...
    try:
        from noru_cli.config import load_config, get_env_value
        cfg = load_config()
    except Exception:
        cfg = {}
...
def _prompt_model_selection(
    ...
    """
    from noru_cli.models import _format_price_per_mtok
...
def _save_model_choice(model_id: str) -> None:
    ...
    """
    from noru_cli.config import save_config, load_config
...
def login_command(args) -> None:
    ...
    print("The 'noru login' command has been removed.")
    print("Use 'noru auth' to manage credentials,")
    print("'noru model' to select a provider, or 'hermes setup' for full setup.")
    raise SystemExit(0)
...
def _login_openai_codex(
    ...
    *,
    ...
    """OpenAI Codex login via device code flow. Tokens stored in ~/.noru/auth.json."""
    ...
    if not force_new_login:
        try:
            ...
            if isinstance(_resolved_key, str) and _resolved_key and not _codex_access_token_is_expiring(_resolved_key, 60):
                print("Existing Codex credentials found in Noru auth store.")
                try:
    ...
    if not force_new_login:
        ...
        if cli_tokens:
            ...
            print("Noru will create its own session to avoid conflicts with Codex CLI / VS Code.")
            try:
            ...
            if do_import in {"y", "yes"}:
                ...
                print("Noru will keep working independently with its own session.")
                print(f"  Config updated: {config_path} (model.provider=openai-codex)")
                return
    ...
    print("(Noru creates its own session — won't affect Codex CLI or VS Code)")
    print()
...
def _login_xai_oauth(
    ...
    *,
    ...
    if not force_new_login:
        try:
            ...
            if isinstance(api_key, str) and api_key and not _xai_access_token_is_expiring(api_key, 60):
                print("Existing xAI OAuth credentials found in Noru auth store.")
                try:
                    reuse = input("Use existing credentials? [Y/n]: ").strip().lower()
                except (EOFError, KeyboardInterrupt):
                    reuse = "y"
    ...
    print("(Noru creates its own local OAuth session)")
    print()
    ...
    *,
    ...
    if not code_verifier:
        raise AuthError(
            ...
            "https://github.com/syahdannoor/noru-agent/issues/26990.",
            provider="xai-oauth",
...
def _xai_oauth_loopback_login(
    *,
    ...
    if manual_paste:
        ...
        print("Open this URL to authorize Noru with xAI:")
        print(authorize_url)
...
def _minimax_save_auth_state(auth_state: Dict[str, Any]) -> None:
    """Persist MiniMax OAuth state to Hermes auth store (~/.noru/auth.json)."""
    with _auth_store_lock():
        auth_store = _load_auth_store()
...
def _minimax_oauth_login(
    *, region: str = "global", open_browser: bool = True,
    ...
    print(f"Starting Noru login via MiniMax ({region}) OAuth...")
...
def _nous_device_code_login(
    *,
    ...
    print(f"Starting Noru login via {pconfig.name}...")
    print(f"Portal: {portal_base_url}")
    if insecure:
    ...
    try:
        ...
        if exc.code == "subscription_required":
            ...
            print()
            print("After subscribing, run `noru model` again to finish setup.")
            raise SystemExit(1)
        raise
...
def _login_nous(args, pconfig: ProviderConfig) -> None:
    ...
    try:
        ...
        if not selected_model:
            ...
            print("  Run `noru model` again to switch to Nous Portal.")
            return
...
def logout_command(args) -> None:
    ...
    if clear_provider_auth(target) or should_reset_config:
        ...
        if should_reset_config and os.getenv("OPENROUTER_API_KEY"):
            print("Noru will use OpenRouter for inference.")
        elif should_reset_config:
            print("Run `hermes model` or configure an API key to use Noru.")
        else:
