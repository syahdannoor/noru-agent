...
the existing module-level functions in noru_cli.gateway and
noru_cli.gateway_windows directly. This protocol is a thin facade
...
# in ``noru_cli.gateway`` (systemd/launchd) and ``noru_cli.gateway_windows``
...
class SystemdServiceManager(_RegistrationUnsupportedMixin):
    """Thin wrapper around the ``systemd_*`` functions in noru_cli.gateway.
    ...
    def start(self, name: str) -> None:
        from noru_cli.gateway import systemd_start
        systemd_start()

    def stop(self, name: str) -> None:
        from noru_cli.gateway import systemd_stop
        systemd_stop()

    def restart(self, name: str) -> None:
        from noru_cli.gateway import systemd_restart
        systemd_restart()

    def is_running(self, name: str) -> bool:
        from noru_cli.gateway import _probe_systemd_service_running
        _, running = _probe_systemd_service_running()
        return running


class LaunchdServiceManager(_RegistrationUnsupportedMixin):
    """Thin wrapper around the ``launchd_*`` functions in noru_cli.gateway."""

    kind: ServiceManagerKind = "launchd"

    def start(self, name: str) -> None:
        from noru_cli.gateway import launchd_start
        launchd_start()

    def stop(self, name: str) -> None:
        from noru_cli.gateway import launchd_stop
        launchd_stop()

    def restart(self, name: str) -> None:
        from noru_cli.gateway import launchd_restart
        launchd_restart()

    def is_running(self, name: str) -> bool:
        from noru_cli.gateway import _probe_launchd_service_running
        return _probe_launchd_service_running()


class WindowsServiceManager(_RegistrationUnsupportedMixin):
    """Thin wrapper around ``noru_cli.gateway_windows`` (Scheduled Task /
    ...
    def is_running(self, name: str) -> bool:
        from noru_cli import gateway_windows
        from noru_cli.gateway import find_gateway_pids
...
class S6ServiceManager:
    ...
    def _render_run_script(
        ...
        The script:
          1. Sources NORU_HOME (and any extra env) via with-contenv —
             so e.g. ``-e NORU_HOME=/data/hermes`` is honored at run
        ...
        root NORU_HOME profile" (the implicit profile that exists at
        the top of $NORU_HOME, not under profiles/). It must be
        spelled this way because ``_profile_suffix()`` returns the
        empty string for the root profile, and the dispatcher in
        ``noru_cli.gateway`` maps that empty string to the
        ``gateway-default`` service slot. Passing ``-p default`` here
        would instead look up ``$NORU_HOME/profiles/default/`` — a
        ...
        ``noru_cli.profiles._allocate_gateway_port``). PR #30136
    ...
    def _render_log_run(profile: str) -> str:
        ...
        OQ8-C: persist to ``${NORU_HOME}/logs/gateways/<profile>/``.
        CRITICAL: the NORU_HOME path is sourced from the runtime env
        via with-contenv — NOT Python-substituted at registration time
        — so a container started with ``-e NORU_HOME=/data/hermes``
        ...
        return (
            ...
            f': "${{NORU_HOME:=/opt/data}}"\n'
            f'log_dir="$NORU_HOME/logs/gateways/{prof}"\n'
            f'mkdir -p "$log_dir"\n'
