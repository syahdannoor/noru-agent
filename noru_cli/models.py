...
from noru_cli import __version__ as _HERMES_VERSION
...
def _codex_curated_models() -> list[str]:
    ...
    """
    from noru_cli.codex_models import DEFAULT_CODEX_MODELS, _add_forward_compat_models
...
# $NORU_HOME/models_dev_cache.json as of 2026-04-28. Whenever xAI renames
...
def _xai_curated_models() -> list[str]:
    ...
    Reads $NORU_HOME/models_dev_cache.json directly (no network) so this
...
def fetch_openrouter_models(
    ...
    *,
    ...
    try:
        from noru_cli.model_catalog import get_curated_openrouter_models
...
def get_curated_nous_model_ids() -> list[str]:
    ...
    """
    try:
        from noru_cli.model_catalog import get_curated_nous_models
...
def list_available_providers() -> list[dict[str, str]]:
    ...
    for pid in provider_order:
        ...
        has_creds = False
        try:
            from noru_cli.auth import get_auth_status, has_usable_secret
            if pid == "custom":
...
def _resolve_static_model_alias(
    ...
    try:
        from noru_cli.model_switch import MODEL_ALIASES
    except Exception:
        return None
...
def normalize_provider(provider: Optional[str]) -> str:
    ...
    ``noru_cli.auth.resolve_provider()`` to resolve it to a concrete
...
def _resolve_copilot_catalog_api_key() -> str:
    ...
    Resolution order:
      ...
      2. ``read_credential_pool("copilot")`` — a token (typically a
         ...
         when the env var is set in ``~/.noru/.env``.
    ...
    """
    try:
        from noru_cli.auth import resolve_api_key_provider_credentials
    ...
    try:
        ...
        from noru_cli.copilot_auth import (
...
def provider_model_ids(provider: Optional[str], *, force_refresh: bool = False) -> list[str]:
    ...
    if normalized == "nous":
        ...
        try:
            from noru_cli.auth import fetch_nous_models, resolve_nous_runtime_credentials
            creds = resolve_nous_runtime_credentials()
            if creds:
...
#
# Cache strategy:
#   - One JSON file at $NORU_HOME/provider_models_cache.json
...
def _provider_models_cache_path() -> Path:
    ...
    return get_noru_home() / "provider_models_cache.json"
...
def _credential_fingerprint(provider: str) -> str:
    ...
    relevant tokens live in ``$NORU_HOME/auth.json`` and external
...
def _ollama_cloud_cache_path() -> Path:
    ...
    from noru_constants import get_noru_home
    return get_noru_home() / "ollama_cloud_models_cache.json"
