...
# NORU_HOME paths we've already pulled external secrets for during this
...
def reset_secret_source_cache() -> None:
    """Forget which NORU_HOME paths have already had external secrets applied.
...
def _sanitize_env_file_if_needed(path: Path) -> None:
    ...
    We delegate to ``noru_cli.config._sanitize_env_lines`` which
    ...
    if not path.exists():
        return
    try:
        from noru_cli.config import _sanitize_env_lines
    except ImportError:
...
def load_hermes_dotenv(
    *,
    ...
    Behavior:
    - `~/.noru/.env` overrides stale shell-exported values when present.
    ...
    home_path = Path(hermes_home or os.getenv("NORU_HOME", Path.home() / ".hermes"))
