...
- Keep data: Remove code but keep ~/.noru/ (configs, sessions, logs)
"""
        ...
        try:
            ...
            for line in content.split('\n'):
                ...
                if '# Hermes Agent' in line or '# hermes-agent' in line:
                    skip_next = True
                    continue
                if skip_next and ('noru' in line.lower() and 'PATH' in line):
                    skip_next = False
                    continue
                skip_next = False
                
                # Remove any PATH line containing hermes
                if 'noru' in line.lower() and ('PATH=' in line or 'path=' in line.lower()):
                    continue
    ...
    for wrapper in wrapper_paths:
        if wrapper.exists():
            try:
                ...
                if 'hermes_cli' in content or 'noru-agent' in content:
                    wrapper.unlink()
...
def remove_node_symlinks(hermes_home: Path) -> list:
    ...
    creates::
        ...
        ~/.local/bin/node -> $NORU_HOME/node/bin/node
        ~/.local/bin/npm  -> $NORU_HOME/node/bin/npm
        ~/.local/bin/npx  -> $NORU_HOME/node/bin/npx
...
def uninstall_gateway_service():
    ...
    try:
        from noru_cli.gateway import kill_gateway_processes, find_gateway_pids
    ...
    # 3. macOS: uninstall launchd plist
        ...
        try:
            from noru_cli.gateway import get_launchd_plist_path
...
#
#   1. Sets User-scope env vars ``NORU_HOME`` and ``HERMES_GIT_BASH_PATH``
...
def _hermes_path_markers(hermes_home: Path) -> list[str]:
    ...
    # Also match if NORU_HOME was customised to somewhere else — find-and-nuke
    # any entry whose path component contains "noru".  We don't want to catch
...
def remove_hermes_env_vars_windows() -> list[str]:
    """Delete NORU_HOME and HERMES_GIT_BASH_PATH from User-scope env vars."""
    try:
        import winreg
    except ImportError:
        return []
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, "Environment", 0,
                            winreg.KEY_READ | winreg.KEY_WRITE) as key:
            for name in ("NORU_HOME", "HERMES_GIT_BASH_PATH"):
                try:
...
def _uninstall_profile(profile) -> None:
    ...
    remove its alias wrapper, and wipe its NORU_HOME directory.
    ...
    current NORU_HOME and can't be easily switched in-process.
    """
    import sys as _sys
    name = profile.name
    ...
    #    Use `python -m noru_cli.main` so we don't depend on a `hermes`
    #    wrapper that may be half-removed mid-uninstall.
    hermes_invocation = [_sys.executable, "-m", "noru_cli.main", "--profile", name]
    ...
    # 3. Wipe the profile's NORU_HOME directory.
    try:
...
def run_uninstall(args):
    ...
    Options:
    - Full uninstall: removes code + ~/.noru/ (configs, data, logs)
    - Keep data: removes code but keeps ~/.noru/ for future reinstall
    """
    project_root = get_project_root()
    hermes_home = get_noru_home()

    # Detect named profiles when uninstalling from the default root —
    # offer to clean them up too instead of leaving zombie NORU_HOMEs
    ...
    # their alias wrappers, and wiping their NORU_HOME dirs. Otherwise
    ...
    if _is_windows():
        ...
        log_info("Removing NORU_HOME / HERMES_GIT_BASH_PATH User env vars...")
    ...
    try:
        if project_root.exists():
            # If the install is inside ~/.noru/, just remove the hermes-agent subdir
    ...
    #     under NORU_HOME but they're install tooling, not config — safe to
    ...
    # 5. Optionally remove ~/.noru/ data directory (and named profiles)
    if full_uninstall:
        # 5a. Stop and remove each named profile's gateway service and
        #     alias wrapper. The profile NORU_HOME dirs live under
    ...
    if not full_uninstall:
        ...
        if _is_windows():
            print(color("  iex (irm https://raw.githubusercontent.com/syahdannoor/noru-agent/main/scripts/install.ps1)", Colors.DIM))
        else:
            print(color("  curl -fsSL https://raw.githubusercontent.com/syahdannoor/noru-agent/main/scripts/install.sh | bash", Colors.DIM))
        print()
