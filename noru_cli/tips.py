...
TIPS = [
    ...
    "Checkpoints use shadow repos in ~/.noru/checkpoints/ — your project's .git is never touched.",
    ...
    "Cron scripts live in ~/.noru/scripts/ and run before the agent — perfect for data collection pipelines.",
    ...
    "The skills audit log at ~/.noru/skills/.hub/audit.log tracks every install and removal operation.",
    "Stale git worktrees are auto-cleaned: 24-72h old with no unpushed commits get pruned on startup.",
    "Each profile gets its own subprocess HOME at NORU_HOME/home/ — isolated git, ssh, npm, gh configs.",
    "NORU_HOME_MODE env var (octal, e.g. 0701) sets custom directory permissions for web server traversal.",
    "Container mode: place .container-mode in NORU_HOME and the host CLI auto-execs into the container.",
    "Ctrl+C has 5 priority tiers: cancel recording → cancel prompts → cancel picker → interrupt agent → exit.",
    "Every interrupt during an agent run is logged to ~/.noru/interrupt_debug.log with timestamps.",
    ...
    "MCP ${ENV_VAR} placeholders in config are resolved at server spawn — including vars from ~/.noru/.env.",
    "Skills from trusted repos (NousResearch) get a 'trusted' security level; community skills get extra scanning.",
    "The skills quarantine at ~/.noru/skills/.hub/quarantine/ holds skills pending security review.",
    ...
    '/reload reloads ~/.noru/.env into the running session — pick up new API keys without restarting.',
    ...
    # --- Gateway Hooks ---
    'Gateway hooks live under ~/.noru/hooks/<name>/ with HOOK.yaml + handler.py — handler must be named `handle`.',
    'Hook events include gateway:startup, session:start, agent:step, and command:* wildcard subscriptions.',
    'Drop a ~/.noru/BOOT.md checklist and a gateway:startup hook runs it as a one-shot agent every boot.',
    ...
    'Drop a YAML in ~/.noru/dashboard-themes/ with two palette colors to reskin the entire dashboard.',
    'Dashboard plugins are drop-in: manifest.json + JS bundle in ~/.noru/dashboard-plugins/ — no npm build required.',
    ...
    # --- TTS ---
    'tts.provider: piper runs 44-language local TTS on CPU — voices auto-download to ~/.noru/cache/piper-voices/.',
    ...
    '/reload-skills re-scans ~/.noru/skills/ so drop-in skills appear without restarting the session.',
    ...
    'Dashboard plugins are served from /dashboard-plugins/<name>/ — drop files into ~/.noru/dashboard-plugins/.',
]
