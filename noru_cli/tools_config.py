...
Saves per-platform tool configuration to ~/.noru/config.yaml under
...
from noru_cli.nous_subscription import (
    apply_nous_managed_defaults,
    get_nous_subscription_features,
)
from noru_cli.nous_account import format_nous_portal_entitlement_message
...
from noru_cli.cli_output import (  # noqa: E402 — late import block
...
def _xai_credentials_present() -> bool:
    ...
    """
    try:
        from noru_cli.auth import _read_xai_oauth_tokens
...
def _get_effective_configurable_toolsets():
    ...
    try:
        from noru_cli.plugins import discover_plugins, get_plugin_toolsets
...
from noru_cli.platforms import PLATFORMS as _PLATFORMS_REGISTRY
    ...
    if post_setup_key in {"agent_browser", "browserbase"}:
        ...
        if not node_modules.exists() and npm_bin:
            ...
            if result.returncode == 0:
                ...
                _print_warning(f"    npm install failed - run manually: cd {display_noru_home()}/hermes-agent && npm install")
                if result.stderr:
        ...
        _print_info("    Switch voices by setting tts.piper.voice in ~/.noru/config.yaml")
        ...
        # to ~/.noru/.env), then continues straight into PKCE. If they
        # already have an app, it skips the wizard and just does OAuth.
        from types import SimpleNamespace
        try:
            from noru_cli.auth import login_spotify_command
        except Exception as exc:
        ...
        try:
            from noru_cli.plugins_cmd import _get_enabled_set, _save_enabled_set
...
def tools_command(args=None, first_install: bool = False, config: dict = None):
    ...
    print()
    from noru_constants import display_noru_home
    print(color(f"  Tool configuration saved to {display_noru_home()}/config.yaml", Colors.DIM))
    print(color("  Changes take effect on next 'noru' or gateway restart.", Colors.DIM))
    print()
