...
from noru_constants import get_default_hermes_root, get_noru_home, display_noru_home
...
_EXCLUDED_DIRS = {
    "noru-agent",     # the codebase repo — re-clone instead
...
def _should_skip_backup_file(abs_path: Path, rel_path: Path, out_path: Path) -> bool:
    ...
    # write can copy data from outside NORU_HOME.
...
def run_backup(args) -> None:
    ...
    # Collect files
    print(f"Scanning {display_noru_home()} ...")
...
def run_import(args) -> None:
    ...
    with zipfile.ZipFile(zip_path, "r") as zf:
        ...
        print(f"Target: {display_noru_home()}")
        ...
        # Guidance
        print()
        if not (hermes_root / "noru-agent").is_dir():
...
# Critical state files to include in quick snapshots (relative to NORU_HOME).
...
def _quick_snapshot_root(hermes_home: Optional[Path] = None) -> Path:
    home = hermes_home or get_noru_home()
...
# Relative path of the cron job database inside NORU_HOME. Kept in sync with
...
def run_quick_backup(args) -> None:
    ...
    if snap_id:
        ...
        print(f"  {len(snaps)} snapshot(s) stored in {display_noru_home()}/state-snapshots/")
...
def create_pre_update_backup(
    ...
    """Create a full zip backup of NORU_HOME under ``backups/``.

    Mirrors :func:`run_backup` (same exclusion rules, same SQLite safe-copy)
    but writes to ``<NORU_HOME>/backups/pre-update-<timestamp>.zip`` and
...
def create_pre_migration_backup(
    ...
    """Create a full zip backup of NORU_HOME under ``backups/`` before a
