...
``$NORU_HOME/profiles/<name>/`` live on the persistent VOLUME, and
...
(the stage2 hook) has chowned the volume and seeded $NORU_HOME, but
...
def reconcile_profile_gateways(
    *,
    ...
    (the implicit profile that lives at the top of ``$NORU_HOME``,
    not under ``profiles/``). The dispatcher in ``noru_cli.gateway``
    ...
    ``$NORU_HOME/gateway_state.json`` (sibling to the profile root,
    not under ``profiles/``); stale runtime files there are swept the
    same way as for named profiles.

    Args:
        hermes_home: The container's NORU_HOME (typically /opt/data).
...
def _is_legacy_gateway_run_request(argv: Sequence[str]) -> bool:
    ...
    if args and args[0].endswith("main-wrapper.sh"):
        args = args[1:]
    if args and Path(args[0]).name == "noru":
        args = args[1:]
...
def _write_reconcile_log(
    ...
    """Append one line per profile to $NORU_HOME/logs/container-boot.log.
...
def main() -> int:
    ...
    hermes_home = Path(os.environ.get("NORU_HOME", "/opt/data"))
