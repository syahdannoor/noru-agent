...
``~/.noru/logs/`` (profile-aware via ``get_noru_home()``).
...
Component separation:
    ...
    gui.log receives dashboard-side records from ``noru_cli.web_server``,
    ``noru_cli.pty_bridge``, ``tui_gateway.*``, and ``uvicorn.*``.
...
from noru_constants import get_config_path, get_noru_home
...
class _ManagedRotatingFileHandler(RotatingFileHandler):
    ...
    def __init__(self, *args, **kwargs):
        from noru_cli.config import is_managed
