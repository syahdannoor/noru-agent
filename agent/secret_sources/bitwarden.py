...
so they don't have to live in plaintext in ``~/.noru/.env``.
...
* The access token is stored in ``~/.noru/.env`` as
...
# never the access token. It's plaintext-equivalent to ~/.noru/.env (which
...
def _disk_cache_path(home_path: Optional[Path] = None) -> Path:
    ...
    to `$NORU_HOME` / `~/.hermes` keeps direct callers working too.
    """
    if home_path is None:
        home_path = Path(os.getenv("NORU_HOME", Path.home() / ".hermes"))
...
def _http_download(url: str, dest: Path) -> None:
    req = urllib.request.Request(url, headers={"User-Agent": "noru-agent"})
    try:
...
# ---------------------------------------------------------------------------
# Public entry point — called from noru_cli.env_loader
# ---------------------------------------------------------------------------
