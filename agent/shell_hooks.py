...
* Python plugins and shell hooks compose naturally: both flow through
  :func:`noru_cli.plugins.invoke_hook` and its aggregators.  Python
...
* First-use consent is gated by the allowlist under
  ``~/.noru/shell-hooks-allowlist.json``.  Non-TTY callers must pass
...
def register_from_config(
    ...
    *,
    ...
    ``cfg`` is the full parsed config dict (``noru_cli.config.load_config``
...
def _parse_response(event: str, stdout: str) -> Optional[Dict[str, Any]]:
    ...
    :func:`noru_cli.plugins.get_pre_tool_call_block_message`.  This is
...
def allowlist_path() -> Path:
    ...
    return get_noru_home() / ALLOWLIST_FILENAME
...
def run_once(
    ...
    ``kwargs`` is the same dict that :func:`noru_cli.plugins.invoke_hook`
