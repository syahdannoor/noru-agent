...
**We are no longer accepting new memory providers into this repo.** The set of built-in providers under `plugins/memory/` (honcho, mem0, supermemory, byterover, hindsight, holographic, openviking, retaindb) is closed. If you want to add a new memory backend, publish it as a **standalone plugin repo** that users install into `~/.noru/plugins/` (or via a pip entry point).
...
```bash
git clone --recurse-submodules https://github.com/syahdannoor/noru-agent.git
cd hermes-agent
...
```bash
mkdir -p ~/.noru/{cron,sessions,logs,memories,skills}
cp cli-config.yaml.example ~/.noru/config.yaml
touch ~/.noru/.env

# Add at minimum an LLM provider key:
echo "OPENROUTER_API_KEY=***" >> ~/.noru/.env
```
...
│
├── skills/                   # Bundled skills (copied to ~/.noru/skills/ on install)
├── optional-skills/          # Official optional skills (discoverable via hub, not activated by default)
├── tests/                    # Test suite
├── website/                  # Documentation site (hermes-agent.nousresearch.com)
│
├── cli-config.yaml.example   # Example configuration (copied to ~/.noru/config.yaml)
└── AGENTS.md                 # Development guide for AI coding assistants
```

### User configuration (stored in `~/.noru/`)

| Path | Purpose |
|------|---------|
| `~/.noru/config.yaml` | Settings (model, terminal, toolsets, compression, etc.) |
| `~/.noru/.env` | API keys and secrets |
| `~/.noru/auth.json` | OAuth credentials (Nous Portal) |
| `~/.noru/skills/` | All active skills (bundled + hub-installed + agent-created) |
| `~/.noru/memories/` | Persistent memory (MEMORY.md, USER.md) |
| `~/.noru/state.db` | SQLite session database |
| `~/.noru/sessions/` | Gateway routing index (`sessions.json`), request-dump breadcrumbs, gateway `*.jsonl` transcripts, and (optionally) per-session JSON snapshots when `sessions.write_json_snapshots: true` is set. The per-session snapshots are off by default; state.db is canonical. |
| `~/.noru/cron/` | Scheduled job data |
| `~/.noru/whatsapp/session/` | WhatsApp bridge credentials |
...
- **Session persistence**: All conversations are stored in SQLite (`hermes_state.py`) with full-text search and unique session titles. Per-session JSON snapshots in `~/.noru/sessions/` were superseded by the SQLite store and are off by default; opt back in with `sessions.write_json_snapshots: true` if you have external tooling that consumes the JSON files directly.
...
Gateway and messaging sessions never collect secrets in-band; they instruct the user to run `hermes setup` or update `~/.noru/.env` locally.
...
- Use [GitHub Issues](https://github.com/syahdannoor/noru-agent/issues)
