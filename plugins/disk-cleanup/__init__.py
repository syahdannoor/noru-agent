...
1. ``post_tool_call`` hook — inspects ``write_file`` and ``terminal``
   ...
   under ``NORU_HOME`` and tracks them silently.  Zero agent
   compliance required.
...
2. ``on_session_end`` hook — when any test files were auto-tracked
   ...
   logs a single line to ``$NORU_HOME/disk-cleanup/cleanup.log``.
...
All operations are scoped to NORU_HOME and /tmp/hermes-*.
...
def _handle_slash(raw_args: str) -> Optional[str]:
    ...
    if sub == "track":
        ...
        return (
            f"Not tracked (already present, missing, or outside NORU_HOME): "
            f"{path_arg}"
        )
