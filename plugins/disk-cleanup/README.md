...
Scoped strictly to `$NORU_HOME` and `/tmp/hermes-*`.
...
| Hook | Behaviour |
|---|---|
| `post_tool_call` | When `write_file` / `terminal` / `patch` creates a file matching `test_*`, `tmp_*`, or `*.test.*` inside `NORU_HOME`, track it silently as `test` / `temp` / `cron-output`. |
...
| empty dirs under NORU_HOME | always | Never |
...
- `is_safe_path()` rejects anything outside `NORU_HOME` or `/tmp/hermes-*`
- Windows mounts (`/mnt/c` etc.) are rejected
- The state directory `$NORU_HOME/disk-cleanup/` is itself excluded
- `$NORU_HOME/logs/`, `memories/`, `sessions/`, `skills/`, `plugins/`,
