...
def _read_dm_role_auth_guild() -> Optional[int]:
    ...
    policy, ``~/.noru/.env`` is for secrets only, and this is a
...
class DiscordAdapter(BasePlatformAdapter):
    ...
    def _command_sync_state_path(self) -> _Path:
        ...
        directory = get_noru_home() / _DISCORD_COMMAND_SYNC_STATE_SUBDIR
        try:
    ...
    def _register_slash_commands(self) -> None:
        ...
        @tree.command(name="reload-skills", description="Re-scan ~/.noru/skills/ for new or removed skills")
        ...
        try:
            from noru_cli.commands import COMMAND_REGISTRY, _is_gateway_available, _resolve_config_gates
        ...
        try:
            from noru_cli.commands import _iter_plugin_command_entries
    ...
    async def _auto_create_thread(self, message: 'DiscordMessage') -> Optional[Any]:
        ...
        thread_name = content[:80] if content else "Noru"
        if len(content) > 80:
...
def _is_connected(config) -> bool:
    ...
    Looks up via ``noru_cli.gateway.get_env_value`` at call time (not via
