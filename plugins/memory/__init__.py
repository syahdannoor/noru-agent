...
2. User-installed providers: ``$NORU_HOME/plugins/<name>/``
...
def _get_user_plugins_dir() -> Optional[Path]:
    """Return ``$NORU_HOME/plugins/`` or None if unavailable."""
    try:
        from noru_constants import get_noru_home
        d = get_noru_home() / "plugins"
...
def load_memory_provider(name: str) -> Optional["MemoryProvider"]:
    ...
    (``$NORU_HOME/plugins/<name>/``) directories.  Bundled takes
