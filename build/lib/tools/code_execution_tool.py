...
def execute_code(
    ...
    try:
        ...
        # {NORU_HOME}/home/ when that directory exists.
        ...
        # but scripts can still read secrets from disk (e.g. open('~/.noru/.env')).
...
def build_execute_code_schema(enabled_sandbox_tools: set = None,
    ...
    if mode == "strict":
        cwd_note = (
            ...
            "(os.path.expanduser('~/.noru/.env')) or terminal()/read_file() for user files."
        )
    else:
        cwd_note = (
