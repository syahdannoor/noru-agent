...
def _inject_context_hermes_home(env: dict) -> None:
    ...
    try:
        from noru_constants import get_noru_home_override

        value = get_noru_home_override()
        if value:
            env["NORU_HOME"] = value
    except Exception:
        pass
...
def _find_bash() -> str:
    ...
    _hermes_portable_git = os.path.join(_local_appdata, "noru", "git") if _local_appdata else ""
...
def _make_run_env(env: dict) -> dict:
    ...
    # npm …) into {NORU_HOME}/home/ when that directory exists.  Only the
...
class LocalEnvironment(BaseEnvironment):
    ...
    def get_temp_dir(self) -> str:
        ...
        **Windows:** hardcoded ``/tmp`` is wrong in two ways — native Python
        ...
        ``NORU_HOME`` instead — single-word path, guaranteed to exist, same
        string resolves in both Git Bash and native Python.
        """
        if _IS_WINDOWS:
            # Derive a Windows-safe temp dir under NORU_HOME.  Using
            ...
            try:
                ...
                cache_dir = get_noru_home() / "cache" / "terminal"
            except Exception:
