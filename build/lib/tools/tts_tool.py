...
- Users can declare any number of named providers with ``type: command``
  under ``tts.providers.<name>`` in ``~/.noru/config.yaml``. Hermes
...
Configuration is loaded from ~/.noru/config.yaml under the 'tts:' key.
...
def get_env_value(name, default=None):
    ...
    Tests may monkeypatch and later restore ``noru_cli.config.get_env_value``
    before this module is imported. Resolve the helper at call time so TTS does
    not keep a stale imported function for the rest of the test process.
    """
    try:
        from noru_cli.config import get_env_value as _get_env_value
    except ImportError:
...
# ===========================================================================
# Config loader -- reads tts: section from ~/.noru/config.yaml
# ===========================================================================
def _load_tts_config() -> Dict[str, Any]:
    """
    Load TTS configuration from ~/.noru/config.yaml.
    ...
    try:
        ...
        logger.debug("noru_cli.config not available, using default TTS config")
        return {}
    except Exception as e:
...
def _get_piper_voices_dir() -> Path:
    ...
    Resolves to ``~/.noru/cache/piper-voices/`` under the active
    NORU_HOME so voice downloads follow profile boundaries.
    """
    from noru_constants import get_hermes_dir
...
def text_to_speech_tool(
    ...
    Reads provider/voice config from ~/.noru/config.yaml (tts: section).
...
TTS_SCHEMA = {
    ...
    "parameters": {
        ...
        "properties": {
            ...
            "output_path": {
                "type": "string",
                "description": f"Optional custom file path to save the audio. Defaults to {display_noru_home()}/audio_cache/<timestamp>.mp3"
            }
        },
        "required": ["text"]
    }
