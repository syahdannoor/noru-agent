...
Copies bundled skills from the repo's skills/ directory into ~/.noru/skills/
...
The manifest lives at ~/.noru/skills/.bundled_manifest.
"""
...
from noru_constants import get_bundled_skills_dir, get_noru_home, get_optional_skills_dir
...
# When present in NORU_HOME, sync_skills() is a no-op so neither the
# installer, `hermes update`, nor a direct sync re-injects bundled skills.
# Delete the file to opt back in. Mirrors
# noru_cli.profiles.NO_BUNDLED_SKILLS_MARKER (kept as a literal here to
...
def _read_suppressed_names() -> set:
    ...
    to reading ``~/.noru/skills/.curator_suppressed`` directly if that import
...
def _compute_relative_dest(skill_dir: Path, bundled_dir: Path) -> Path:
    ...
    e.g., bundled/skills/mlops/axolotl -> ~/.noru/skills/mlops/axolotl
    """
...
def sync_skills(quiet: bool = False) -> dict:
    """
    Sync bundled skills into ~/.noru/skills/ using the manifest.
    ...
    if (NORU_HOME / NO_BUNDLED_SKILLS_MARKER).exists():
        if not quiet:
    ...
    for skill_name, skill_src in bundled_skills:
        ...
        # (~/.noru/skills/.curator_suppressed) is written when the curator
...
def set_bundled_skills_opt_out(enabled: bool) -> dict:
    ...
    When ``enabled`` is True, writes NORU_HOME/.no-bundled-skills so the
    ...
    Returns:
        dict with keys: ok (bool), changed (bool), marker (str path),
                        message (str).
    """
    marker = NORU_HOME / NO_BUNDLED_SKILLS_MARKER
    existed = marker.exists()
    try:
        if enabled:
            NORU_HOME.mkdir(parents=True, exist_ok=True)
            marker.write_text(
...
def is_bundled_skills_opt_out() -> bool:
    ...
    return (NORU_HOME / NO_BUNDLED_SKILLS_MARKER).exists()
...
if __name__ == "__main__":
    print("Syncing bundled skills into ~/.noru/skills/ ...")
