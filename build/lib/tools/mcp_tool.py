...
Configuration is read from ~/.noru/config.yaml under the ``mcp_servers`` key.
...
# per-profile log file (~/.noru/logs/mcp-stderr.log), tagged with the
    ...
    if os.sep not in resolved_command:
        ...
        if which_hit:
            ...
            hermes_home = os.path.expanduser(
                os.getenv(
                    "NORU_HOME", os.path.join(os.path.expanduser("~"), ".hermes")
                )
            )
            candidates = [
...
def _handle_auth_error_and_retry(
    ...
    return json.dumps({
        "error": (
            ...
            f"file under ~/.noru/mcp-tokens/ and restart). Do NOT retry "
...
def _load_mcp_config() -> Dict[str, dict]:
    ...
    ``os.environ`` (which includes ``~/.noru/.env`` loaded at startup).
    """
    try:
