...
# appends every unhandled exception to ~/.noru/logs/tui_gateway_crash.log
...
def resolve_skin() -> dict:
    try:
        from noru_cli.skin_engine import init_skin_from_config, get_active_skin
...
def _resolve_startup_runtime() -> tuple[str, str | None]:
    ...
    try:
        from noru_cli.models import detect_static_provider_for_model
...
def _load_enabled_toolsets() -> list[str] | None:
    ...
    if explicit and validate_toolset is not None:
        ...
        try:
            ...
            from noru_cli.tools_config import _parse_enabled_flag
...
def _apply_model_switch(sid: str, session: dict, raw_input: str) -> dict:
    from noru_cli.model_switch import parse_model_flags, switch_model
    ...
    custom_provs = None
    try:
        from noru_cli.config import get_compatible_custom_providers, load_config
...
def _session_info(agent, session: dict | None = None) -> dict:
    ...
    try:
        ...
        pass
    try:
        from noru_cli.banner import get_update_result
...
def _wire_callbacks(sid: str):
    ...
    def secret_cb(env_var, prompt, metadata=None):
        ...
        from noru_cli.config import save_env_value_secure
...
def _available_personalities(cfg: dict | None = None) -> dict:
    try:
        ...
        try:
            from noru_cli.config import load_config as _load_full_cfg
...
#
# Layout:  $NORU_HOME/spawn-trees/<session_id>/<timestamp>.json
...
def _spawn_trees_root():
    ...
    root = get_noru_home() / "spawn-trees"
        ...
        try:
            ...
            if images:
                try:
                    ...
                    from noru_cli.config import load_config as _tui_load_config
            ...
            if (
                ...
                try:
                    from noru_cli.voice import speak_text
                    ...
                    logger.warning("voice TTS skipped: noru_cli.voice unavailable")
                except Exception as e:
...
def _(rid, params: dict) -> dict:
    ...
    if err:
        return err
    try:
        from noru_cli.clipboard import has_clipboard_image, save_clipboard_image
    except Exception as e:
...
def _(rid, params: dict) -> dict:
    ...
    if key == "provider":
        try:
            from noru_cli.models import list_available_providers, normalize_provider
    ...
    if key == "profile":
        ...
        return _ok(rid, {"home": str(_hermes_home), "display": display_noru_home()})
    if key == "project":
...
def _(rid, params: dict) -> dict:
    try:
        from noru_cli.main import _has_any_provider_configured
...
def _(rid, params: dict) -> dict:
    ...
    try:
        from noru_cli.runtime_provider import resolve_runtime_provider
        from noru_cli.auth import has_usable_secret
        from noru_cli.main import _has_any_provider_configured
...
@method("reload.env")
def _(rid, params: dict) -> dict:
    """Re-read ``~/.noru/.env`` into the gateway process via
    ``noru_cli.config.reload_env``, matching classic CLI's ``/reload``
...
@method("cli.exec")
def _(rid, params: dict) -> dict:
    """Run `python -m noru_cli.main` with argv; capture stdout/stderr (non-interactive only)."""
    ...
    try:
        r = subprocess.run(
            [sys.executable, "-m", "noru_cli.main", *argv],
            capture_output=True,
            text=True,
...
def _(rid, params: dict) -> dict:
    ...
    try:
        from noru_cli.commands import SlashCommandCompleter
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        from noru_cli.config import is_managed, save_env_value
        from noru_cli.inventory import build_models_payload, load_picker_context
...
def _(rid, params: dict) -> dict:
    ...
    """
    try:
        from noru_cli.auth import PROVIDER_REGISTRY, clear_provider_auth
        from noru_cli.config import remove_env_value
    ...
    * ``tts`` — toggle speech-output of agent replies. Requires mode on
    ...
    if action in {"on", "off"}:
        ...
        if not enabled:
            ...
            try:
                from noru_cli.voice import stop_continuous
...
def _(rid, params: dict) -> dict:
    ...
    try:
        if action == "start":
            ...
            from noru_cli.voice import start_continuous
...
def _failure_messages(url: str, port: int, system: str) -> list[str]:
    from noru_cli.browser_connect import manual_chrome_debug_command
...
def _browser_connect(rid, params: dict) -> dict:
    ...
    from noru_cli.browser_connect import DEFAULT_BROWSER_CDP_URL
    ...
    try:
        ...
        if parsed.scheme in {"ws", "wss"} and parsed.path.startswith(
            ...
            if not ok and _is_default_local_cdp(parsed):
                from noru_cli.browser_connect import try_launch_chrome_debug
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        if action == "install":
            from noru_cli.skills_hub import do_install
        ...
        if action == "browse":
            from noru_cli.skills_hub import browse_skills
        ...
        if action == "inspect":
            from noru_cli.skills_hub import inspect_skill
