...
class QQAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # to ~/.noru/.update_response. Set here so the cross-adapter gateway
    ...
    async def _default_interaction_dispatch(
        ...
        - ``update_prompt:<answer>`` →
          writes the answer to ``~/.noru/.update_response`` for the
    ...
    async def send_update_prompt(
        ...
        ``~/.noru/.update_response`` so the detached update process
        can read it.
        """
