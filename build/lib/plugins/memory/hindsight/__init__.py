...
Or via $NORU_HOME/hindsight/config.json (profile-scoped), falling back to
...
def _load_config() -> dict:
    ...
    Resolution order:
      1. $NORU_HOME/hindsight/config.json  (profile-scoped)
    ...
    profile_path = get_noru_home() / "hindsight" / "config.json"
    ...
    return {
        ...
        "banks": {
            "noru": {
                "bankId": os.environ.get("HINDSIGHT_BANK_ID", "noru"),
...
def _embedded_profile_name(config: dict[str, Any]) -> str:
    ...
    profile = config.get("profile", "noru")
    return str(profile or "noru")
...
class HindsightMemoryProvider(MemoryProvider):
    ...
    def save_config(self, values, hermes_home):
        """Write config to $NORU_HOME/hindsight/config.json."""
        import json
        from pathlib import Path
    ...
    def post_setup(self, hermes_home: str, config: dict) -> None:
        ...
        from noru_cli.memory_setup import _curses_select
        ...
        provider_config.setdefault("bank_id", "noru")
    ...
    def get_config_schema(self):
        return [
            ...
            {"key": "bank_id", "description": "Memory bank name (static fallback when bank_id_template is unset)", "default": "noru"},
        ...
        if self._client is None:
            if self._mode == "local_embedded":
                ...
                logger.debug("Creating HindsightEmbedded client (profile=%s, provider=%s)",
                             self._config.get("profile", "noru"), llm_provider)
                kwargs = dict(
                    profile=self._config.get("profile", "noru"),
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        banks = cfg_get(self._config, "banks", "noru", default={})
        static_bank_id = self._config.get("bank_id") or banks.get("bankId", "noru")
