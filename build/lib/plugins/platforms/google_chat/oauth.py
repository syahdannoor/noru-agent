...
- Per-user tokens (keyed by sender email):
    ``${NORU_HOME}/google_chat_user_tokens/<sanitized_email>.json``
- Legacy single-user token (fallback, untouched for backward compat):
    ``${NORU_HOME}/google_chat_user_token.json``
- Per-user pending OAuth state during /setup-files start → exchange:
    ``${NORU_HOME}/google_chat_user_oauth_pending/<sanitized_email>.json``
- Legacy pending state:
    ``${NORU_HOME}/google_chat_user_oauth_pending.json``
- Shared OAuth client (one per host):
    ``${NORU_HOME}/google_chat_user_client_secret.json``
"""
...
# Use the project's NORU_HOME helper so the token follows the user's
# profile (e.g. tests can override via NORU_HOME=/tmp/...).
try:
    from noru_constants import display_noru_home, get_noru_home
except (ModuleNotFoundError, ImportError):
    # Fallback for environments where noru_constants isn't importable
...
def _hermes_home() -> Path:
    """Resolve NORU_HOME at call time (NOT module import).

    Tests and ``NORU_HOME=...`` env overrides need this to be late-
...
# ``ls ~/.noru/google_chat_user_tokens/`` trivial.
...
def store_client_secret(path: str) -> None:
    """Validate and copy the user's OAuth client_secret.json into NORU_HOME."""
...
def exchange_auth_code(code: str, email: Optional[str] = None) -> None:
    ...
    rel_label = (
        f"{display_noru_home()}/google_chat_user_tokens/{_sanitize_email(email)}.json"
        if email
        else f"{display_noru_home()}/google_chat_user_token.json"
    )
