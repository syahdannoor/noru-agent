...
class LineAdapter(BasePlatformAdapter):
    ...
    async def _handle_media(self, request) -> Any:
        ...
        ``/private/tmp`` on macOS), and ``NORU_HOME``. PR #8398.
        """
        from aiohttp import web
...
def interactive_setup() -> None:
    ...
    one optional public URL. Writes to ``~/.noru/.env`` via ``noru_cli.config``.
    """
    print()
    ...
    try:
        from noru_cli.config import get_env_var, set_env_var
    except ImportError:
        print("noru_cli.config not available; set LINE_* vars manually in ~/.noru/.env")
        return
