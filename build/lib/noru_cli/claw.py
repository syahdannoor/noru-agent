...
from noru_cli.config import get_noru_home, get_config_path, load_config, save_config
...
def _cmd_migrate(args):
    ...
    if preview_conflicts > 0 and not overwrite:
        ...
        print_info(
            "Each conflict is an item whose target already exists in ~/.noru/. "
    ...
    # Delegates to noru_cli.backup.create_pre_migration_backup(), which
    ...
    if not no_backup:
        try:
            from noru_cli.backup import create_pre_migration_backup, _format_size
