...
from noru_cli import __version__ as VERSION, __release_date__ as RELEASE_DATE
...
_UPSTREAM_REPO_URL = "https://github.com/syahdannoor/noru-agent.git"
...
def _fetch_pypi_latest(package: str = "noru-agent") -> Optional[str]:
...
def check_for_updates() -> Optional[int]:
    """Check whether a Noru update is available.
    ...
    if embedded_rev:
        ...
        # $NORU_HOME/hermes-agent/ may be a stale copy from --clone-all;
        # Path(__file__) always resolves to the actual installed checkout.
        repo_dir = Path(__file__).parent.parent.resolve()
        if not (repo_dir / ".git").exists():
            repo_dir = hermes_home / "noru-agent"
        if not (repo_dir / ".git").exists():
...
def _resolve_repo_dir() -> Optional[Path]:
    """Return the active Noru git checkout, or None if this isn't a git install.

    Prefers the running code's location over the profile-scoped path
    because ``$NORU_HOME/hermes-agent/`` may be a stale copy carried
    over by ``--clone-all``.
    """
    repo_dir = Path(__file__).parent.parent.resolve()
    if not (repo_dir / ".git").exists():
...
def get_git_banner_state(repo_dir: Optional[Path] = None) -> Optional[dict]:
    ...
    if repo_dir is None:
        ...
        try:
            from noru_cli.build_info import get_build_sha
...
_RELEASE_URL_BASE = "https://github.com/syahdannoor/noru-agent/releases/tag"
...
def get_latest_release_tag(repo_dir: Optional[Path] = None) -> Optional[tuple]:
    ...
    Noru checkout. Cached per-process. Release URL always points at the
    canonical syahdannoor/noru-agent repo (forks don't get a link).
    """
...
def format_banner_version_label() -> str:
    ...
    base = f"Noru Agent v{VERSION} ({RELEASE_DATE})"
...
def build_welcome_banner(console: "Console", model: str, cwd: str,
    ...
    try:
        from noru_cli.codex_runtime_switch import get_current_runtime
        from noru_cli.config import load_config as _load_cfg
    ...
    try:
        ...
        if behind is not None and behind != 0:
            from noru_cli.config import get_managed_update_command, recommended_update_command
            if behind > 0:
