...
Config files are stored in ~/.noru/ for easy access:
- ~/.noru/config.yaml  - All settings (model, toolsets, terminal, etc.)
- ~/.noru/.env         - API keys and secrets
...
def _warn_config_parse_failure(config_path: Path, exc: Exception) -> None:
    ...
    A YAML parse error in ``~/.noru/config.yaml`` causes ``load_config()``
...
# * ``NORU_HOME`` / ``HERMES_PROFILE`` / ``HERMES_CONFIG`` /
...
_ENV_VAR_NAME_DENYLIST: frozenset[str] = frozenset({
    ...
    "NORU_HOME", "HERMES_PROFILE", "HERMES_CONFIG", "HERMES_ENV",
})
...
def _reject_denylisted_env_var(key: str) -> None:
    ...
    if key in _ENV_VAR_NAME_DENYLIST:
        raise ValueError(
            ...
            "(NORU_HOME, HERMES_PROFILE, ...) cannot be persisted via "
...
from noru_cli.default_soul import DEFAULT_SOUL_MD
...
def get_managed_system() -> Optional[str]:
    ...
    managed_marker = get_noru_home() / ".managed"
...
def is_managed() -> bool:
    ...
    or a .managed marker file in NORU_HOME (set by the NixOS activation
...
def detect_install_method(project_root: Optional[Path] = None) -> str:
    ...
    Resolution order:
    1. Stamped ``~/.noru/.install_method`` file (written by installers)
    ...
    """
    stamp = get_noru_home() / ".install_method"
    try:
...
def stamp_install_method(method: str) -> None:
    """Write the install method to ~/.noru/.install_method."""
    stamp = get_noru_home() / ".install_method"
    try:
...
Notes:
  ...
  • Your config and session history live under ``$NORU_HOME`` (``/opt/data``
...
def get_container_exec_info() -> Optional[dict]:
    """Read container mode metadata from NORU_HOME/.container-mode.
    ...
    container_mode_file = get_noru_home() / ".container-mode"
    ...
    container_name = info.get("container_name", "noru-agent")
    exec_user = info.get("exec_user", "noru")
...
# Re-export from noru_constants — canonical definition lives there.
from noru_constants import get_noru_home  # noqa: F811,E402
from utils import atomic_replace

def get_config_path() -> Path:
    """Get the main config file path."""
    return get_noru_home() / "config.yaml"

def get_env_path() -> Path:
    """Get the .env file path (for API keys)."""
    return get_noru_home() / ".env"
...
def _resolve_hermes_uid_gid() -> tuple[Optional[int], Optional[int]]:
    ...
    ownership. The entrypoint chowns the top-level NORU_HOME once, but
...
def _secure_dir(path):
    ...
    The mode can be overridden via the NORU_HOME_MODE environment variable
    (e.g. NORU_HOME_MODE=0701) for deployments where a web server (nginx,
    caddy, etc.) needs to traverse NORU_HOME to reach a served subdirectory.
    ...
    if is_managed():
        return
    try:
        mode_str = os.environ.get("NORU_HOME_MODE", "").strip()
...
def _ensure_default_soul_md(home: Path) -> None:
    """Seed a default SOUL.md into NORU_HOME if the user doesn't have one yet."""
...
def _ensure_hermes_home_managed(home: Path):
    ...
    if not home.is_dir():
        raise RuntimeError(
            f"NORU_HOME {home} does not exist. "
...
DEFAULT_CONFIG = {
    ...
    "checkpoints": {
        ...
        "max_snapshots": 20,
        # Hard ceiling on total ``~/.noru/checkpoints/`` size (MB).  When
    ...
    "tts": {
        ...
        "piper": {
            ...
            # "voices_dir": "",        # Override voice cache dir; default = ~/.noru/cache/piper-voices/
    ...
    # a plugin in plugins/context_engine/<name>/ or ~/.noru/plugins/.
    "context": {
        "engine": "compressor",
    },
    ...
    "curator": {
        ...
        # skipped), snapshot ~/.noru/skills/ into
        # ~/.noru/skills/.curator_backups/<utc-iso>/skills.tar.gz so the
    ...
    "whatsapp": {
        ...
        # Default (None) uses the built-in "⚕ *Noru Agent*" header.
    ...
    # stored approval from ~/.noru/shell-hooks-allowlist.json.
    ...
    # Logging — controls file logging to ~/.noru/logs/.
    ...
    "gateway": {
        ...
        # system-path denylist (/etc, /proc, ~/.ssh, ~/.aws, ~/.noru/.env,
        ...
        # cache (~/.noru/cache/{documents,images,audio,video,screenshots})
    ...
    # Session storage — controls automatic cleanup of ~/.noru/state.db.
    ...
    "sessions": {
        ...
        # rewrites ``~/.noru/sessions/session_{sid}.json`` on every turn
    ...
    "updates": {
        # Run a full ``hermes backup``-style zip of NORU_HOME before every
        # ``hermes update``.  Backups land in ``<NORU_HOME>/backups/`` and
        # can be restored with ``hermes import <path>``.  Off by default —
        # on large NORU_HOME directories the zip can add minutes to every
    ...
    "lsp": {
        ...
        #              ``<NORU_HOME>/lsp/bin/`` on first use.
    ...
    # rather than storing them in ~/.noru/.env.
    "secrets": {
        "bitwarden": {
            ...
            # in ~/.noru/.env (or your shell) and never in config.yaml.
            ...
            # ~/.noru/bin/ on first use.  When False you must install
...
OPTIONAL_ENV_VARS = {
    ...
    "API_SERVER_MODEL_NAME": {
        "description": "Model name advertised on /v1/models. Defaults to the profile name (or 'noru-agent' for the default profile). Useful for multi-user setups with OpenWebUI.",
...
def get_custom_provider_context_length(
    ...
    used by:
      ...
      * ``noru_cli.model_switch.resolve_display_context_length`` (``/model`` confirmation display)
...
def warn_deprecated_cwd_env_vars(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    if lines:
        hint_path = os.environ.get("NORU_HOME", "~/.hermes")
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    if current_ver < 21:
        ...
        if "enabled" not in plugins_cfg:
            ...
            # Scan ``$NORU_HOME/plugins/`` for currently installed user plugins.
            grandfathered: List[str] = []
            try:
                user_plugins_dir = get_noru_home() / "plugins"
    ...
    #   3. Creates `~/.noru/logs/curator/` if missing (belt-and-suspenders
    #      on top of ensure_hermes_home() — old profiles that predate this
    #      migration still benefit).
    if current_ver < 23:
        try:
            curator_dir = get_noru_home() / "logs" / "curator"
...
def read_raw_config() -> Dict[str, Any]:
    """Read ~/.noru/config.yaml as-is, without merging defaults or migrating.
...
def load_config() -> Dict[str, Any]:
    """Load configuration from ~/.noru/config.yaml.
    ...
    (which change ``NORU_HOME`` and therefore ``get_config_path()``)
    don't collide.
...
def save_config(config: Dict[str, Any]):
    """Save configuration to ~/.noru/config.yaml."""
    with _CONFIG_LOCK:
        if is_managed():
...
def load_env() -> Dict[str, str]:
    """Load environment variables from ~/.noru/.env.
...
def sanitize_env_file() -> int:
    """Read, sanitize, and rewrite ~/.noru/.env in place.
...
def save_env_value(key: str, value: str):
    """Save or update a value in ~/.noru/.env."""
    if is_managed():
...
def remove_env_value(key: str) -> bool:
    """Remove a key from ~/.noru/.env and os.environ.
...
def reload_env() -> int:
    """Re-read ~/.noru/.env into os.environ. Returns count of vars updated.
...
def get_env_value(key: str) -> Optional[str]:
    """Get a value from ~/.noru/.env or environment."""
