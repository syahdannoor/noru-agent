...
Each profile is a fully independent NORU_HOME directory with its own
config.yaml, .env, memory, sessions, skills, gateway, cron, and logs.
Profiles live under ``~/.noru/profiles/<name>/`` by default.
...
_PROFILE_DIRS = [
    ...
    # See noru_constants.get_subprocess_home() and issue #4426.
    "home",
]
...
_DEFAULT_EXPORT_EXCLUDE_ROOT = frozenset({
    # Infrastructure
    "noru-agent",         # repo checkout (multi-GB)
    ".worktrees",           # git worktrees
...
_RESERVED_NAMES = frozenset({
    "noru", "default", "test", "tmp", "root", "sudo",
})
...
def _get_profiles_root() -> Path:
    ...
    Anchored to the hermes root, NOT to the current NORU_HOME
    (which may itself be a profile).  This ensures ``coder profile list``
    can see all profiles.

    In Docker/custom deployments where NORU_HOME points outside
    ``~/.hermes``, profiles live under ``NORU_HOME/profiles/`` so
...
def _get_default_hermes_home() -> Path:
    """Return the default (pre-profile) NORU_HOME path.

    In standard deployments this is ``~/.hermes``.
    In Docker/custom deployments where NORU_HOME is outside ``~/.hermes``
    (e.g. ``/opt/data``), returns NORU_HOME directly.
    """
...
def validate_profile_name(name: str) -> None:
    ...
    collisions (a ``hermes`` profile inside ``~/.noru/``) or get refused
...
def get_profile_dir(name: str) -> Path:
    """Resolve a profile name to its NORU_HOME directory."""
...
def create_profile(
    ...
    if clone_all and source_dir:
        # Full copy of source profile (exclude sibling ~/.noru/profiles/)
        shutil.copytree(
            source_dir,
            profile_dir,
...
def seed_profile_skills(profile_dir: Path, quiet: bool = False) -> Optional[dict]:
    ...
    Uses subprocess because sync_skills() caches NORU_HOME at module level.
    ...
    try:
        result = subprocess.run(
            ...
            env={**os.environ, "NORU_HOME": str(profile_dir)},
...
def _maybe_register_gateway_service(profile_name: str) -> None:
    ...
    try:
        ...
        from noru_cli.service_manager import get_service_manager
...
def _cleanup_gateway_service(name: str, profile_dir: Path) -> None:
    ...
    # Temporarily set NORU_HOME so _profile_suffix resolves correctly
    old_home = os.environ.get("NORU_HOME")
    try:
        os.environ["NORU_HOME"] = str(profile_dir)
        from noru_cli.gateway import get_service_name, get_launchd_plist_path
        ...
        if old_home is not None:
            os.environ["NORU_HOME"] = old_home
        elif "NORU_HOME" in os.environ:
            del os.environ["NORU_HOME"]
...
def set_active_profile(name: str) -> None:
    ...
    Writes to ``~/.noru/active_profile``. Use ``"default"`` to clear.
    """
    canon = normalize_profile_name(name)
    validate_profile_name(canon)
...
def get_active_profile_name() -> str:
    """Infer the current profile name from NORU_HOME.

    Returns ``"default"`` if NORU_HOME is not set or points to ``~/.hermes``.
    Returns the profile name if NORU_HOME points into ``~/.noru/profiles/<name>``.
    Returns ``"custom"`` if NORU_HOME is set to an unrecognized path.
    """
    from noru_constants import get_noru_home
...
def resolve_profile_env(profile_name: str) -> str:
    """Resolve a profile name to a NORU_HOME path string.

    Called early in the CLI entry point, before any hermes modules
    are imported, to set the NORU_HOME environment variable.
    """
