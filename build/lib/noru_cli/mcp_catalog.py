...
- Secrets prompted at install time go to ``~/.noru/.env`` (the
...
from noru_constants import get_noru_home, get_optional_mcps_dir
...
from noru_cli.cli_output import prompt as _prompt_input
...
def _install_root() -> Path:
    ...
    root = get_noru_home() / "mcp-installs"
...
def _do_git_install(entry: CatalogEntry) -> Path:
    """Clone the entry's repo into ``~/.noru/mcp-installs/<name>`` and run
...
def _prompt_env_vars(specs: List[EnvVarSpec]) -> Dict[str, str]:
    ...
    non-secrets alike to ~/.noru/.env via save_env_value()."""
...
def _probe_tools(name: str) -> Optional[List[tuple]]:
    ...
    try:
        ...
        from noru_cli.mcp_config import _probe_single_server
