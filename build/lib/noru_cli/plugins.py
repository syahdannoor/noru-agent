...
1. **Bundled plugins** – ``<repo>/plugins/<name>/`` (shipped with hermes-agent;
   ...
   own discovery paths)
2. **User plugins**   – ``~/.noru/plugins/<name>/``
3. **Project plugins** – ``./.hermes/plugins/<name>/`` (opt-in via
   ``HERMES_ENABLE_PROJECT_PLUGINS``)
4. **Pip plugins**     – packages that expose the ``noru_agent.plugins``
   entry-point group.
...
# stderr in addition to ~/.noru/logs/agent.log. Aimed at plugin authors
...
ENTRY_POINTS_GROUP = "noru_agent.plugins"
...
class PluginManifest:
    ...
    #              in ~/.noru/plugins/ still gated by ``plugins.enabled``
    #              (untrusted code).
    kind: str = "standalone"
...
class PluginContext:
    ...
    def register_dashboard_auth_provider(self, provider) -> None:
        ...
        ``provider`` must be an instance of
        :class:`noru_cli.dashboard_auth.DashboardAuthProvider`. Used by
    ...
    def register_auxiliary_task(
        ...
        *,
        ...
        # Lazy import to avoid circular: noru_cli.main imports plugins indirectly
        from noru_cli.main import _AUX_TASKS as _BUILTIN_AUX_TASKS
    ...
    def register_skill(
        ...
        ``~/.noru/skills/`` tree and is **not** listed in the system
...
class PluginManager:
    ...
    def discover_and_load(self, force: bool = False) -> None:
        ...
        # 2. User plugins (~/.noru/plugins/)
        user_dir = get_noru_home() / "plugins"
