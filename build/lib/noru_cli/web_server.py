...
Usage:
    python -m noru_cli.main web          # Start on http://127.0.0.1:9119
    python -m noru_cli.main web --port 8080
"""
...
app = FastAPI(title="Noru Agent", version=__version__, lifespan=_lifespan)
...
#
# This list is defined in ``noru_cli.dashboard_auth.public_paths`` so the
...
# ---------------------------------------------------------------------------
from noru_cli.dashboard_auth.public_paths import (
...
async def _dashboard_auth_gate(request: Request, call_next):
    from noru_cli.dashboard_auth.middleware import gated_auth_middleware
...
async def get_status():
    ...
    try:
        from noru_cli.dashboard_auth import list_providers as _list_providers
...
async def get_system_stats():
    ...
    try:
        ...
        try:
            du = psutil.disk_usage(str(get_noru_home()))
            info["disk"] = {
                "total": du.total,
                "used": du.used,
                "free": du.free,
...
async def get_portal_status():
    ...
    features = []
    try:
        from noru_cli.nous_subscription import get_nous_subscription_features
...
# streamed to a per-action log file under ``~/.noru/logs/<action>.log`` so
# the dashboard can tail them back to the user.
# ---------------------------------------------------------------------------

_ACTION_LOG_DIR: Path = get_noru_home() / "logs"
...
def _spawn_hermes_action(subcommand: List[str], name: str) -> subprocess.Popen:
    ...
    Uses the running interpreter's ``noru_cli.main`` module so the action
    ...
    cmd = [sys.executable, "-m", "noru_cli.main", *subcommand]
...
async def speak_text(payload: TTSSpeakRequest):
    ...
    configured in ``~/.noru/config.yaml`` under ``tts.``.
    """
...
def get_recommended_default_model(provider: str = ""):
    ...
    if slug == "nous":
        try:
            ...
            from noru_cli.auth import get_provider_auth_state
...
async def set_model_assignment(body: ModelAssignment):
    ...
    Writes to ``~/.noru/config.yaml`` — applies to **new** sessions only.
    ...
    try:
        ...
        if scope == "main":
            ...
            if provider.strip().lower() == "nous":
                try:
                    from noru_cli.nous_subscription import apply_nous_managed_defaults
...
def _anthropic_oauth_status() -> Dict[str, Any]:
    ...
    1. ``~/.noru/.anthropic_oauth.json`` — Hermes-managed PKCE flow
...
def _resolve_provider_status(provider_id: str, status_fn) -> Dict[str, Any]:
    ...
    try:
        from noru_cli import auth as hauth
        if provider_id == "nous":
...
async def disconnect_oauth_provider(provider_id: str, request: Request):
    ...
    if provider_id in {"anthropic", "claude-code"}:
        ...
        try:
            from noru_cli.auth import clear_provider_auth
...
#          → persists to ~/.noru/.anthropic_oauth.json AND credential pool
...
def _codex_full_login_worker(session_id: str) -> None:
    ...
    try:
        ...
        from noru_cli.auth import _save_codex_tokens
...
async def get_logs(
    ...
    from noru_cli.logs import _read_tail, LOG_FILES

    log_name = LOG_FILES.get(file)
    if not log_name:
        raise HTTPException(status_code=400, detail=f"Unknown log file: {file}")
    log_path = get_noru_home() / "logs" / log_name
    if not log_path.exists():
        return {"file": file, "lines": []}

    try:
        from noru_logging import COMPONENT_PREFIXES
    except ImportError:
        COMPONENT_PREFIXES = {}
...
def _cron_profile_home(profile: Optional[str]) -> Tuple[str, Path]:
    """Resolve a profile query value to (profile_name, NORU_HOME)."""
    from noru_cli import profiles as profiles_mod
...
def _call_cron_for_profile(profile: Optional[str], func_name: str, *args, **kwargs):
    ...
    from the process NORU_HOME at import time. The dashboard is a single
...
#
# Wraps the same config data layer the CLI uses (noru_cli.mcp_config), so
...
async def add_mcp_server(body: MCPServerCreate):
    from noru_cli.mcp_config import _get_mcp_servers, _save_mcp_server
...
async def test_mcp_server(name: str):
    ...
    from noru_cli.mcp_config import _get_mcp_servers, _probe_single_server
...
async def list_mcp_catalog():
    ...
    """
    try:
        from noru_cli import mcp_catalog
    except Exception as exc:
...
#
# Wraps the same JSON store the CLI uses (noru_cli.webhook); the webhook
...
async def list_credential_pool():
    ...
    from noru_cli.auth import read_credential_pool
...
async def list_hooks():
    ...
    try:
        from noru_cli.plugins import VALID_HOOKS
...
async def list_checkpoints():
    ...
    cp_dir = get_noru_home() / "checkpoints"
    sessions = []
    total_bytes = 0
    if cp_dir.is_dir():
...
async def get_skills():
    ...
    from noru_cli.skills_config import get_disabled_skills
    config = load_config()
...
async def toggle_skill(body: SkillToggle):
    from noru_cli.skills_config import get_disabled_skills, save_disabled_skills
    config = load_config()
    disabled = get_disabled_skills(config)
    if body.enabled:
...
async def get_toolsets():
    from noru_cli.tools_config import (
...
def _ws_auth_ok(ws: "WebSocket") -> bool:
    ...
    if auth_required:
        ...
        from noru_cli.dashboard_auth.audit import AuditEvent, audit_log
...
def _resolve_chat_argv(
    ...
    matching what ``noru_cli.main._launch_tui`` does for the CLI path.
    ...
    """
    from noru_cli.main import PROJECT_ROOT, _make_tui_argv
...
def _build_sidecar_url(channel: str) -> Optional[str]:
    ...
    if getattr(app.state, "auth_required", False):
        ...
        from noru_cli.dashboard_auth.ws_tickets import mint_ticket
...
def _normalise_prefix(raw: Optional[str]) -> str:
    ...
    Thin re-export of :func:`noru_cli.dashboard_auth.prefix.normalise_prefix`
    ...
    """
    from noru_cli.dashboard_auth.prefix import normalise_prefix
...
def _normalise_theme_definition(data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    # YAML in ~/.noru/, same trust level as the config file itself.
...
def _discover_user_themes() -> list:
    """Scan ~/.noru/dashboard-themes/*.yaml for user-created themes.

    Returns a list of fully-normalised theme definitions ready to ship
    to the frontend, so the client can apply them without a secondary
    round-trip or a built-in stub.
    """
    themes_dir = get_noru_home() / "dashboard-themes"
...
async def get_dashboard_themes():
    ...
    from `~/.noru/dashboard-themes/*.yaml` ship with their full
...
def _discover_dashboard_plugins() -> list:
    ...
    Checks three plugin sources (same as noru_cli.plugins):
    1. User plugins:    ~/.noru/plugins/<name>/dashboard/manifest.json
    ...
    from noru_cli.plugins import get_bundled_plugins_dir
    bundled_root = get_bundled_plugins_dir()
    search_dirs = [
        (get_noru_home() / "plugins", "user"),
...
def _merged_plugins_hub() -> Dict[str, Any]:
    ...
    from noru_cli.plugins_cmd import (
    ...
    plugins_root_resolved = (get_noru_home() / "plugins").resolve()
...
async def post_agent_plugin_install(request: Request, body: _AgentPluginInstallBody):
    _require_token(request)
    from noru_cli.plugins_cmd import dashboard_install_plugin
...
async def post_agent_plugin_enable(request: Request, name: str):
    ...
    from noru_cli.plugins_cmd import dashboard_set_agent_plugin_enabled
...
async def post_agent_plugin_update(request: Request, name: str):
    _require_token(request)
    name = _validate_plugin_name(name)
    from noru_cli.plugins_cmd import dashboard_update_user_plugin
...
async def delete_agent_plugin(request: Request, name: str):
    _require_token(request)
    name = _validate_plugin_name(name)
    from noru_cli.plugins_cmd import dashboard_remove_user_plugin
...
from noru_cli.dashboard_auth.routes import router as _dashboard_auth_router  # noqa: E402
...
def start_server(
    ...
    *,
    ...
    if app.state.auth_required:
        ...
        from noru_cli.dashboard_auth import list_providers
