...
from noru_cli.auth import AuthError, resolve_provider
from noru_cli.colors import Colors, color
from noru_cli.config import get_env_path, get_env_value, get_noru_home, load_config
...
from noru_cli.runtime_provider import resolve_requested_provider
...
def show_status(args):
    ...
    if _effective_provider_label() == "LM Studio":
        from noru_cli.models import probe_lmstudio_models
        model_cfg = config.get("model")
    ...
    try:
        from noru_cli.gateway import get_gateway_runtime_snapshot, _format_gateway_pids
    ...
    jobs_file = get_noru_home() / "cron" / "jobs.json"
    if jobs_file.exists():
        import json
        try:
    ...
    sessions_file = get_noru_home() / "sessions" / "sessions.json"
