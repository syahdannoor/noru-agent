...
* We write two files: a shared ``gateway.cmd`` wrapper script (cwd + env + the
  actual ``python -m noru_cli.main gateway run --replace`` invocation) and
...
_TASK_DESCRIPTION = "Noru Agent Gateway - Messaging Platform Integration"
...
def _current_profile_cli_args() -> list[str]:
    ...
    from noru_cli.gateway import _profile_arg
...
def _launch_elevated_gateway_command(command: str, extra_args: list[str] | None = None) -> bool:
    ...
    """
    _assert_windows()
    args = ["-m", "noru_cli.main", *_current_profile_cli_args(), "gateway", command]
    if extra_args:
...
def get_task_name() -> str:
    ...
    from noru_cli.gateway import _profile_suffix
...
def get_task_script_path() -> Path:
    ...
    (or ``<NORU_HOME>/gateway-service/<task_name>.cmd`` so per-profile
    Hermes installs stay self-contained).
    """
    _assert_windows()
    from noru_cli.config import get_noru_home

    script_dir = Path(get_noru_home()) / "gateway-service"
...
def _build_gateway_cmd_script(
    ...
    The script:
      ...
      - exports NORU_HOME, PYTHONIOENCODING, VIRTUAL_ENV
      - invokes ``pythonw -m noru_cli.main [--profile X] gateway run``
    ...
    lines.append(f'set "NORU_HOME={hermes_home}"')
    lines.append('set "PYTHONIOENCODING=utf-8"')
    lines.append('set "HERMES_GATEWAY_DETACHED=1"')
    # VIRTUAL_ENV lets the gateway's own python detection find the venv
    # if someone imports noru_constants-based logic during startup.
    venv_dir = str(Path(python_path).resolve().parent.parent)
    lines.append(f'set "VIRTUAL_ENV={venv_dir}"')

    pythonw_path = _derive_venv_pythonw(python_path)
    prog_args = [pythonw_path, "-m", "noru_cli.main"]
    if profile_arg:
...
def _write_task_script() -> Path:
    ...
    hermes_home = str(Path(get_noru_home()).resolve())
...
def _build_gateway_argv() -> tuple[list[str], str, dict[str, str]]:
    ...
    argv = [python_exe, "-m", "noru_cli.main"]
    if profile_arg:
        argv.extend(profile_arg.split())
    argv.extend(["gateway", "run"])

    env_overlay = {
        "NORU_HOME": hermes_home,
...
def _spawn_detached(script_path: Path | None = None) -> int:
    ...
    We spawn ``pythonw.exe -m noru_cli.main gateway run``
    ...
    log_dir = Path(get_noru_home()) / "logs"
...
def _prompt_install_choices(
    ...
    from noru_cli.setup import prompt_yes_no
...
def _install_startup_fallback(script_path: Path, start_now: bool, detail: str) -> None:
    ...
    from noru_cli.gateway import find_gateway_pids, _profile_arg
...
def _report_gateway_start(via: str) -> None:
    ...
    if pids:
        ...
        from noru_cli.config import get_noru_home
        print(f"    type {Path(get_noru_home()).resolve()}\\logs\\gateway.log")
        print(f"    type {Path(get_noru_home()).resolve()}\\logs\\gateway-stdio.log")


def _print_next_steps() -> None:
    from noru_cli.config import get_noru_home

    hermes_home = Path(get_noru_home()).resolve()
    print()
    print("Next steps:")
...
def stop() -> None:
    ...
    """
    _assert_windows()
    from noru_cli.gateway import kill_gateway_processes, _get_restart_drain_timeout
