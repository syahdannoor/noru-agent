...
# display_noru_home imported lazily at call sites (stale-module safety during hermes update)
...
from noru_cli.cli_output import (  # noqa: E402
    print_error,
    print_info,
    print_success,
    print_warning,
)
from noru_cli.secret_prompt import masked_secret_prompt  # noqa: E402
...
def prompt_checklist(title: str, items: list, pre_selected: list = None) -> list:
    ...
    from noru_cli.curses_ui import curses_checklist
...
def _print_setup_summary(config: dict, hermes_home):
    ...
    if subscription_features.image_gen.managed_by_nous:
        ...
        try:
            ...
            from noru_cli.plugins import _ensure_plugins_discovered
    ...
    if subscription_features.video_gen.managed_by_nous:
        ...
        try:
            ...
            from noru_cli.plugins import _ensure_plugins_discovered as _ensure_plugins
            _ensure_plugins()
            _video_backend = None
    ...
    print()
    print(f"   {color('noru', Colors.GREEN)}              Start chatting")
...
def setup_model_provider(config: dict, *, quick: bool = False):
    ...
    from noru_cli.main import select_provider_and_model
    try:
...
def _setup_tts_provider(config: dict):
    ...
    if selected == "nous-openai":
        ...
        if get_env_value("VOICE_TOOLS_OPENAI_KEY") or get_env_value("OPENAI_API_KEY"):
            print_warning(
                "Direct OpenAI credentials are still configured and may take precedence until removed from ~/.noru/.env."
            )
...
def _write_slack_manifest_and_instruct():
    """Generate the Slack manifest, write it under NORU_HOME, and print
    ...
    try:
        ...
        target = Path(get_noru_home()) / "slack-manifest.json"
...
def _setup_qqbot():
    ...
    from noru_cli.gateway import _setup_qqbot as _gateway_setup_qqbot
...
def setup_gateway(config: dict):
    ...
    from noru_cli.gateway import _all_platforms, _platform_status, _configure_platform
...
def _model_section_has_credentials(config: dict) -> bool:
    ...
    Sources of truth:
      * ``PROVIDER_REGISTRY`` in ``noru_cli.auth`` — lists every supported
...
def _get_section_config_summary(config: dict, section_key: str) -> Optional[str]:
    ...
    ``get_env_value`` is the module-level import from noru_cli.config
    ...
    if section_key == "model":
        ...
        from noru_cli.gateway import _all_platforms, _platform_status
...
def _run_portal_one_shot(config: dict) -> None:
    ...
    from noru_cli.auth_commands import auth_add_command
...
def run_setup_wizard(args):
    ...
    """
    from noru_cli.config import is_managed, managed_error
    if is_managed():
...
def _run_first_time_quick_setup(config: dict, hermes_home, is_existing: bool):
    ...
    print()
    try:
        from noru_cli.main import _model_flow_nous
