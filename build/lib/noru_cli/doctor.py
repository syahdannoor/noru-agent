...
Diagnoses issues with Noru Agent setup.
"""
...
from noru_cli.config import get_project_root, get_noru_home, get_env_path
from noru_cli.env_loader import load_hermes_dotenv
from noru_constants import display_noru_home

PROJECT_ROOT = get_project_root()
NORU_HOME = get_noru_home()
_DHH = display_noru_home()  # user-facing display path (e.g. ~/.hermes or ~/.noru/profiles/coder)

# Load environment variables from ~/.noru/.env so API key checks work
_env_path = get_env_path()
load_hermes_dotenv(hermes_home=_env_path.parent, project_env=PROJECT_ROOT / ".env")

from noru_cli.colors import Colors, color
from noru_cli.models import _HERMES_USER_AGENT
from noru_constants import OPENROUTER_MODELS_URL
...
from noru_constants import is_termux as _is_termux
...
def _has_provider_env_config(content: str) -> bool:
    """Return True when ~/.noru/.env contains provider auth/base URL settings."""
...
def _has_healthy_oauth_fallback_for_apikey_provider(provider_label: str) -> bool:
    ...
    if normalized in {"google / gemini", "gemini"}:
        try:
            from noru_cli.auth import get_gemini_oauth_auth_status
            return bool((get_gemini_oauth_auth_status() or {}).get("logged_in"))
        except Exception:
            return False
    if normalized == "minimax":
        try:
            from noru_cli.auth import get_minimax_oauth_auth_status
            return bool((get_minimax_oauth_auth_status() or {}).get("logged_in"))
        except Exception:
            return False
    if normalized == "xai":
        try:
            from noru_cli.auth import get_xai_oauth_auth_status
...
def _check_version_consistency(issues: list[str]) -> None:
    """Verify pyproject.toml version matches noru_cli.__version__.
    ...
    """
    try:
        from noru_cli import __version__ as init_version
    except Exception:
        return
...
def _build_apikey_providers_list() -> list:
    ...
    try:
        ...
        try:
            from noru_cli.providers import normalize_provider as _normalize_provider
...
def run_doctor(args):
    ...
    if ack_target:
        ...
        if ack_advisory(ack_target):
            ...
            print(color(
                ...
                f"Check ~/.noru/config.yaml is writable.",
                Colors.RED,
            ))
            sys.exit(1)
        return
    ...
    print(color("│                 🩺 Noru Doctor                        │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    # Check ~/.noru/.env (primary location for user config)
    env_path = NORU_HOME / '.env'
    if env_path.exists():
    ...
    # Check ~/.noru/config.yaml (primary) or project cli-config.yaml (fallback)
    config_path = NORU_HOME / 'config.yaml'
    if config_path.exists():
        ...
        try:
            ...
            try:
                ...
                pass
            try:
                from noru_cli.config import get_compatible_custom_providers as _compatible_custom_providers
                from noru_cli.providers import (
            ...
            if runtime_provider and runtime_provider not in ("auto", "custom"):
                try:
                    if runtime_provider == "openrouter":
                        ...
                        from noru_cli.auth import PROVIDER_REGISTRY, get_auth_status
                    ...
                    if not configured:
                        _fail_and_issue(
                            ...
                            "(check ~/.noru/.env or run 'hermes setup')",
                            (
        ...
        if fallback_config.exists():
            ...
            if should_fix:
                ...
                if example_config.exists():
                    ...
                    from noru_cli.config import DEFAULT_CONFIG, save_config
    ...
    if config_path.exists():
        try:
            from noru_cli.config import check_config_version, migrate_config
    ...
    if sys.platform != "win32":
        ...
        for _venv_name in ("venv", ".venv"):
            _candidate = PROJECT_ROOT / _venv_name / "bin" / "noru"
    ...
    try:
        from noru_constants import is_container as _is_container
    ...
    try:
        import yaml as _yaml
        _mem_cfg_path = NORU_HOME / "config.yaml"
    ...
    try:
        from noru_cli.profiles import list_profiles, _get_wrapper_dir, profile_exists
        import re as _re
