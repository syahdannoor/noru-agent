...
- ``noru_cli.providers``        -- canonical provider identity + overlays
- ``noru_cli.model_normalize``  -- per-provider name formatting
...
# The previous substring check (`"noru" in name.lower()`) false-positived on
# unrelated local Modelfiles like ``hermes-brain:qwen3-14b-ctx16k`` that just
# happen to carry "noru" in their tag but are fully tool-capable.
#
...
def _ensure_direct_aliases() -> None:
    ...
    the module attribute. This keeps `from noru_cli.model_switch import
...
def switch_model(
    ...
    if explicit_provider:
        ...
        from noru_cli.models import _AGGREGATOR_PROVIDERS as _AGG_PROVIDERS
        from noru_cli.providers import ALIASES as _PROVIDER_ALIAS_TABLE
    ...
    if provider_changed or explicit_provider:
        ...
        if explicit_provider and user_providers:
            from noru_cli.providers import resolve_user_provider as _ruser
    ...
    # Mirrors the same logic in noru_cli.runtime_provider.resolve_runtime_provider;
...
def list_authenticated_providers(
    ...
    Uses the curated model lists from noru_cli/models.py (OPENROUTER_MODELS,
    ...
    def _record_builtin_endpoint(slug: str) -> None:
        ...
        try:
            from noru_cli.auth import PROVIDER_REGISTRY as _reg
        except Exception:
            return
        pcfg = _reg.get(slug)
        if not pcfg:
    ...
    for hermes_id, mdev_id in PROVIDER_TO_MODELS_DEV.items():
        ...
        if not has_creds:
            try:
                from noru_cli.auth import _load_auth_store
    ...
    from noru_cli.providers import HERMES_OVERLAYS
    from noru_cli.auth import PROVIDER_REGISTRY as _auth_registry
    ...
    for pid, overlay in HERMES_OVERLAYS.items():
        ...
        # reflects the active region (eu.*, ap.*) not the static us.* list.
            ...
            try:
                ...
                from noru_cli.auth import get_provider_auth_state as _nous_state
    ...
    # with `hermes model`).
    try:
        from noru_cli.models import CANONICAL_PROVIDERS as _canon_provs
    except ImportError:
        _canon_provs = []
...
def list_picker_providers(
    ...
    - OpenRouter's model list is replaced with the output of
      :func:`noru_cli.models.fetch_openrouter_models`, which filters the
    ...
    """
    from noru_cli.models import fetch_openrouter_models
