...
deployments where ``NORU_HOME`` points outside ``~/.hermes`` (e.g.
``/opt/hermes``), ``<root>`` is ``NORU_HOME``. Legacy env-var
...
def kanban_home() -> Path:
    ...
    2. ``get_default_hermes_root()``, which already returns ``<root>``
       when ``NORU_HOME`` is ``<root>/profiles/<name>``, and returns
       ``NORU_HOME`` directly for Docker / custom deployments.

    The kanban board is shared across profiles **by design** (see the
    module docstring). Resolving the kanban paths through the active
    profile's ``NORU_HOME`` would silently fork the board per profile,
    ...
    with _cross_process_init_lock(path):
        ...
        try:
            ...
            with _INIT_LOCK:
                ...
                # See noru_state._WAL_INCOMPAT_MARKERS for detection logic.
                from noru_state import apply_wal_with_fallback
...
def _canonical_assignee(assignee: Optional[str]) -> Optional[str]:
    ...
    if assignee is None:
        return None
    from noru_cli.profiles import normalize_profile_name
...
def has_spawnable_ready(conn: sqlite3.Connection) -> bool:
    ...
    if not rows:
        return False
    try:
        from noru_cli.profiles import profile_exists  # local import: avoids cycle
    except Exception:
...
def dispatch_once(
    ...
    *,
    ...
    if _default_assignee:
        try:
            from noru_cli.profiles import profile_exists as _pe
...
def _module_hermes_argv() -> list[str]:
    ...
    # ``noru_cli.main`` is the console-script target declared in
...
def _resolve_hermes_argv() -> list[str]:
    ...
    2. ``shutil.which("noru")`` — the console-script shim, normalized to
    ...
    3. ``sys.executable -m noru_cli.main`` — fallback for setups where
    ...
    hermes_bin = _safe_which_no_cwd("noru") if _IS_WINDOWS else shutil.which("noru")
    if hermes_bin:
...
def _kanban_worker_skill_available(hermes_home: Optional[str]) -> bool:
    ...
    # An unset NORU_HOME means the worker falls back to the default root
...
def _default_spawn(
    ...
    *,
    ...
    # Inject NORU_HOME so the worker reads the profile-scoped config.yaml
    # (fallback_providers, toolsets, agent settings, etc.) instead of the root
    # config.  Without this, `env = dict(os.environ)` copies only the parent's
    # env, and when the child process starts `hermes -p <name>` the
    # _apply_profile_override() runs *before* noru_constants is imported.
    # If NORU_HOME is absent from the child's env, get_noru_home() falls
    # back to Path.home() / ".hermes" (the DEFAULT profile root), ignoring the
    # profile-specific config entirely.  Fixes profile-scoped fallback_providers
    # being invisible to kanban workers.
    from noru_cli.profiles import resolve_profile_env
    try:
        env["NORU_HOME"] = resolve_profile_env(profile_arg)
    except FileNotFoundError:
        # Profile dir doesn't exist — defer resolution to the CLI's
        # _apply_profile_override() via HERMES_PROFILE (set below).
        # This only happens in test fixtures where the isolated
        # NORU_HOME never had profiles created.
        pass
    if task.tenant:
    ...
    # rewrites NORU_HOME), its kanban paths still match the
    ...
    cmd = [
        *_resolve_hermes_argv(),
        "-p", profile_arg,
        # Worker subprocesses switch to a profile-scoped NORU_HOME above,
    ...
    if _kanban_worker_skill_available(env.get("NORU_HOME")):
    ...
    try:
        ...
        raise RuntimeError(
            ...
            "Install Noru Agent or activate its venv before running the kanban dispatcher."
        )
...
def list_profiles_on_disk() -> list[str]:
    ...
    ``noru_cli.profiles`` (which pulls in a large chunk of the CLI startup
    path).
    """
    try:
        from noru_constants import get_default_hermes_root
