...
2. Python deps declared by user-installed plugins under ``~/.noru/plugins``
...
def _discover_plugins(hermes_home: Path) -> list[Component]:
    """Python deps declared by plugins under ``~/.noru/plugins``.
...
def run_audit(
    *,
    ...
    home = hermes_home or Path(get_noru_home())
