...
def generate_bash(parser: argparse.ArgumentParser) -> str:
    ...
    return f"""# Noru Agent bash completion
# Add to ~/.bashrc:
#   eval "$(hermes completion bash)"

_hermes_profiles() {{
    local profiles_dir="$HOME/.noru/profiles"
...
_hermes_profiles() {{
    local -a profiles
    profiles=(default)
    if [[ -d "$HOME/.noru/profiles" ]]; then
        profiles+=($HOME/.noru/profiles/*(N/:t))
    fi
...
def generate_fish(parser: argparse.ArgumentParser) -> str:
    ...
    lines: list[str] = [
        ...
        "    echo default",
        "    if test -d $HOME/.noru/profiles",
        "        for d in $HOME/.noru/profiles/*/",
        "            basename $d",
        "        end",
        "    end",
        "end",
