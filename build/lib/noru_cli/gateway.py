...
# display_noru_home is imported lazily at call sites to avoid ImportError
# when noru_constants is cached from a pre-update version during `hermes update`.
...
from noru_cli.colors import Colors, color
...
def _scan_gateway_pids(exclude_pids: set[int], all_profiles: bool = False) -> list[int]:
    ...
    pids: list[int] = []
    patterns = [
        "noru_cli.main gateway",
        "noru_cli.main --profile",
        "noru_cli.main -p",
    ...
    current_home = str(get_noru_home().resolve())
    ...
    def _matches_current_profile(command: str) -> bool:
        ...
        # the command doesn't advertise *some other* profile. NORU_HOME
...
def find_profile_gateway_processes(
    ...
    try:
        from gateway.status import get_running_pid
        from noru_cli.profiles import list_profiles
    except Exception:
        return processes
...
def _gateway_run_args_for_profile(profile: str) -> list[str]:
    args = [get_python_path(), "-m", "noru_cli.main"]
    if profile != "default":
...
def _sync_hermes_home_from_systemd_unit(system: bool) -> None:
    """When acting on a system-scope unit, adopt its ``NORU_HOME``.

    Under ``sudo``, ``NORU_HOME`` is stripped and ``HOME=/root``, so
    :func:`get_noru_home` falls back to ``/root/.hermes`` — the wrong
    profile. The unit file pins ``NORU_HOME`` for the actual gateway
    ...
    unit_home = env.get("NORU_HOME", "").strip()
    if not unit_home:
        return
    current = os.environ.get("NORU_HOME", "").strip()
    if current == unit_home:
        return
    os.environ["NORU_HOME"] = unit_home
...
def get_gateway_runtime_snapshot(system: bool = False) -> GatewayRuntimeSnapshot:
    ...
    if is_linux() and is_container():
        ...
        try:
            from noru_cli.service_manager import detect_service_manager
...
def _gateway_list() -> None:
    ...
    """
    try:
        from noru_cli.profiles import list_profiles, get_active_profile_name
    except Exception:
...
def stop_profile_gateway() -> bool:
    """Stop only the gateway for the current profile (NORU_HOME-scoped).
...
from noru_constants import is_container, is_termux, is_wsl
...
SERVICE_DESCRIPTION = "Noru Agent Gateway - Messaging Platform Integration"


def _profile_suffix() -> str:
    """Derive a service-name suffix from the current NORU_HOME.

    Returns ``""`` for the default root, the profile name for
    ``<root>/profiles/<name>``, or a short hash for any other path.
    Works correctly in Docker (NORU_HOME=/opt/data) and standard deployments.
    """
    import hashlib
    import re
    from noru_constants import get_default_hermes_root

    home = get_noru_home().resolve()
    ...
    try:
        ...
        pass
    # Fallback: short hash for arbitrary NORU_HOME paths
    return hashlib.sha256(str(home).encode()).hexdigest()[:8]


def _profile_arg(hermes_home: str | None = None) -> str:
    """Return ``--profile <name>`` only when NORU_HOME is a named profile.

    For ``~/.noru/profiles/<name>``, returns ``"--profile <name>"``.
    For the default profile or hash-based custom paths, returns the empty string.

    Args:
        hermes_home: Optional explicit NORU_HOME path. Defaults to the current
            ``get_noru_home()`` value. Should be passed when generating a
            service definition for a different user (e.g. system service).
    """
    import re
    from noru_constants import get_default_hermes_root

    home = Path(hermes_home or str(get_noru_home())).resolve()
    default = get_default_hermes_root().resolve()
    if home == default:
        return ""
    profiles_root = (default / "profiles").resolve()
...
def get_service_name() -> str:
    """Derive a systemd service name scoped to this NORU_HOME.

    Default ``~/.hermes`` returns ``hermes-gateway`` (backward compatible).
    Profile ``~/.noru/profiles/coder`` returns ``hermes-gateway-coder``.
    Any other NORU_HOME appends a short hash for uniqueness.
    """
...
def get_launchd_plist_path() -> Path:
    ...
    Profile ``~/.noru/profiles/coder`` → ``ai.hermes.gateway-coder.plist``.
    """
    suffix = _profile_suffix()
...
def _hermes_home_for_target_user(target_home_dir: str) -> str:
    """Remap the current NORU_HOME to the equivalent under a target user's home.

    When installing a system service via sudo, get_noru_home() resolves to
    root's home.  This translates it to the target user's equivalent path:
      /root/.hermes                    → /home/alice/.hermes
      /root/.hermes/profiles/coder     → /home/alice/.hermes/profiles/coder
      /opt/custom-hermes               → /opt/custom-hermes  (kept as-is)
    """
    current_hermes = get_noru_home().resolve()
...
def _stable_service_working_dir() -> str:
    ...
    uses an absolute python interpreter and ``-m noru_cli.main``, so module
    ...
    ``NORU_HOME`` is the stable anchor: it is where config/state/logs live,
    it never moves, and it is guaranteed to exist whenever the gateway is
    meaningfully installed. Fall back to ``PROJECT_ROOT`` only if NORU_HOME
...
def generate_systemd_unit(system: bool = False, run_as_user: str | None = None) -> str:
    ...
    if system:
        ...
        # Anchor cwd to the target user's NORU_HOME (stable, always exists)
...
User={username}
Group={group_name}
ExecStart={python_path} -m noru_cli.main{f" {profile_arg}" if profile_arg else ""} gateway run --replace
...
Environment="NORU_HOME={hermes_home}"
Restart=always
RestartSec=5
...
"""
    ...
    hermes_home = str(get_noru_home().resolve())
    profile_arg = _profile_arg(hermes_home)
...
def refresh_systemd_unit_if_needed(system: bool = False) -> bool:
    ...
    # sandboxed by the test conftest (only NORU_HOME is). If a test
    # exercises ``run_gateway()`` with a pytest-tmp NORU_HOME, the freshly
    # generated unit bakes that ``/tmp/pytest-of-.../hermes_test`` path into
    # ``Environment="NORU_HOME=..."``. Writing that to the developer's
...
def launchd_install(force: bool = False):
    ...
    from noru_constants import display_noru_home as _dhh
...
def launchd_status(deep: bool = False):
    ...
    if deep:
        log_file = get_noru_home() / "logs" / "gateway.log"
        if log_file.exists():
            print()
            print("Recent logs:")
...
def _guard_official_docker_root_gateway() -> None:
    ...
    print(
        "  The image entrypoint normally drops privileges to the 'noru' user. "
        "If you override entrypoint in Docker Compose, include "
        "/opt/hermes/docker/entrypoint.sh before the Hermes command."
    )
    print(
        "  Running the gateway as root can leave root-owned files in "
        "$NORU_HOME and break later non-root dashboard/gateway runs."
    )
    print(
...
def _all_platforms() -> list[dict]:
    ...
    # User-installed platform plugins under ~/.noru/plugins/ still require
...
def _platform_status(platform: dict) -> str:
    ...
    if token_var == "WHATSAPP_ENABLED":
        if val and val.lower() == "true":
            session_file = get_noru_home() / "whatsapp" / "session" / "creds.json"
...
def _setup_whatsapp():
    ...
    from noru_cli.main import cmd_whatsapp
    import argparse
...
def _setup_dingtalk():
    ...
    if method == 0:
        ...
        try:
            from noru_cli.dingtalk_auth import dingtalk_qr_auth
...
def _is_service_installed() -> bool:
    ...
    if supports_systemd_services():
        ...
        from noru_cli import gateway_windows
...
def _setup_weixin():
    ...
    print_info(
        "  3. Hermes will store the returned account_id/token in ~/.noru/.env."
    )
    print_info(
    ...
    try:
        credentials = asyncio.run(qr_login(str(get_noru_home())))
    except KeyboardInterrupt:
        print()
...
def _builtin_setup_fn(key: str):
    ...
    Late-bound to avoid a circular import with ``noru_cli.setup`` (which
    imports from this module for the remaining bespoke flows).
    """
    from noru_cli import setup as _s
...
def _configure_platform(platform: dict) -> None:
    ...
    is needed here. User-installed platform plugins under ~/.noru/plugins/
    ...
    if required:
        print_info(f"  Set these env vars in ~/.noru/.env: {', '.join(required)}")
    else:
        print_info(
...
def _dispatch_via_service_manager_if_s6(
    ...
    :mod:`noru_cli.service_manager` are caught and surfaced as
    actionable CLI messages (no raw ``CalledProcessError`` traceback).
    """
    from noru_cli.service_manager import (
    ...
    if profile is None:
        ...
        # NORU_HOME=<root>/profiles/<name>, "" for the default root,
...
def _maybe_redirect_run_to_s6_supervision(args) -> bool:
    ...
    # `docker logs` and ${NORU_HOME}/logs/gateways/<profile>/current,
...
def _gateway_command_inner(args):
    ...
    if subcmd == "install":
        ...
        if supports_systemd_services():
            ...
            print(
                "  nohup hermes gateway run > ~/.noru/logs/gateway.log 2>&1 &  # background"
            )
            sys.exit(1)
        elif is_container():
        ...
        if supports_systemd_services() and (
            ...
            if pids:
                ...
                if is_termux():
                    print(
                        "  nohup hermes gateway run > ~/.noru/logs/gateway.log 2>&1 &  # Best-effort background start"
                    )
                elif is_wsl():
                    print(
                        "  tmux new -s hermes 'hermes gateway run'         # persistent via tmux"
