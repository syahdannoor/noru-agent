...
Usage:
    ...
    hermes uninstall           Uninstall Noru Agent
...
# IMPORTANT: noru_bootstrap must be the very first import — it sets up
# UTF-8 stdio on Windows so print()/subprocess children don't hit
# UnicodeEncodeError with non-ASCII characters.  No-op on POSIX.
#
# Guarded against ModuleNotFoundError because ``noru_bootstrap`` is a
...
# new code references ``noru_bootstrap`` but the editable install's
...
try:
    import noru_bootstrap  # noqa: F401
...
def _set_process_title() -> None:
    """Set the process title to 'noru' so tools like 'ps', 'top', and
    ...
    try:
        ...
        setproctitle.setproctitle("noru")
        return
    except ImportError:
        pass
    ...
    try:
        ...
        if system == "Linux":
            ...
            libc.prctl(15, b"noru", 0, 0, 0)  # PR_SET_NAME = 15
        elif system == "Darwin":
            libc = ctypes.CDLL("libc.dylib", use_errno=True)
            libc.pthread_setname_np(b"noru")
...
# that run *before* noru_cli.config is importable. Mirrors the explicit
...
def _print_fast_version_info() -> None:
    from noru_cli import __release_date__, __version__

    project_root = os.path.abspath(os.path.join(os.path.dirname(__file__), os.pardir))
    print(f"Noru Agent v{__version__} ({__release_date__})")
...
#
# Many modules cache NORU_HOME at import time (module-level constants).
# We intercept --profile/-p from sys.argv here and set the env var so that
# every subsequent ``os.getenv("NORU_HOME", ...)`` resolves correctly.
# The flag is stripped from sys.argv so argparse never sees it.
# Falls back to ~/.noru/active_profile for sticky default.
# ---------------------------------------------------------------------------
def _apply_profile_override() -> None:
    """Pre-parse --profile/-p and set NORU_HOME before module imports."""
    argv = sys.argv[1:]
    profile_name = None
    consume = 0
    ...
    # Mirrors noru_cli.profiles._PROFILE_ID_RE so we never call
    ...
    # 1.5 If NORU_HOME is already set and no explicit flag was given, trust it
    # only when it already points to a specific profile directory.  The
    # distinguishing heuristic: a profile path has "profiles" as its immediate
    # parent directory name (e.g. ~/.noru/profiles/coder or
    # /opt/data/profiles/coder).  If NORU_HOME points to the hermes root
    # instead (e.g. systemd hardcodes NORU_HOME=/root/.hermes), we must
    # still read active_profile — the user may have switched profiles via
    # `hermes profile use` and the gateway should honour that choice.
    # See issue #22502.
    hermes_home_env = os.environ.get("NORU_HOME", "")
    ...
    if profile_name is None:
        try:
            from noru_constants import get_default_hermes_root
    ...
    # 3. If we found a profile, resolve and set NORU_HOME
    if profile_name is not None:
        try:
            from noru_cli.profiles import resolve_profile_env
            ...
            return
        os.environ["NORU_HOME"] = hermes_home
...
# Load .env from ~/.noru/.env first, then project root as dev fallback.
# User-managed env files should override stale shell exports on restart.
from noru_cli.config import get_noru_home
from noru_cli.env_loader import load_hermes_dotenv

load_hermes_dotenv(project_env=PROJECT_ROOT / ".env")

# Bridge security.redact_secrets from config.yaml → HERMES_REDACT_SECRETS env
# var BEFORE noru_logging imports agent.redact (which snapshots the flag at
...
try:
    ...
    _cfg_path = get_noru_home() / "config.yaml"
    if _cfg_path.exists():
...
try:
    from noru_logging import setup_logging as _setup_logging
...
if _FORCE_IPV4_EARLY:
    try:
        from noru_constants import apply_ipv4_preference as _apply_ipv4

        _apply_ipv4(force=True)
    except Exception:
        pass  # best-effort — don't crash if noru_constants not importable yet
...
from noru_cli import __version__, __release_date__
...
def _termux_bundled_skills_stamp_path() -> Path:
    return get_noru_home() / "skills" / ".termux_bundled_sync_stamp"
...
def _has_any_provider_configured() -> bool:
    ...
    from noru_cli.config import get_env_path, get_noru_home, load_config
    ...
    auth_file = get_noru_home() / "auth.json"
    if auth_file.exists():
        try:
            import json
...
def _exec_in_container(container_info: dict, cli_args: list):
    ...
    Args:
        ...
        cli_args: the original CLI arguments (everything after 'noru')
    """
...
def _ensure_tui_node() -> None:
    ...
    hermes_home = os.environ.get("NORU_HOME") or str(Path.home() / ".hermes")
    try:
        ...
        result = subprocess.run(
            ...
            env={**os.environ, "NORU_HOME": hermes_home},
            capture_output=True,
            text=True,
            check=False,
        )
...
def _make_tui_argv(tui_dir: Path, tui_dev: bool) -> tuple[list[str], Path]:
    ...
    def _node_bin(bin: str) -> str:
        ...
        if not path and bin == "node":
            try:
                from noru_cli.dep_ensure import ensure_dependency
...
def _normalize_tui_toolsets(toolsets: object) -> list[str]:
    ...
    try:
        from noru_cli.oneshot import _normalize_toolsets
...
def _launch_tui(
    ...
    if code == 42:
        from noru_cli.relaunch import relaunch
...
def _pin_kanban_board_env() -> None:
    ...
    if os.environ.get("HERMES_KANBAN_BOARD"):
        return
    try:
        from noru_cli.kanban_db import get_current_board
...
def _sync_bundled_skills_quietly() -> None:
    """Seed ``~/.noru/skills/`` with the bundled skill library on first launch.
...
def cmd_chat(args):
    ...
    try:
        from noru_cli.xai_retirement import (
            MIGRATION_GUIDE_URL,
            RETIREMENT_DATE,
            find_retired_xai_refs,
            format_issue,
        )
        from noru_cli.config import load_config as _load_config_for_xai_check
    ...
    if _termux_should_prefetch_update_check():
        try:
            from noru_cli.banner import prefetch_update_check
    ...
    # user's ~/.noru/config.yaml and return built-in defaults. Set BEFORE
...
def cmd_gateway(args):
    ...
    from noru_cli.gateway import gateway_command
...
def cmd_proxy(args):
    ...
    from noru_cli.proxy.cli import cmd_proxy as _cmd_proxy
...
def cmd_whatsapp(args):
    ...
    if (session_dir / "creds.json").exists():
        ...
        if wa_mode == "bot":
            ...
            print()
            print("  Tip: Agent responses are prefixed with '⚕ Noru Agent'")
        else:
            print("  Next steps:")
            print("    1. Start the gateway:  hermes gateway")
...
def cmd_postinstall(args):
    ...
    from noru_cli.config import stamp_install_method
    from noru_cli.dep_ensure import ensure_dependency
...
def cmd_model(args):
    ...
    if getattr(args, "refresh", False):
        try:
            from noru_cli.models import clear_provider_models_cache
...
def select_provider_and_model(args=None):
    ...
    # a leftover OPENAI_BASE_URL in ~/.noru/.env can poison auxiliary
...
def _clear_stale_openai_base_url():
    """Remove OPENAI_BASE_URL from ~/.noru/.env if the active provider is not 'custom'.
    ...
    """
    from noru_cli.config import get_env_value, save_env_value, load_config
...
def _all_aux_tasks() -> list[tuple[str, str, str]]:
    ...
    :meth:`noru_cli.plugins.PluginContext.register_auxiliary_task`) appear
    in the same surfaces as built-in ones without core knowing about them.
    """
    tasks = list(_AUX_TASKS)
    try:
        from noru_cli.plugins import get_plugin_auxiliary_tasks
...
def _aux_flow_provider_model(
    ...
    from noru_cli.models import get_pricing_for_provider
...
def _prompt_provider_choice(choices, *, default=0):
    ...
    """
    try:
        from noru_cli.setup import _curses_prompt_choice
...
def _model_flow_openrouter(config, current_model=""):
    ...
    from noru_constants import OPENROUTER_BASE_URL
    from noru_cli.auth import (
        ProviderConfig,
    ...
    from noru_cli.config import get_env_value

    # Route through _prompt_api_key so users can replace a stale/broken key
    # in-flow (K/R/C) instead of having to edit ~/.noru/.env by hand. The
    ...
    from noru_cli.models import model_ids, get_pricing_for_provider
...
def _model_flow_nous(config, current_model="", args=None):
    ...
    from noru_cli.nous_subscription import prompt_enable_tool_gateway
    ...
    if free_tier and not model_ids:
        ...
        if unavailable_models:
            from noru_cli.auth import DEFAULT_NOUS_PORTAL_URL
...
def _model_flow_openai_codex(config, current_model=""):
    ...
    from noru_cli.codex_models import get_codex_model_ids
    ...
    if not _codex_token:
        try:
            from noru_cli.auth import resolve_codex_runtime_credentials
...
def _model_flow_xai_oauth(_config, current_model="", *, args=None):
    ...
    from noru_cli.models import _PROVIDER_MODELS
...
def _model_flow_qwen_oauth(_config, current_model=""):
    ...
    from noru_cli.models import fetch_api_models
...
def _model_flow_custom(config):
    ...
    """
    from noru_cli.auth import _save_model_choice, deactivate_provider
    from noru_cli.config import get_env_value, load_config, save_config
    from noru_cli.secret_prompt import masked_secret_prompt
    ...
    from noru_cli.models import probe_api_models
...
def _prompt_custom_api_mode_selection(base_url: str, current_api_mode: str = "") -> Optional[str]:
    ...
    """
    from noru_cli.runtime_provider import _detect_api_mode_for_url
...
def _model_flow_azure_foundry(config, current_model=""):
    ...
    * Manual entry when every probe fails (private endpoints, etc.).
    ...
    """
    from noru_cli.auth import _save_model_choice, deactivate_provider  # noqa: F401
    from noru_cli.config import (
        get_env_value,
        save_env_value,
        load_config,
    ...
    from noru_cli import azure_detect
...
def _remove_custom_provider(config):
    ...
    try:
        from noru_cli.curses_ui import curses_radiolist
...
# downstream call sites read `noru_cli.main._PROVIDER_MODELS` directly,
...
def _model_flow_copilot(config, current_model=""):
    ...
    from noru_cli.config import save_env_value, load_config, save_config
    from noru_cli.models import (
        _PROVIDER_MODELS,
        fetch_api_models,
    ...
    if not api_key:
        ...
        if choice == "1":
            try:
                from noru_cli.copilot_auth import copilot_device_code_login
            ...
            # Validate token type
            try:
                from noru_cli.copilot_auth import validate_copilot_token
        ...
        if source in {"GITHUB_TOKEN", "GH_TOKEN"}:
            from noru_cli.env_loader import format_secret_source_suffix
...
def _prompt_api_key(pconfig, existing_key: str, provider_id: str = "") -> tuple:
    ...
    recover from a malformed paste without editing ``~/.noru/.env`` by hand.

    Returns ``(resolved_key, abort)``.  ``abort=True`` means the caller should
    ``return`` immediately — the user cancelled entry, declined to replace, or
    cleared the key and is now unconfigured.
    """
    from noru_cli.auth import LMSTUDIO_NOAUTH_PLACEHOLDER
...
def _model_flow_stepfun(config, current_model=""):
    ...
    from noru_cli.models import _PROVIDER_MODELS, fetch_api_models
...
def _model_flow_api_key_provider(config, provider_id, current_model=""):
    ...
    if provider_id == "lmstudio":
        ...
        from noru_cli.models import fetch_lmstudio_models
        ...
        from noru_cli.models import fetch_ollama_cloud_models
...
def _run_anthropic_oauth_flow(save_env_value):
    ...
    def _activate_claude_code_credentials_if_available() -> bool:
        ...
        if creds and (
            ...
            from noru_constants import display_noru_home as _dhh_fn
...
def _model_flow_anthropic(config, current_model=""):
    ...
    from noru_cli.auth import get_anthropic_key
...
def cmd_login(args):
    """Authenticate Noru CLI with a provider."""
    from noru_cli.auth import login_command
...
def cmd_logout(args):
    ...
    from noru_cli.auth import logout_command
...
def cmd_auth(args):
    ...
    from noru_cli.auth_commands import auth_command
...
def cmd_status(args):
    ...
    from noru_cli.status import show_status
...
def cmd_cron(args):
    """Cron job management."""
    from noru_cli.cron import cron_command
...
def cmd_webhook(args):
    ...
    from noru_cli.webhook import webhook_command
...
def cmd_slack(args):
    ...
    if sub == "manifest":
        from noru_cli.slack_cli import slack_manifest_command
...
def cmd_kanban(args):
    ...
    from noru_cli.kanban import kanban_command
...
def cmd_hooks(args):
    ...
    from noru_cli.hooks import hooks_command
...
def cmd_doctor(args):
    ...
    from noru_cli.doctor import run_doctor
...
def cmd_security(args):
    ...
    if sub in ("audit", None):
        from noru_cli.security_audit import cmd_security_audit
...
def cmd_dump(args):
    ...
    from noru_cli.dump import run_dump
...
def cmd_config(args):
    ...
    from noru_cli.config import config_command
...
def cmd_backup(args):
    ...
    if getattr(args, "quick", False):
        from noru_cli.backup import run_quick_backup

        run_quick_backup(args)
    else:
        from noru_cli.backup import run_backup
...
def cmd_import(args):
    ...
    from noru_cli.backup import run_import
...
def _print_version_info(*, check_updates: bool = True) -> None:
    ...
    try:
        from noru_cli.banner import check_for_updates
        from noru_cli.config import recommended_update_command
...
def cmd_uninstall(args):
    """Uninstall Noru Agent."""
    _require_tty("uninstall")
    from noru_cli.uninstall import run_uninstall
...
def _build_web_ui(web_dir: Path, *, fatal: bool = False) -> bool:
    ...
    # build path usable on a stock `py -m noru_cli.main web` invocation.
...
#
# Stamp file: $NORU_HOME/desktop-build-stamp.json
# Schema:
#   {
...
def _desktop_stamp_path() -> Path:
    """Return the path to the desktop build stamp file under $NORU_HOME."""
    from noru_constants import get_noru_home
    return get_noru_home() / "desktop-build-stamp.json"
...
def _desktop_packaged_executable(desktop_dir: Path) -> Optional[Path]:
    ...
    if sys.platform == "darwin":
        ...
        candidates = [
            release_dir / "linux-unpacked" / "noru",
            release_dir / "linux-unpacked" / "Noru",
        ]
...
def cmd_gui(args: argparse.Namespace):
    ...
    try:
        from noru_logging import setup_logging as _setup_logging_gui
...
def _find_stale_dashboard_pids() -> list[int]:
    ...
    try:
        if sys.platform == "win32":
            ...
            # with `noru_cli.gateway._scan_gateway_pids` and avoids the
...
def _update_via_zip(args):
    """Update Noru Agent by downloading a ZIP archive.
    ...
    if branch != "main":
        ...
        sys.exit(1)
    zip_url = (
        f"https://github.com/syahdannoor/noru-agent/archive/refs/heads/{branch}.zip"
    )
    ...
    from noru_cli.managed_uv import ensure_uv, rebuild_venv, update_managed_uv
...
OFFICIAL_REPO_URLS = {
    "https://github.com/syahdannoor/noru-agent.git",
    "git@github.com:syahdannoor/noru-agent.git",
    "https://github.com/syahdannoor/noru-agent",
    "git@github.com:syahdannoor/noru-agent",
}
OFFICIAL_REPO_URL = "https://github.com/syahdannoor/noru-agent.git"
...
def _should_skip_upstream_prompt() -> bool:
    ...
    return (get_noru_home() / SKIP_UPSTREAM_PROMPT_FILE).exists()
...
def _mark_skip_upstream_prompt():
    ...
    try:
        ...
        (get_noru_home() / SKIP_UPSTREAM_PROMPT_FILE).touch()
    except Exception:
        pass
...
def _sync_with_upstream_if_needed(git_cmd: list[str], cwd: Path) -> None:
    ...
    if not has_upstream:
        ...
        print("  This means you may miss updates from syahdannoor/noru-agent.")
        print()
        try:
            response = (
        ...
        if response in {"", "y", "yes"}:
            ...
            if _add_upstream_remote(git_cmd, cwd):
                print(
                    "  ✓ Added upstream: https://github.com/syahdannoor/noru-agent.git"
                )
                has_upstream = True
            else:
            ...
            print(
                "  Skipped. Run 'git remote add upstream https://github.com/syahdannoor/noru-agent.git' to add later."
            )
...
def _install_psutil_android_compat(
    ...
    *,
    ...
    from noru_cli.psutil_android import PSUTIL_URL, prepare_patched_psutil_sdist
...
def _ensure_uv_for_termux(pip_cmd: list[str]) -> str | None:
    ...
    standalone uv into ``$NORU_HOME/bin/uv``, but on Termux the official
...
class _UpdateOutputStream:
    ...
    * Every write is also mirrored to an append-only log file
      (``~/.noru/logs/update.log``) that users can inspect after the
...
def _install_hangup_protection(gateway_mode: bool = False):
    ...
    2. ``sys.stdout`` / ``sys.stderr`` are wrapped to mirror output to
       ``~/.noru/logs/update.log`` and to silently absorb
    ...
    **intentionally left alone** — those are legitimate cancellation
    ...
    try:
        ...
        # noru_cli.config.get_noru_home to simulate setup failure.
        from noru_cli.config import get_noru_home as _get_noru_home

        logs_dir = _get_noru_home() / "logs"
...
def _cmd_update_check(branch: str = "main", *, branch_explicit: bool = False):
    ...
    dropping the flag.
    """
    from noru_cli.config import detect_install_method
    ...
    if method == "docker":
        ...
        from noru_cli.config import format_docker_update_message
        print(format_docker_update_message())
        sys.exit(1)
    if method == "pip":
        from noru_cli.config import recommended_update_command
        from noru_cli.banner import check_via_pypi
...
def _ensure_fhs_path_guard() -> None:
    ...
    path_comment = (
        "# Noru Agent — ensure /usr/local/bin is on PATH " "(RHEL non-login shells)"
    )
    wrote_any = False
...
def _run_pre_update_backup(args) -> None:
    """Create a full zip backup of NORU_HOME before running the update.
    ...
    NORU_HOME directories.  The ``--backup`` flag on ``hermes update``
    ...
    try:
        from noru_cli.backup import create_pre_update_backup
    except Exception as exc:
        print(
    ...
    # Render path using display_noru_home so the user sees ~/.noru/...
    try:
        from noru_constants import get_noru_home, display_noru_home

        home = get_noru_home()
        try:
            display_path = f"{display_noru_home()}/{out_path.relative_to(home)}"
        except ValueError:
...
def cmd_update(args):
    """Update Noru Agent to the latest version.
    ...
    if is_managed():
        managed_error("update Noru Agent")
        return
...
def _cmd_update_pip(args):
    ...
    from noru_cli.config import is_uv_tool_install

    print(f"→ Current version: {__version__}")
    print("→ Checking PyPI for updates...")

    from noru_cli.managed_uv import ensure_uv, update_managed_uv
    ...
    if is_uv_tool_install():
        if not uv:
            ...
            sys.exit(1)
        cmd = [uv, "tool", "upgrade", "noru-agent"]
        ...
        cmd = [pipx, "upgrade", "noru-agent"]
    elif uv:
        cmd = [uv, "pip", "install", "--upgrade", "noru-agent"]
        if in_venv:
        ...
        cmd = [sys.executable, "-m", "pip", "install", "--upgrade", "noru-agent"]
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    if not git_dir.exists():
        if sys.platform == "win32":
            ...
            print(
                "  curl -fsSL https://raw.githubusercontent.com/syahdannoor/noru-agent/main/scripts/install.sh | bash"
            )
            sys.exit(1)
    ...
    try:
        ...
        # though `git pull` can't touch $NORU_HOME, this is cheap
        ...
        try:
            from noru_cli.backup import create_quick_snapshot
        ...
        # the old bytecode (e.g. get_noru_home added to noru_constants).
        removed = _clear_bytecode_cache(PROJECT_ROOT)
        if removed:
            print(
                f"  ✓ Cleared {removed} stale __pycache__ director{'y' if removed == 1 else 'ies'}"
        ...
        if (desktop_dir / "package.json").exists() and shutil.which("npm") and has_desktop_app:
            ...
            build_result = subprocess.run(
                [sys.executable, "-m", "noru_cli.main", "desktop", "--build-only"],
                cwd=PROJECT_ROOT,
                check=False,
            )
            if build_result.returncode != 0:
        ...
        # modules in this process.  Reload noru_constants so that any lazy
        # import executed below (skills sync, gateway restart) sees new
        # attributes like display_noru_home() added since the last release.
        try:
            import importlib
            import noru_constants as _hc
        ...
        # seed_profile_skills() uses subprocess with an explicit NORU_HOME so
        # it is not affected by sync_skills()'s module-level NORU_HOME cache,
        # which means the active profile is reliably synced regardless of whether
        # the caller's NORU_HOME env var points at the default or a named profile.
        try:
        ...
        try:
            from noru_cli.backup import restore_cron_jobs_if_emptied
        ...
        try:
            if sys.platform == "darwin" and shutil.which("cua-driver"):
                from noru_cli.tools_config import install_cua_driver
        ...
        if gateway_mode:
            _exit_code_path = get_noru_home() / ".update_exit_code"
            try:
...
def cmd_profile(args):
    ...
    from noru_constants import display_noru_home
    ...
    if action == "list":
        ...
        from noru_cli import profiles as _profiles_mod
        ...
        if name and not text_value and not auto_flag:
            try:
                if _profiles_mod.normalize_profile_name(name) == "default":
                    from noru_constants import get_noru_home as _hh
        ...
        from noru_cli import profile_describer as _pd
        ...
        from noru_cli.profiles import profile_exists
        ...
        from noru_cli.profiles import rename_profile
        ...
        from noru_cli.profiles import export_profile
        ...
        from noru_cli.profiles import import_profile
        ...
        import tempfile
        from noru_cli.profile_distribution import (
            plan_install,
        ...
        from noru_cli.profiles import get_profile_dir, normalize_profile_name
        ...
        from noru_cli.profile_distribution import describe_distribution, DistributionError
...
def _render_distribution_plan(plan) -> None:
    ...
    from noru_cli.profile_distribution import MANIFEST_FILENAME
    mf = plan.manifest
...
def cmd_dashboard(args):
    ...
    try:
        from noru_cli.plugins import discover_plugins
        discover_plugins()
    except Exception as exc:
    ...
    from noru_cli.web_server import start_server
...
def cmd_completion(args, parser=None):
    ...
    from noru_cli.completion import generate_bash, generate_zsh, generate_fish
...
def cmd_prompt_size(args):
    ...
    from noru_cli.prompt_size import cmd_prompt_size as _impl
...
def cmd_logs(args):
    ...
    from noru_cli.logs import tail_log, list_logs
...
def _prepare_agent_startup(args) -> None:
    ...
    if _is_tui_chat_launch(args):
        ...
        try:
            from noru_cli.mcp_startup import start_background_mcp_discovery
...
def _try_termux_fast_cli_launch() -> bool:
    ...
    from noru_cli._parser import build_top_level_parser
    ...
    if getattr(args, "oneshot", None):
        ...
        from noru_cli.oneshot import run_oneshot
...
def main():
    ...
    # Cosmetic: make the process show up as 'noru' instead of 'python3.11'
    # in ps/top/htop.  Non-fatal — just a nicer UX.
    _set_process_title()

    # Force UTF-8 stdio on Windows before anything prints.  No-op elsewhere.
    try:
        from noru_cli.stdio import configure_windows_stdio
    ...
    # =========================================================================
    from noru_cli.fallback_cmd import cmd_fallback
    ...
    secrets_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "instead of storing them in ~/.noru/.env.  Currently supports "
    ...
    from noru_cli import secrets_cli as _secrets_cli
    ...
    # migrate command
    # =========================================================================
    from noru_cli.migrate import cmd_migrate, cmd_migrate_xai
    ...
    setup_parser = subparsers.add_parser(
        ...
        description="Configure Noru Agent with an interactive wizard. "
    ...
    slack_manifest.add_argument(
        ...
        "writes to $NORU_HOME/slack-manifest.json.",
    )
    slack_manifest.add_argument(
        "--name",
        default=None,
        help='Bot display name (default: "Noru")',
    )
    slack_manifest.add_argument(
        "--description",
        default=None,
    ...
    # =========================================================================
    from noru_cli.send_cmd import register_send_subparser
    ...
    login_parser = subparsers.add_parser(
        ...
        description="Run OAuth device authorization flow for Noru CLI",
    )
    ...
    status_parser = subparsers.add_parser(
        ...
        description="Display status of Noru Agent components",
    )
    ...
    cron_create.add_argument(
        "--script",
        help=(
            "Path to a script under ~/.noru/scripts/. Default mode: "
    ...
    cron_edit.add_argument(
        "--script",
        help=(
            "Path to a script under ~/.noru/scripts/. Pass empty string to clear. "
    ...
    # =========================================================================
    from noru_cli.portal_cli import add_parser as _add_portal_parser
    _add_portal_parser(subparsers)

    # =========================================================================
    # kanban command — multi-profile collaboration board
    # =========================================================================
    from noru_cli.kanban import build_parser as _build_kanban_parser
    ...
    hooks_parser = subparsers.add_parser(
        ...
        description=(
            "Inspect shell-script hooks declared in ~/.noru/config.yaml, "
            "test them against synthetic payloads, and manage the first-use "
            "consent allowlist at ~/.noru/shell-hooks-allowlist.json."
        ),
    )
    ...
    doctor_parser = subparsers.add_parser(
        ...
        description="Diagnose issues with Noru Agent setup",
    )
    ...
    security_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "~/.noru/plugins/, and pinned npx/uvx MCP servers in config.yaml. "
    ...
    debug_parser = subparsers.add_parser(
        ...
        description="Debug utilities for Noru Agent. Use 'hermes debug share' to "
...
""",
    ...
    checkpoints_parser = subparsers.add_parser(
        "checkpoints",
        help="Inspect / prune / clear ~/.noru/checkpoints/",
    ...
    from noru_cli.checkpoints import register_cli as _register_checkpoints_cli
    ...
    config_parser = subparsers.add_parser(
        ...
        description="Manage Noru Agent configuration",
    )
    ...
    def cmd_pairing(args):
        from noru_cli.pairing import pairing_command
    ...
    skills_reset = skills_subparsers.add_parser(
        ...
        description=(
            "Clear a bundled skill's entry from the sync manifest (~/.noru/skills/.bundled_manifest) "
    ...
    def cmd_skills(args):
        ...
        if getattr(args, "skills_action", None) == "config":
            ...
            from noru_cli.skills_config import skills_command as skills_config_command

            skills_config_command(args)
        else:
            from noru_cli.skills_hub import skills_command
    ...
    from noru_cli.bundles import register_cli as _bundles_register, bundles_command
    ...
    def cmd_plugins(args):
        from noru_cli.plugins_cmd import plugins_command
    ...
    if _plugin_cli_discovery_needed():
        try:
            ...
            from noru_cli.plugins import discover_plugins, get_plugin_manager
    ...
    try:
        from noru_cli.curator import register_cli as _register_curator_cli
    ...
    def cmd_memory(args):
        ...
        if sub == "off":
            ...
            if not existing:
                print(
                    f"\n  Nothing to reset — no memory files found in {display_noru_home()}/memories/\n"
                )
                return
            ...
            print(f"  Files were in: {display_noru_home()}/memories/\n")
        else:
            from noru_cli.memory_setup import memory_command
    ...
    def cmd_tools(args):
        ...
        if action in {"list", "disable", "enable"}:
            from noru_cli.tools_config import tools_disable_enable_command

            tools_disable_enable_command(args)
        else:
            _require_tty("tools")
            from noru_cli.tools_config import tools_command
        ...
        if action == "list":
            ...
            if not args.yes:
                if not _confirm_prompt(
                    ...
                    print("Cancelled.")
                    return
            sessions_dir = get_noru_home() / "sessions"
    ...
    claw_migrate.add_argument(
        "--no-backup",
        action="store_true",
        help="Skip the pre-migration zip snapshot of ~/.noru/ (by default a "
        "single restore-point archive is written to ~/.noru/backups/ "
    ...
    def cmd_claw(args):
        from noru_cli.claw import claw_command
    ...
    update_parser = subparsers.add_parser(
        "update",
        help="Update Noru Agent to the latest version",
    ...
    uninstall_parser = subparsers.add_parser(
        "uninstall",
        help="Uninstall Noru Agent",
        description="Remove Noru Agent from your system. Can keep configs/data for reinstall.",
    )
    ...
    acp_parser = subparsers.add_parser(
        "acp",
        help="Run Noru Agent as an ACP (Agent Client Protocol) server",
        description="Start Noru Agent in ACP mode for editor integration (VS Code, Zed, JetBrains)",
    )
    ...
    acp_parser.add_argument(
        "--setup-browser",
        action="store_true",
        help="Install agent-browser + Playwright Chromium into ~/.noru/node/ "
    ...
    def cmd_acp(args):
        """Launch Noru Agent as an ACP server."""
        try:
    ...
    dashboard_parser = subparsers.add_parser(
        ...
        description="Launch the Noru Agent web dashboard for managing config, API keys, and sessions",
    )
...
""",
    ...
    from noru_cli.config import get_container_exec_info
