"""``hermes debug`` debug tools for Noru Agent.
...
Currently supports:
    hermes debug share    Upload debug report (system info + logs) to a
                          ...
                          ``~/.noru/logs/*.log`` are not leaked into
...
def _pending_file() -> Path:
    """Path to ``~/.noru/pastes/pending.json``.
    ...
    """
    return get_noru_home() / "pastes" / "pending.json"
...
def _schedule_auto_delete(urls: list[str], delay_seconds: int = _AUTO_DELETE_SECONDS):
    ...
    The replacement is stateless: we append to ``~/.noru/pastes/pending.json``
...
def _primary_log_path(log_name: str) -> Optional[Path]:
    ...
    from noru_cli.logs import LOG_FILES

    filename = LOG_FILES.get(log_name)
    return (get_noru_home() / "logs" / filename) if filename else None
