...
from noru_constants import get_noru_home, get_skills_dir, is_wsl
...
DEFAULT_AGENT_IDENTITY = (
    "You are Noru Agent, an intelligent AI assistant created by Nous Research. "
...
HERMES_AGENT_HELP_GUIDANCE = (
    If the user asks about configuring, setting up, or using Noru Agent "
    "itself, load the `hermes-agent` skill with skill_view(name='noru-agent') "
...
KANBAN_GUIDANCE = (
    ...
    "the shared board at `~/.noru/kanban.db`. Your task id is in "
...
def _skills_prompt_snapshot_path() -> Path:
    return get_noru_home() / ".skills_prompt_snapshot.json"
...
def build_skills_system_prompt(
    ...
    scanned alongside the local ``~/.noru/skills/`` directory.  External dirs
    ...
    if not skills_by_category:
        ...
        result = (
            ...
            "or troubleshoot Noru Agent itself — its CLI, config, models, providers, tools, "
...
def load_soul_md() -> Optional[str]:
    """Load SOUL.md from NORU_HOME and return its content, or None.
    ...
    """
    try:
        from noru_cli.config import ensure_hermes_home
        ensure_hermes_home()
    except Exception as e:
        logger.debug("Could not ensure NORU_HOME before loading SOUL.md: %s", e)

    soul_path = get_noru_home() / "SOUL.md"
...
def build_context_files_prompt(cwd: Optional[str] = None, skip_soul: bool = False) -> str:
    ...
    SOUL.md from NORU_HOME is independent and always included when present.
    ...
    # SOUL.md from NORU_HOME only — skip when already loaded as identity
    if not skip_soul:
