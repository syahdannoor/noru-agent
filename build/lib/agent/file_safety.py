...
def _hermes_home_path() -> Path:
    """Resolve the active NORU_HOME (profile-aware) without circular imports."""
    try:
        from noru_constants import get_noru_home  # local import to avoid cycles
        return get_noru_home()
    except Exception:
...
def _hermes_root_path() -> Path:
    ...
    try:
        from noru_constants import get_default_hermes_root  # local import to avoid cycles
...
def get_read_block_error(path: str) -> Optional[str]:
    ...
    Three categories are blocked:
      ...
      * Internal Hermes cache files under ``NORU_HOME/skills/.hub`` —
        readable metadata that an attacker could use as a prompt-injection
        carrier.
      * Credential / secret stores under NORU_HOME and the global Hermes
    ...
    **This is NOT a security boundary.** The terminal tool runs as the
    ...
    or ``cat ~/.noru/.env`` and exfiltrate the file. The read-deny exists
    ...
    # Resolve BOTH the active NORU_HOME (profile-aware) AND the global
    # Hermes root so credential stores at <root>/auth.json etc. are also
    # blocked when running under a profile (NORU_HOME points at
...
#
# Hermes profiles are separate NORU_HOME dirs under
...
# edited skills under both ``~/.noru/profiles/hermes-security/skills/``
# AND ``~/.noru/skills/`` (the default profile's skills) without realizing
# the second path belonged to a different profile.
# ---------------------------------------------------------------------------

# Profile-scoped directories under NORU_HOME / <root> / <root>/profiles/<X>/
...
def _resolve_active_profile_name() -> str:
    """Return the active profile name derived from NORU_HOME.
...
#
#   <NORU_HOME>/profiles/<name>/sandboxes/<backend>/<task>/home/.hermes/...
#
...
def classify_sandbox_mirror_target(path: str) -> Optional[dict]:
    ...
    NORU_HOME resolution would be ambiguous.
    """
    try:
        target = Path(os.path.expanduser(str(path))).resolve()
    except (OSError, RuntimeError):
...
def get_sandbox_mirror_warning(path: str) -> Optional[str]:
    ...
    return (
        ...
        f"under the real NORU_HOME. Use the host-side tool for "
...
def classify_container_mirror_target(
    ...
    under the mirror prefix. Otherwise returns:
      ...
      * ``inner_path``: portion under the mirror root (what the agent
        likely meant to address in the host NORU_HOME)
    """
...
def get_container_mirror_warning(
    ...
    return (
        ...
        f"{info['inner_path']!r} under the real NORU_HOME. Use the "
