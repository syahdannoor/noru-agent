...
def _seed_from_singletons(provider: str, entries: List[PooledCredential]) -> Tuple[bool, Set[str]]:
    ...
    try:
        from noru_cli.auth import is_source_suppressed as _is_suppressed
    except ImportError:
    ...
    if provider == "anthropic":
        ...
        # that masquerade.  Prefer ~/.noru/.env over os.environ for the
        ...
        try:
            from noru_cli.auth import resolve_qwen_runtime_credentials
        ...
        # MiniMax OAuth tokens live in ~/.noru/auth.json providers.minimax-oauth.
...
def _seed_from_env(provider: str, entries: List[PooledCredential]) -> Tuple[bool, Set[str]]:
    ...
    # Prefer ~/.noru/.env over os.environ — the user's config file is the
    ...
    # won't be re-seeded from the user's shell environment or ~/.noru/.env.
    ...
    try:
        from noru_cli.auth import is_source_suppressed as _is_source_suppressed
    except ImportError:
    ...
    def _secret_source_for_env(env_var: str) -> Optional[str]:
        try:
            from noru_cli.env_loader import get_secret_source
    ...
    if provider == "openrouter":
        # Prefer ~/.noru/.env over os.environ
