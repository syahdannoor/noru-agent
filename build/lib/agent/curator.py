...
def _state_file() -> Path:
    return get_noru_home() / "skills" / ".curator_state"
...
def _load_config() -> Dict[str, Any]:
    """Read curator.* config from ~/.noru/config.yaml. Tolerates missing file."""
    try:
...
CURATOR_REVIEW_PROMPT = (
    ...
    "into ~/.noru/skills/.archive/) is the maximum destructive action. "
    ...
    "~/.noru/skills/<umbrella>/references/ && mv ... <umbrella>/"
...
def _reports_root() -> Path:
    ...
    Lives under the profile-aware logs dir (``~/.noru/logs/curator/``)
    ...
    authored skill data in ``~/.noru/skills/``.
    ...
    that bypasses both.
    """
    root = get_noru_home() / "logs" / "curator"
    try:
...
def _render_report_markdown(p: Dict[str, Any]) -> str:
    ...
    # on disk still lives under ~/.noru/skills/.archive/ (every removal is
    ...
    if consolidated:
        ...
        lines.append(
            ...
            "The original directory was moved to `~/.noru/skills/.archive/` for "
    ...
    if pruned:
        ...
        lines.append(
            ...
            "Directories live under `~/.noru/skills/.archive/`. "
    ...
    lines.append("- All archives live under `~/.noru/skills/.archive/` and are recoverable by `mv`")
