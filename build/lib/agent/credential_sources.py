...
Hermes seeds its credential pool from many places:
    ...
    env:<VAR>     — os.environ / ~/.noru/.env
    claude_code   — ~/.claude/.credentials.json
    hermes_pkce   — ~/.noru/.anthropic_oauth.json
...
def _remove_env_source(provider: str, removed) -> RemovalResult:
    ...
    Handles three user situations:
      1. Var lives only in ~/.noru/.env  → clear it
    ...
    """
    from noru_cli.config import get_env_path, remove_env_value
...
def _remove_hermes_pkce(provider: str, removed) -> RemovalResult:
    """~/.noru/.anthropic_oauth.json is ours — delete it outright."""
    from noru_constants import get_noru_home

    result = RemovalResult()
    oauth_file = get_noru_home() / ".anthropic_oauth.json"
...
def _register_all_sources() -> None:
    ...
    register(RemovalStep(
        ...
        description="~/.noru/.anthropic_oauth.json",
    ))
    register(RemovalStep(
