...
def _get_sessions_dir() -> Path:
    """Return the sessions directory using NORU_HOME."""
    try:
        from noru_constants import get_noru_home
        return get_noru_home() / "sessions"
    except ImportError:
        return Path(os.environ.get("NORU_HOME", Path.home() / ".hermes")) / "sessions"
...
def _load_channel_directory() -> dict:
    ...
    try:
        from noru_constants import get_noru_home
        directory_file = get_noru_home() / "channel_directory.json"
    except ImportError:
        directory_file = Path(
            os.environ.get("NORU_HOME", Path.home() / ".hermes")
...
class EventBridge:
    ...
    def _poll_once(self, db):
        ...
        try:
            from noru_constants import get_noru_home
            db_file = get_noru_home() / "state.db"
        except ImportError:
            db_file = Path(os.environ.get("NORU_HOME", Path.home() / ".hermes")) / "state.db"
