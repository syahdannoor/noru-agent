...
Loads environment variables from ``~/.noru/.env``, configures logging
...
def _load_env() -> None:
    """Load .env from NORU_HOME (default ``~/.hermes``)."""
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        prog="hermes-acp",
        description="Run Noru Agent as an ACP stdio server.",
    )
...
def _print_version() -> None:
    from noru_cli import __version__ as hermes_version
...
def _run_setup() -> None:
    from noru_cli.main import main as hermes_main

    old_argv = sys.argv[:]
    try:
        sys.argv = [old_argv[0] if old_argv else "noru", "model"]
        hermes_main()
    finally:
        sys.argv = old_argv
