...
//! NORU_HOME/logs/bootstrap-<timestamp>.log.
//!
//! Lifecycle:
...
pub struct StartBootstrapArgs {
    ...
    /// Optional override for NORU_HOME. Tests use this; production
...
async fn run_bootstrap(
    ...
    let install_root = PathBuf::from(&hermes_home).join("noru-agent");

    // Copy ourselves to NORU_HOME/hermes-setup.exe so the desktop app can
    ...
    if let Err(err) = crate::paths::copy_self_to_hermes_home() {
        tracing::warn!(?err, "failed to copy installer into NORU_HOME (non-fatal)");
        emit_log(&format!(
