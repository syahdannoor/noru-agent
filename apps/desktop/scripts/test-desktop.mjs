...
const APP = (() => {
  ...
  return {
    appPath: unpacked,
    binary: path.join(unpacked, 'noru'),
    resourcesPath: path.join(unpacked, 'resources'),
    asarPath: path.join(unpacked, 'resources', 'app.asar'),
    unpackedDistIndex: path.join(unpacked, 'resources', 'app.asar.unpacked', 'dist', 'index.html')
  }
...
// Default NORU_HOME for non-sandboxed runs -- matches main.cjs's
...
// NORU_HOME and never touches this.
const DEFAULT_NORU_HOME = (() => {
  if (PLATFORM === 'win32' && process.env.LOCALAPPDATA) {
    return path.join(process.env.LOCALAPPDATA, 'noru')
  }
  return path.join(os.homedir(), '.hermes')
})()
const VENV_ROOT = path.join(DEFAULT_NORU_HOME, 'noru-agent', 'venv')
...
function launchFresh() {
  ...
  const hermesHome = path.join(sandbox, 'noru-home')
  ...
  console.log(`  NORU_HOME: ${hermesHome}`)
  console.log(`  cwd: ${cwd}`)

  return { runtimeRoot: path.join(hermesHome, 'noru-agent', 'venv') }
}
...
function validateBundle() {
  ...
  const staleFactoryMarker = path.join(APP.resourcesPath, 'noru-agent', 'hermes_cli', 'main.py')
...
function help() {
  ...
  npm run test:desktop:fresh     # build packaged app, launch with temp userData + NORU_HOME
