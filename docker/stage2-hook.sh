...
# When HERMES_UID is remapped or the top-level $NORU_HOME isn't owned by
# the runtime hermes UID, restore ownership to hermes — but ONLY for the
# directories hermes actually writes to. The full $NORU_HOME may be a
...
needs_chown=false
if [ "$(stat -c %u "$NORU_HOME" 2>/dev/null)" != "$actual_hermes_uid" ]; then
    needs_chown=true
fi
if [ "$needs_chown" = true ]; then
    echo "[stage2] Fixing ownership of $NORU_HOME (targeted) to hermes ($actual_hermes_uid)"
    # In rootless Podman the container's "root" is mapped to an
    # unprivileged host UID — chown will fail. That's fine: the volume
    # is already owned by the mapped user on the host side.
    #
    # Top-level $NORU_HOME: chown the directory itself (not its contents)
    # so hermes can mkdir new subdirs but bind-mounted host files keep
    # their existing ownership.
    chown hermes:hermes "$NORU_HOME" 2>/dev/null || \
        echo "[stage2] Warning: chown $NORU_HOME failed (rootless container?) — continuing"
    ...
    for sub in cron sessions logs hooks memories skills skins plans workspace home profiles; do
        if [ -e "$NORU_HOME/$sub" ]; then
            chown -R hermes:hermes "$NORU_HOME/$sub" 2>/dev/null || \
                echo "[stage2] Warning: chown $NORU_HOME/$sub failed (rootless container?) — continuing"
        fi
    done
    ...
    # These are under $INSTALL_DIR (not $NORU_HOME), so the bind-mount
...
# Always reset ownership of $NORU_HOME/profiles to hermes on every
...
# chown would fail.
if [ -d "$NORU_HOME/profiles" ]; then
    chown -R hermes:hermes "$NORU_HOME/profiles" 2>/dev/null || true
fi
...
# *subdirectories*; loose state files living directly under $NORU_HOME
...
# sweep so host-owned files in a bind-mounted $NORU_HOME are never
...
# entries of noru_cli.profile_distribution.USER_OWNED_EXCLUDE plus the
...
for f in \
    ...
    active_profile; do
    if [ -e "$NORU_HOME/$f" ]; then
        chown hermes:hermes "$NORU_HOME/$f" 2>/dev/null || true
    fi
done
...
if [ -f "$NORU_HOME/config.yaml" ]; then
    chown hermes:hermes "$NORU_HOME/config.yaml" 2>/dev/null || true
    chmod 640 "$NORU_HOME/config.yaml" 2>/dev/null || true
fi
...
# shell isn't a second interpreter — defends against $NORU_HOME values
...
printf 'docker\n' | as_hermes tee "$NORU_HOME/.install_method" >/dev/null \
    || true
...
seed_one() {
    dest=$1
    src=$2
    if [ ! -f "$NORU_HOME/$dest" ] && [ -f "$INSTALL_DIR/$src" ]; then
        as_hermes cp "$INSTALL_DIR/$src" "$NORU_HOME/$dest"
    fi
}
...
if [ -f "$NORU_HOME/.env" ]; then
    chown hermes:hermes "$NORU_HOME/.env" 2>/dev/null || true
    chmod 600 "$NORU_HOME/.env" 2>/dev/null || true
fi

# auth.json: bootstrap from env on first boot only. Same semantics as the
# pre-s6 entrypoint — the [ ! -f ] guard is critical to avoid clobbering
# rotated refresh tokens on container restart.
if [ ! -f "$NORU_HOME/auth.json" ] && [ -n "${HERMES_AUTH_JSON_BOOTSTRAP:-}" ]; then
    printf '%s' "$HERMES_AUTH_JSON_BOOTSTRAP" > "$NORU_HOME/auth.json"
    chown hermes:hermes "$NORU_HOME/auth.json" 2>/dev/null || true
    chmod 600 "$NORU_HOME/auth.json"
fi
...
if [ ! -f "$NORU_HOME/gateway_state.json" ] && \
        [ "${HERMES_GATEWAY_BOOTSTRAP_STATE:-}" = "running" ]; then
    printf '{"gateway_state":"running"}\n' > "$NORU_HOME/gateway_state.json"
    chown hermes:hermes "$NORU_HOME/gateway_state.json" 2>/dev/null || true
    chmod 644 "$NORU_HOME/gateway_state.json"
fi
